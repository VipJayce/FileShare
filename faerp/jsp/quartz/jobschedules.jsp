...
<%@ page import="gap.quartz.extend.ConditionBackTool" %>
...
<script language="javascript">
    writeTableTop('<fmt:message key="Scheduling_Schedules"/>','<venus:base/>/');
</script>
...
<table class="table_div_control">
    <tr>
        <td>
            <img src="<%=request.getContextPath()%>/images/icon/07-0.gif" class="div_control_image" onClick="javascript:hideshow('ccChildq',this,'<venus:base/>/')">
...
<table class="table_div_content">
    ...
    <tr>
        ...
        <td><input type="text" class="text_field_reference" name="startDate" id="startDate" />
          <img class="img_1" src="<venus:base/>/images/icon/reference.gif" onClick="javascript:getYearMonthDay('startDate','<venus:base/>/');"/>
        </td>
    ...
    -->
    <tr>        
        <td><input type="button"  class="button_ellipse" value='<fmt:message key="Query"/>' onclick="searchJob();"></td>
    </tr>
</table>
</div>
...
<table class="table_div_control">
    <tr>
        <td>
            <img src="<%=request.getContextPath()%>/images/icon/07-0.gif" class="div_control_image" onClick="javascript:hideshow('ccChild1',this,'<venus:base/>/')">
        ...
        <img name="start" src="<venus:base/>/images/icon/send.gif" title='<fmt:message key="immediate_run_the_task"/>' class="div_control_image"><fmt:message key="Run"/></td>
        <td nowrap class="button_ellipse" onClick = "javascript:jobManager(resume);">
        <img name="resume" src="<venus:base/>/images/icon/return.gif"  title='<fmt:message key="recover_the_suspended_task"/>' class="div_control_image"><fmt:message key="Recover"/></td>
        <td nowrap class="button_ellipse" onClick = "javascript:jobManager(pause);">
        <img name="pause" src="<venus:base/>/images/icon/pause.gif"  title='<fmt:message key="suspended_task_will_not_be_triggered"/>' class="div_control_image"><fmt:message key="Suspend"/></td>
        <!--        
        ...
        <img name="deleteJob" src="<venus:base/>/images/icon/delete.gif"  title='<fmt:message key="remove_the_task_from_the_schedule"/>' class="div_control_image"><fmt:message key="Delete"/></td>
        </tr>
        </table>
        </td>
    </tr>
    ...
    <tr>
        ...
        <layout:collection name="triggerList" id="triggerBo" styleClass="listCss" width="99%" indexId="orderNumber" align="center" sortAction="0">
            ...
            <layout:collectionItem width="3%" title="" style="text-align:center;">
                ...
                <input title="<%=LocaleHolder.getMessage("gap.quartz.trigger")+wy3%>" type="radio" name="checkbox_template" value="<%=wy3+","+wy4%>"/>
            </layout:collectionItem>
            
            <layout:collectionItem width="3%" title='<%=LocaleHolder.getMessage("gap.quartz.Index")%>' style="text-align:center;">
            <venus:sequence/>
                <bean:define id="wy3" name="triggerBo" property="name"/>
                <input type="hidden" signName="hiddenId" value="<%=wy3%>"/>
            </layout:collectionItem>

            <layout:collectionItem width="10%" title='<%=LocaleHolder.getMessage("gap.quartz.Trigger_Name")%>' property="name" sortable="true"/>
            <layout:collectionItem width="10%" title='<%=LocaleHolder.getMessage("gap.quartz.Trigger_Group")%>' property="group" sortable="true"/>
            <layout:collectionItem width="10%" title='<%=LocaleHolder.getMessage("gap.quartz.Job_Name")%>' property="jobName" sortable="true"/>
            <layout:collectionItem width="10%" title='<%=LocaleHolder.getMessage("gap.quartz.Job_Group")%>' property="jobGroup" sortable="true"/>
            <layout:collectionItem width="10%" title='<%=LocaleHolder.getMessage("gap.quartz.Start_Time")%>' property="startTime" sortable="true"/>
            <layout:collectionItem width="10%" title='<%=LocaleHolder.getMessage("gap.quartz.End_Time")%>' property="endTime" sortable="true"/>
            <layout:collectionItem width="10%" title='<%=LocaleHolder.getMessage("gap.quartz.State")%>' property="state" sortable="true"/>
            <layout:collectionItem width="10%" title='<%=LocaleHolder.getMessage("gap.quartz.Last_Trigger_Time")%>' property="previousFireTime" sortable="true"/>
            <layout:collectionItem width="10%" title='<%=LocaleHolder.getMessage("gap.quartz.Next_Trigger_Time")%>' property="nextFireTime" sortable="true"/>
            <!--
