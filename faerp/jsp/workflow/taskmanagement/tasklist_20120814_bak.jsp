...
<script language="javascript">
    ...
    function queryHistoryTask(){
        ...
        if(theid==null)return;
        var form = document.all["form"];
        form.action = "<%=request.getContextPath()%>/queryHistoryTask.do?taskuid="+theid;
        form.submit();
    }
    ...
    function queryTaskByCondition(){
        if (checkAllForms()) {
            ...
            if(form.workitemData!=undefined){       
                ...
                if(form.taskName.value=="" && form.procName.value=="" && form.workitemData.value=="")
                {
                    ...
                    if(isDone){
                        if(form.endTime_pre.value=="" && form.endTime_after.value==""){
                            alert('<fmt:message key="please_enter_query_condition"/>');
                ...
                form.action = "<%=request.getContextPath()%>/dynamicQueryTaskByPersonIDAction.do?done=<%=done%>&procName=<%=procName%>";
            }
...
<tr>
    <td width="13%">
      <fmt:message key="task_name"/>
    </td>
    <td>      
      <input name="taskName" type="text" class="text_field" id="taskName" maxlength="30" inputName='<fmt:message key="task_name"/>' validate="isSearch" />
    </td>
</tr>
<%if(null==registerRelevantData||"".equals(registerRelevantData)){%>
<tr>
    <td>
      <fmt:message key="process_name"/>
    </td>
    <td>      
      <input name="procName" type="text" class="text_field" id="procName" maxlength="30" inputName='<fmt:message key="process_name"/>' validate="isSearch">
    </td>
</tr>
<%}%>
<%if(done){%>
<tr>
    <td>
      <fmt:message key="complete_time"/>
    </td>
    <td>
        <input name="endTime_pre" id="endTime_pre" type="text"  class="text_field_reference"  inputName='<fmt:message key="complete_time"/>' maxlength="30" validate="notNull" readonly="true" value=""><img src="<%=request.getContextPath()%>/images/icon/reference.gif" onClick="javascript:getYearMonthDay('endTime_pre','<%=request.getContextPath()%>/')" class="img_1">
      &nbsp;<fmt:message key="to"/>&nbsp;
      <input name="endTime_after" id="endTime_after" type="text" class="text_field_reference"  inputName='<fmt:message key="complete_time"/>' maxlength="30" validate="notNull" readonly="true" value=""><img src="<%=request.getContextPath()%>/images/icon/reference.gif" onClick="javascript:getYearMonthDay('endTime_after','<%=request.getContextPath()%>/')" class="img_1">
    </td>
</tr>
<%}else{%>
...
<tr>
    <td>
      <fmt:message key="relevant_data_value"/>
    </td>
    <td>      
      <input name="workitemData" type="text" class="text_field" id="workitemData" maxlength="30" inputName='<fmt:message key="relevant_data_value"/>' validate="isSearch">
    </td>
</tr>
<tr>
    <td>
...
<table class="table_div_control">
    <tr>
        <td>
            ...
            <%if(done) {%>
                <fmt:message key="completed_task_list"/>
            <% }else {%>
                <fmt:message key="actived_task_list"/>
            <%} %>
        </td>
        <td>
            <table align="right">
