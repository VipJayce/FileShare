...
<script language="javascript">
	...
	function insert_onClick(){  //插入单条数据
    	form.action="<%=request.getContextPath()%>/ReportFileAction.do?cmd=insertFolder";
    	form.target="_parent";
	    form.submit();
	}
  	function update_onClick(id){  //保存修改后的单条数据
	    ...
	    form.action="<%=request.getContextPath()%>/ReportFileAction.do?cmd=updateFolder";
	    form.target="_parent";
    	form.submit();
	}
</script>
		...
		<table class="table_div_content_inner">
			...
			<tr>
				...
				<td align="left"><input type="text" class="text_field_readonly"
					name="parent_name" value="<%=request.getParameter("parent_name")%>" maxlength="30" />
				</td>
			</tr>
			<%}%>
			<tr>
				<td align="right"><span class="style_required_red">* </span><fmt:message key="filemanage.part_name"/></td>
...
<input type="hidden" name="code" value=""> <!--自编码--> <input
	...
	value="1"> <!--报表类型--> <input type="hidden" name="type_is_leaf"
	value="0"> <!--类型内是否叶子--> <input type="hidden" name="order_code"
