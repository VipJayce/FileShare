...
<%@ page import="gap.report.example.WebReportView"%>
...
<script>
	function simpleQuery_onClick(){  //简单的模糊查询
    	form.action=form.action+"?cmd=queryReportc";
	...
	function getBuildList() {
		var obj = window.showModalDialog('<venus:base/>/ReportAction.do?cmd=queryBuild','','dialogTop:' + event.screenY + 'px;dialogLeft:' + event.screenX + 'px;dialogHeight:300px;dialogWidth:490px;help:no;resizable:no;scroll:no;status:0;');
		if(obj) {
...
<input type="hidden" name="cmd" value=""> <input type="hidden"
	name="whereClause" value=""> <!-- 查询开始 -->
<div id="ccParent0">
<table class="table_div_control">
	<tr>
		<td><img src="<venus:base/>/images/icon/07-0.gif"
			onClick="javascript:hideshow('ccChild0',this,'<venus:base/>/')">
		<fmt:message key="template.query_by_condition"/></td>
	</tr>
</table>
</div>
...
<table class="table_div_content">
	<tr>
		...
		<td><input name="build_name" type="text" class="text_field_reference"
			validate="isSearch" readOnly="true"
			rubber_id="build_Id"><img
			src="<venus:base/>/images/icon/reference.gif"
			onClick="javascript:getBuildList();" class="img_1"> <input
			type="hidden" name="build_Id" id="rubber_id" value=""></td>
	</tr>
	<tr>
		<td align="right" width="10%" nowrap><fmt:message key="template.room_template"/></td>
		<td><input name="name" type="text" class="text_field" 
			validate="isSearch"><input type="button" name="Submit"
...
<div id="ccChild1"><%
	out.print(WebReportView.generateReport(request,response));
 %></div>
</form>
<script language="javascript">
