...
<script>
	...
	function query_data(code,conditions){
	   ...
	   if(jQuery("#dispStyle").val()){
		...
		JmmdaService.query(code,conditions,isCheckable,dispStyleValue ,pageVo.getOrderSortStr(),pageVo,function(data){getMapResult(data,code)});
	}
	...
	function toModify_onClick() { 
		...
		if(ids == -1)return;
		form.action="<%=request.getContextPath()%>/JmmdaAction.do?cmd=find4update&id=" + ids+"&moduleName="+moduleName+"&detailView="+detailView+"&tableView="+tableView+"&queryView="+queryView+"&toolView="+toolView;
		form.submit();
	}
	function toDelete_onClick(){  
	    ...
	    form.action="<%=request.getContextPath()%>/JmmdaAction.do?cmd=delete&ids=" + ids+"&moduleName="+moduleName+"&detailView="+detailView+"&tableView="+tableView+"&queryView="+queryView+"&toolView="+toolView;
	    form.submit();
	}

	function toAdd_onClick() {
		window.location="<venus:base/>/jsp/dd/scenario/jmerp/mda/edit.jsp?moduleName="+moduleName+"&detailView="+detailView+"&tableView="+tableView+"&queryView="+queryView+"&toolView="+toolView;
	}
	...
	function toZoom_onClick(){
		...
		if(ids == -1)return;
    	form.action="<%=request.getContextPath()%>/JmmdaAction.do?cmd=find4zoom&id=" + ids+"&moduleName="+moduleName+"&detailView="+detailView+"&tableView="+tableView+"&queryView="+queryView+"&toolView="+toolView;
    	form.submit();
	}
</script>
</head>
