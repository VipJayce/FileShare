...
<!--菜单树开始-->   
		<script language="javascript">
			...
			var checkboxType = "single";
			var selTxtArray = new Array("<fmt:message key='gap.authority.Read_only' bundle='${applicationAuResources}' />","<fmt:message key='gap.authority.Can_write' bundle='${applicationAuResources}' />","<fmt:message key='gap.authority.May_authorize' bundle='${applicationAuResources}' />");
			var selValArray = new Array('3','5','2');
		</script>
		<script language="javascript" src="<venus:base/>/js/au/tree4au.js"></script>
		<script language="javascript">
			if (document.getElementById){
				//根节点
				var t0=new WebFXTree('<b><fmt:message key='gap.authority.Field_Resources' bundle='${applicationAuResources}' /></b>','',selTxtArray,selValArray);
...
%>
						var node<%=vo.getTable_name()%> = new WebFXTreeItem("<%=vo.getTable_chinesename()%>","<%="<fmt:message key='gap.authority.Table_name' bundle='${applicationAuResources}' />:"+vo.getTable_name()+";<fmt:message key='gap.authority.Table_Chinese_name' bundle='${applicationAuResources}' />:"+vo.getTable_chinesename()%>","<%=vo.getTable_name()%>","",new Array(0),new Array(0),"","","");
...
%>
				...
				var node<%=vo.getId()%> = new WebFXTreeItem("<%=vo.getName()%><%=resSet.contains(vo.getId())?"[<fmt:message key='gap.authority.Open' bundle='${applicationAuResources}' />]":""%>",
				"<%="<fmt:message key='gap.authority.Field_name' bundle='${applicationAuResources}' />:"+vo.getField_name()+";<fmt:message key='gap.authority.Chinese_name_field' bundle='${applicationAuResources}' />:"+vo.getField_chinesename()+";<fmt:message key='gap.authority.Notes' bundle='${applicationAuResources}' />:"+vo.getHelp()%>",
...
<!--菜单树结束-->
			...
			<input type="hidden" name="auType" value="<%=GlobalConstants.getResType_fild()%>">
			</td> 
		</tr> 
    </table>
<script language="javascript">
	function saveOnClick() {
		...
		for(var i=0; i<inputArray.length; i++) {
			...
			if(selCbx.length>0) {
				var selStatus = "2";//授权状态
				var selAccess = 1; 	//读取方式（用素数表示，例如:可授权2，只读3，可写5...）
				for(var k=0; k<selCbx.length; k++) {
					if(selCbx[k].value=="0" || selCbx[k].value=="1") {
						selStatus = selCbx[k].value;
					}else {
						selAccess = selAccess * selCbx[k].value;
					}
				}
				if(selStatus != selAccess) {//除掉可授权，统统为允许
					selStatus = "1";
				}
				status += selStatus + ",";
				access += selAccess + ",";
				ids += selCbx[0].name + ",";
				codes += selCbx[0].code + ",";
				types += "<%=GlobalConstants.getResType_fild()%>" + ",";
			}
		}
