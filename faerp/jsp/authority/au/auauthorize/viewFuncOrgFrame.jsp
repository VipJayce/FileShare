...
<table class="table_noFrame" width="100%">
  <tr> 
     <td valign="top" width="100%"> 
     	<!--树开始--> 
		<iframe id="myTree" name="myTree" width="100%" height="600" src="<%=request.getContextPath()%>/AuAppendAction.do?cmd=<%=request.getParameter("cmd")%>&vCode=<%=request.getParameter("vCode")%>&partyId=<%=request.getParameter("partyId")%>&isUser=<%=request.getParameter("isUser")%>"></iframe>
		<!--树结束-->
    </td>
  </tr>
</table>
...
<table class="table_noFrame" width="100%">
  <tr> 
     <td valign="top" width="100%"> 
     	<input type="hidden" name="orgTreeSrc" id="orgTreeSrc" value="<%=request.getContextPath()%>/jsp/authority/tree/deeptree4AuOrg.jsp?inputType=checkbox&nodeRelationType=noRelation&vCode=<%=request.getParameter("vCode")%>	&rootXmlSource=<%=gap.authority.util.StringHelperTools.encodeUrl(rootXmlSource)%>">
     	<!--树开始--> 
