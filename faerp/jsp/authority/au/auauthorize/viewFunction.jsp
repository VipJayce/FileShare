...
%>
				...
				var node<%=vo.getTotal_code()%> = new WebFXTreeItem("<%=vo.getName()%><%=resSet.contains(vo.getId())?"[<fmt:message key='gap.authority.Open' bundle='${applicationAuResources}' />]":""%>","<%=vo.getName()%>","<%=vo.getId()%>","<%=vo.getTotal_code()%>",selTxtArray,selValArray,new Array(0),new Array('<%=selfType%>','<%=selfAccs%>'),"<%=isDisable%>");
<%
			}else {
%>
