...
<script>    
	function simpleQuery_onClick(){  //简单的模糊查询
    	form.action = "<venus:base/>/AuResourceAction.do?cmd=queryAllfield";
    	form.submit();
  	}
	function findCheckbox_onClick() {  //从多选框到修改页面
		...
		form.action = "<venus:base/>/AuResourceAction.do?cmd=initupdatefield&id=" + id;
		form.submit();
	}
	
	function toAdd_onClick() {  //到增加记录页面
		window.location="<venus:base/>/AuResourceAction.do?cmd=initinsert&resource_type=3&forward=<%=IAuResourceConstants.REQUEST_QUERY_ALL_ATBLE%>";
	}
	...
	function enable_onClick() {  //从多选框到修改页面
		...
		form.action="<venus:base/>/AuResourceAction.do?cmd=enablestatus&id=" + ids;
		form.submit();
	}   
	function disable_onClick() {  //从多选框到修改页面
		...
		form.action="<venus:base/>/AuResourceAction.do?cmd=disablestatus&id=" + ids;
		form.submit();
	}  
...
<table class="table_div_content">
	<tr>
		...
		<layout:collection name="beans" id="wy1" styleClass="listCss" width="100%" indexId="orderNumber" align="center" sortAction="0" >
			...
			<layout:collectionItem  title='<%=venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Chinese_name_field") %>' property="field_chinesename" />
			...
			<layout:collectionItem width="60" title='<%=venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Whether_the_public") %>' sortable="false" style="text-align:center;">
			<bean:define id="is_public" name="wy1" property="is_public"/>
		    <%="1".equals(is_public)?venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Be"):venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.No0")%>
		    </layout:collectionItem>
