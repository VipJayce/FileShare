...
<%@ page import="gap.authority.au.auauthorizelog.util.IConstants"%>
...
try {
    ...
    if(null!=allMap&&allMap.size()>0){
	    ...
	    Set allKeySplitSet = new java.util.HashSet();
	    ...
	    String strsql = "select * from au_partyrelation where parent_code like '"+parent_code+"%' and (1=2 ";
		...
		for(Iterator it = lParty.iterator(); it.hasNext(); ) {
			...
			DeepTreeVo dtv = new DeepTreeVo(code, name, "1".equals(isLeaf) ? "0" : "1", 
				request.getContextPath()+"/jsp/authority/au/proxyhistory/viewOrg.jsp?parent_code="+code+"&vCode="+vCode+"&auHisTag="+auHisTag+"&data_limit="+data_limit);
			//勾中已授权的节点
        ...
        DeepTreeVo dtv = new DeepTreeVo("", venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Had_not_been_granted_permission_"), "0" , "");
        dtv.setIsSubmit("0");
        dt.addTreeNode(dtv);
    }
