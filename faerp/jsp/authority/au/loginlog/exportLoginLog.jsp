...
<%
	...
	try {
        ...
        WritableSheet wsheet = wbook.createSheet(venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.First_Page"), 0); //sheet名称
        WritableCellFormat cellFormatNumber = new WritableCellFormat();
        cellFormatNumber.setAlignment(Alignment.RIGHT);
        
		int rowIndex = 0;
		int columnIndex = 0;
		wsheet.addCell(new Label(columnIndex++, rowIndex, venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Sequence")));
		
		wsheet.addCell(new Label(columnIndex++, rowIndex, venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Primary_key")));
		
		wsheet.addCell(new Label(columnIndex++, rowIndex, venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Login_account")));
		
		wsheet.addCell(new Label(columnIndex++, rowIndex, venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.User_Name")));
		
		wsheet.addCell(new Label(columnIndex++, rowIndex, "IP<fmt:message key='gap.authority.Address' bundle='${applicationAuResources}' />"));
		
		wsheet.addCell(new Label(columnIndex++, rowIndex, "IE<fmt:message key='gap.authority.Version' bundle='${applicationAuResources}' />"));
		
		wsheet.addCell(new Label(columnIndex++, rowIndex, venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Operating_system")));
		
		wsheet.addCell(new Label(columnIndex++, rowIndex, venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Host_name")));
		
		//wsheet.addCell(new Label(columnIndex++, rowIndex, "退出类型"));
		
		wsheet.addCell(new Label(columnIndex++, rowIndex, venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Login_time")));
		
		wsheet.addCell(new Label(columnIndex++, rowIndex, venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Exit_time")));
