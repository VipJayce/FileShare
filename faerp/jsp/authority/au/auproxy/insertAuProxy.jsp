...
<script language="javascript">
	function insert_onClick(){  //插入单条数据
    	form.action="<%=request.getContextPath()%>/AuProxyAction.do?cmd=insert";
  	...
  	function update_onClick(id){  //保存修改后的单条数据
	    form.action="<%=request.getContextPath()%>/AuProxyAction.do?cmd=update";
	    var parentRelId = form.parentRelId.value;
    	if(parentRelId!="")
    		form.target="_parent";
    	form.submit();
...
<table class="table_div_control">
	<tr> 
		<td>
			<img src="<%=request.getContextPath()%>/images/icon/07-0.gif" class="div_control_image" onClick="javascript:hideshow('ccChild1',this,'<%=request.getContextPath()%>/')"><%=isModify?venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Modify"):venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Added")%><fmt:message key='gap.authority.Proxy' bundle='${applicationAuResources}' />
		</td>
	</tr>
</table>
</div>
	...
	<tr>
		<td> 
			<table class="table_div_content_inner">
				<tr>
					<td width="30%" align="right"><span class="style_required_red">* </span><fmt:message key='gap.authority.Agent_Name' bundle='${applicationAuResources}' /></td>
					<td width="70%" align="left">
						<input type="text" class="text_field" name="name" inputName="<fmt:message key='gap.authority.Agent_Name' bundle='${applicationAuResources}' />" validate="notNull;isSearch" value="" maxLength="300" />
						&nbsp;&nbsp;<fmt:message key='gap.authority.To_facilitate_the_maintenance_use_with_agents_on_the_name_' bundle='${applicationAuResources}' />
					</td>
				</tr>
				<tr>
