...
<%@ page import="gap.authority.helper.AuHelper"%>
<%
try {
	//标识是做导航还是做选择
	...
	for(Iterator it = lParty.iterator(); it.hasNext(); ) {
		...
		if("1".equals(has_checkbox) || "yes".equals(has_checkbox)) {
			...
			if(proxyMap.containsKey(code)) {
			    dtv.setText(dtv.getText() + "(<fmt:message key='gap.authority.Has_been_associated' bundle='${applicationAuResources}' />)"); //已关联的代理
				dtv.setIsSubmit("0");
			}
		}else {
			//设置导航url
			dtv.setHrefPath(request.getContextPath()+"/AuProxyAction.do?cmd=detail&id="+partyId+"&relationId="+id);
