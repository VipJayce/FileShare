...
<title><fmt:message key='gap.authority.Function_menu_sort' bundle='${applicationAuResources}' /></title>
...
function sort_onClick() {
	...
	for (var i=box.options.length-1; i>=0; i--) {
   	...
   	document.form1.action= "<%=request.getContextPath()%>/AuFunctreeAction.do?cmd=sort";  
...
<table class="table_div_control">
	<tr> 
		<td>
			<img src="<%=request.getContextPath()%>/images/icon/07-0.gif" class="div_control_image" onClick="javascript:hideshow('auDivChild1',this,'<%=request.getContextPath()%>/')"><%=venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Function_menu")%><fmt:message key='gap.authority.Sort' bundle='${applicationAuResources}' />
		</td>
	</tr>
</table>
</div>
				...
				<table class="table_div_content_inner">
					...
					<tr>
						...
						<td align="left" colspan="2">
							<input type="text" class="text_field_readonly" name="parent_name" inputname="<fmt:message key='gap.authority.Parent_organization' bundle='${applicationAuResources}' />" value="<%=request.getParameter("parent_name")%>" maxlength="30" />
							<input type="hidden" name="parent_code" value="<%=parent_code%>"/>
						</td>
					</tr>
					<tr> 
						<td align="right"><fmt:message key='gap.authority.Node_list' bundle='${applicationAuResources}' /></td>
						<td align="left">			
