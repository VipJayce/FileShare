	...
	for(Iterator it = lParty.iterator(); it.hasNext(); ) {
		...
		if("1".equals(has_checkbox) || "yes".equals(has_checkbox)) {
			...
			if(roleMap.containsKey(code)) {
			    //dtv.setText(dtv.getText() + "(<fmt:message key='gap.authority.Has_been_associated' bundle='${applicationAuResources}' />)"); //已关联的角色
				//dtv.setIsSubmit("0");
			    dtv.setIsSelected("1");//20100528人寿修改
			}
		}else {
			//设置导航url
			dtv.setHrefPath(request.getContextPath()+"/AuRoleAction.do?cmd=detailWithPrivilage&id="+partyId+"&relationId="+id);
