...
try {
	...
	if(dataLimit!=null && dataLimit.length>0) {//有数据权限
		...
		for(Iterator it = lParty.iterator(); it.hasNext(); ) {
			...
			String email = (String)(m_Party.get("email") != null ? m_Party.get("email") : venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.No"));
			...
			if(userCodeMap.containsKey(code)) {
			    dtv.setText(dtv.getText() + "("+venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Allocated")+")");
				dtv.setIsDisable("1");
			}			
			...
			if((GlobalConstants.getPartyType_empl()).equals(partyTypeId)) { //只有人员节点才显示详细信息
			  /*String sex = "男";
				...
				dtv.setTitle(venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Number")+":" + partyId + "\n"+venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Name0")+":" + name +  "\n"+venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Mailbox")+":" + email);
			} 
		...
		DeepTreeVo dtv = new DeepTreeVo("", venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Sorry_you_do_not_have_permission_to_view_this_data"), "0" , "");
		dtv.setIsSubmit("0");
		dt.addTreeNode(dtv);
	}
    String xmlStr = dt.getStringFromDocument();
    if (xmlStr.indexOf("TreeNode") == -1) { //没有组织机构数据时的提示
        xmlStr = "<Trees><TreeNode id=''  text= '" + venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Can_not_find_the_data_") + "' hasChild= '0' isSubmit='0' /></Trees>";
    }    
    out.print(xmlStr);
} catch(Exception e) {
	e.printStackTrace();
