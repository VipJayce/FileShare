	...
	function saveOnClick() {  //获得所有选择的checkbox和取消的checkbox
		...
		if(inputType == "checkbox") {
			...
			if(submitType == "parentPriority") {
				submitStringArray = clearChild(checkedArray);  //全面扫描checkedArray,把其中冗余的子节点信息去掉,Id列表放入submitStringArray
			} else {
    	...
    	form.action="<venus:base/>/AuAppendAction.do";
		form.submit();
	}
...
<input type="hidden" name="authorizeId" value="<%=authorizeId%>">

<input type="hidden" name="cmd" value="saveFunOrgAuByRelId">
<input type="hidden" name="add_codes" value="">
<input type="hidden" name="del_codes" value="">
<input type="hidden" name="names" value="">
<input type="hidden" name="types" value="">
<input type="hidden" name="closeWindow" value="<%=closeWindow%>">
