...
/**********************　开始表单提交相关方法 **********************/
	...
	function changeRelation(srcrelationid) {  //获得所有选择的checkbox
		...
		document.form.action = "<%=request.getContextPath()%>/AdjustOrganizeAction.do?cmd=updateRelation&srcrelationid="+srcrelationid+"&newNodeCode="+newNodeCode;
	...
	function transactionMessage() { //返回事务操作的类型
  		...
  		if (adjustStatus == "1") {
  			alert("<fmt:message key='gap.authority.Transfer_level_success' bundle='${applicationAuResources}' />!");
  			window.parent.window.returnValue = true; //父页面是否刷新,true为刷新
  			parent.close();
  		} 
  		if (adjustStatus == "0") {
  			var failCause = "<%=failCause%>";
  			alert("<fmt:message key='gap.authority.Transfer_level_failure' bundle='${applicationAuResources}' />!<fmt:message key='gap.authority.The_reason_is' bundle='${applicationAuResources}' />:" + failCause);
  			window.parent.window.returnValue = false; //父页面是否刷新,false为不刷新
  			parent.close();  		
  		}
  		if (adjustStatus == "-1") {
  			alert("<fmt:message key='gap.authority.Transfer_level_failure' bundle='${applicationAuResources}' />!");
  			window.parent.window.returnValue = false; //父页面是否刷新,false为不刷新
  			parent.close();  		
  		}  		
  	}
