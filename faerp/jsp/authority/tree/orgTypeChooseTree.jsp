...
try {
    ...
    if(typeLevel == null || typeLevel.length()==0) {
        typeLevel = "0";
    }
		...
		*/
		for(Iterator it = lParty.iterator(); it.hasNext(); ) {
			...
			DeepTreeVo dtv = new DeepTreeVo(code, name, "1".equals(isLeaf) ? "0" : "1", 
				request.getContextPath()+"/jsp/authority/tree/orgTypeChooseTree.jsp?parent_code="+code
				+"&submit_all="+submit_all+"&return_type="+return_type+"&tree_level="+tree_level+"&data_limit="+data_limit+"&typeLevel="+typeLevel);
			...
			//判断可提交
            if("0".equals(typeLevel)) {
                dtv.setIsSubmit("1");
            }else{
                if(partyTypeComp.equals(partyTypeId)&&typeLevel.contains("company")){
                    dtv.setIsSubmit("1");//对只显示公司的树，仅叶子节点可以提交
                }else if(partyTypeDept.equals(partyTypeId)&&typeLevel.contains("department")) {
                    dtv.setIsSubmit("1");
                }else if(partyTypePosi.equals(partyTypeId)&&typeLevel.contains("position")) {
                    dtv.setIsSubmit("1");
                }else if(partyTypeEmpl.equals(partyTypeId)&&typeLevel.contains("employee")) {
                    dtv.setIsSubmit("1");
                }else{
                    dtv.setIsSubmit("0");
                }
