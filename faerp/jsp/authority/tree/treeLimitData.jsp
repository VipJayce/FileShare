...
<%@ page import="gap.authority.util.transform.json.JsonDataTools"%>
...
<%@ page import="gap.authority.service.history.bs.IHistoryLogBs"%>
...
try {
    ...
    if(!"".equals(currentRelid)&&null==currentCode){
        HistoryLogVo logVo = (HistoryLogVo)((IHistoryLogBs) Helper.getBean(gap.authority.service.history.util.IContants.BS_KEY)).queryByCondition("a.source_id='"+currentRelid+"' and a.operate_type='"+GlobalConstants.HISTORY_LOG_DELETE+"'"," a.operate_date desc").get(0);
    ...
    if(dataLimit!=null && dataLimit.length>0) {//有数据权限
        ...
        if((parent_code.length()>19&&!"true".equals(recall))||"true".equals(recall)){
