	...
	if(dataLimit!=null && dataLimit.length>0) {//有数据权限
		...
		for(Iterator it = lParty.iterator(); it.hasNext(); ) {
			...
			DeepTreeVo dtv = new DeepTreeVo(code, name, "1".equals(isLeaf) ? "0" : "1", 
				request.getContextPath()+"/jsp/authority/tree/orgQueryTree.jsp?parent_code="+code
				+"&submit_all="+submit_all+"&return_type="+return_type+"&tree_level="+tree_level+"&data_limit="+data_limit+"&query_organize_name="+query_organize_name);
			...
			if("1".equals(typeIsLeaf)&&"1".equals(isLeaf)&&(query_organize_name != null && query_organize_name.length()!=0)&&(name.indexOf(query_organize_name)==-1)){
				continue;//与查询的名称不匹配，跳过该节点的构造
			}else if((query_organize_name != null && query_organize_name.length()!=0)&&(name.indexOf(query_organize_name)==-1)&&(!DeepTreeSearch.searchAllSubWhetherLikeName(code,query_organize_name))){
				continue;//下级节点不符合查询条件，跳过该节点构造
			}
			dt.addTreeNode(dtv);
		} 
	}else {//没有数据权限
