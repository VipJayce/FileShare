...
<%
try {
	//参数：根节点编码，如果为空则为全部组织的根节点
	String root_code = request.getParameter("root_code");
	if(root_code == null || root_code.length()==0) {
		root_code = GlobalConstants.getRelaType_comp()+"00001";
	}
	...
	if("0".equals(data_limit) || "no".equals(data_limit) || "false".equals(data_limit)) {
		...
		dataLimit[0] = root_code;//如果不控制数据权限，则默认有全部权限
	}else {
	...
	if(dataLimit!=null && dataLimit.length>0) {//有数据权限
		//从数据库取数据
		String strsql = "select * from au_partyrelation where code='"+root_code+"' ";
