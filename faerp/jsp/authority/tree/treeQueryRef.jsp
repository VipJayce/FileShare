...
<script language="javascript">
	...
	function query_onClick(){
		//document.frames["myTree"].location.href = "deeptree.jsp?inputType=<%=request.getParameter("inputType")%>&submitType=<%=request.getParameter("submitType")==null?"submitAll":request.getParameter("submitType")%>&nodeRelationType=<%=request.getParameter("nodeRelationType")==null?"hasRelation":request.getParameter("nodeRelationType")%>&rootXmlSource=<%=gap.authority.util.StringHelperTools.encodeUrl(rootXmlSource)%>"+"%26query_organize_name%3D"+document.all.organize_name.value;
		var httpPost = "deeptree.jsp?inputType=<%=request.getParameter("inputType")%>&submitType=<%=request.getParameter("submitType")==null?"submitAll":request.getParameter("submitType")%>&nodeRelationType=<%=request.getParameter("nodeRelationType")==null?"hasRelation":request.getParameter("nodeRelationType")%>&rootXmlSource=<%=gap.authority.util.StringHelperTools.encodeUrl(rootXmlSource)%>";
		post(httpPost, {
...
<table class="table_noFrame">
	<tr>
		...
		<tr>
			<td>&nbsp;&nbsp;<fmt:message key='gap.authority.Name' bundle='${applicationAuResources}' />
			<input type="text" class="text_field" validate="isSearch" name="organize_name" inputName="<fmt:message key='gap.authority.Name' bundle='${applicationAuResources}' />" maxLength="50"/>
			<input name="button_ok" class="button_ellipse" type="button" value="<fmt:message key='gap.authority.Inquiry' bundle='${applicationAuResources}' />" onClick="javascript:query_onClick()"></td>
		</tr>
	</tr>
</table>
<table class="table_noFrame">
  <tr> 
     <td width="100%" valign="top"> 
		...
		<iframe name="myTree" width="100%" height="450" src="deeptree.jsp
			...
			&rootXmlSource=<%=gap.authority.util.StringHelperTools.encodeUrl(rootXmlSource)%>">
		</iframe>
		<!--树结束-->
    </td>
  </tr>
