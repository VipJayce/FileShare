...
<script>
	...
	function findCheckbox_onClick() {  //从多选框到修改页面
		...
		form.action = "<venus:base/>/SysParamsAction.do?cmd=find&id=" + ids;
		form.submit();
	}
	function deleteMulti_onClick(){  //从多选框物理删除多条记录
		...
		if(confirm("<fmt:message key='gap.authority.It_completely_remove_the_data_' bundle='${applicationAuResources}' />")) {
	    	form.action="<venus:base/>/SysParamsAction.do?cmd=delete&id=" + ids;
    		form.submit();
		}
	}
	...
	function enable_onClick() {  //从多选框到修改页面
		...
		form.action="<%=request.getContextPath()%>/SysParamsAction.do?cmd=enable&enable=1&id=" + ids;
		form.submit();
		alert("<fmt:message key='gap.authority.Change_the_system_configuration_items_in_the_system_log_back_on_after_the_entry_into_force_of_organizational_competence_' bundle='${applicationAuResources}' />");
	}   
	function disable_onClick() {  //从多选框到修改页面
		...
		form.action="<%=request.getContextPath()%>/SysParamsAction.do?cmd=enable&enable=0&id=" + ids;
		form.submit();
		alert("<fmt:message key='gap.authority.After_changing_the_configuration_items_need_to_log_in_to_take_effect_' bundle='${applicationAuResources}' />");
	}  
...
<script language="javascript">
	writeTableTop("<fmt:message key='gap.authority.The_system_configuration_items' bundle='${applicationAuResources}' />",'<venus:base/>/');
</script>

<form name="form" method="post" action="<venus:base/>/SysParamsAction.do">
...
<table class="table_div_control">
	<tr > 
		...
		<td> 
			<table align="right">
				<tr> 
					<td id="addBtn" class="button_ellipse" onClick="javascript:toAdd_onClick();"><img src="<venus:base/>/images/icon/add.gif" class="div_control_image"><fmt:message key='gap.authority.Added' bundle='${applicationAuResources}' /></td>
					<td id="deleteBtn" class="button_ellipse" onClick="javascript:deleteMulti_onClick();"><img src="<venus:base/>/images/icon/delete.gif" class="div_control_image"><fmt:message key='gap.authority.Delete' bundle='${applicationAuResources}' /></td>
					<td id="updateBtn" class="button_ellipse" onClick="javascript:findCheckbox_onClick();"><img src="<venus:base/>/images/icon/modify.gif" class="div_control_image"><fmt:message key='gap.authority.Modify' bundle='${applicationAuResources}' /></td>
...
<table class="table_div_content">
	<tr>
		<td>
		<layout:collection name="beans" id="wy1" styleClass="listCss" width="100%" indexId="orderNumber" align="center" sortAction="0" onRowClick="javascript:buttonControl_onClick(getRowHiddenIdOnClick());">
			...
			<layout:collectionItem width="200"  title='<%=venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Parameter_name") %>' property="propertykey" />
			<layout:collectionItem width="50"  title='<%=venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Parameter_values") %>' property="value" />
			<layout:collectionItem width="80"  title='<%=venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Parameter_type") %>'>
				<bean:define id="propertytype" name="wy1" property="propertytype"/>
				<%=paramTypeMap.getLabel(propertytype.toString()) %>
			</layout:collectionItem>
			<layout:collectionItem width="180" title='<%=venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Set_the_time") %>' property="updateTime">
				<bean:define id="updateTime" name="wy1" property="updateTime"/>
				<%=StringHelperTools.prt(updateTime, 19)%>
			</layout:collectionItem>
			<layout:collectionItem width="100" title='<%=venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Set_of_people") %>' property="creatorName" />
			<layout:collectionItem  title='<%=venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Description") %>' property="description" />
			<layout:collectionItem width="60" title='<%=venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Status") %>' style="text-align:center;">
				<bean:define id="enable" name="wy1" property="enable"/>
