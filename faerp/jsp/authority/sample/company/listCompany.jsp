...
<title><fmt:message key='gap.authority.Company_Management' bundle='${applicationAuResources}' /></title>
<script language="javascript">
	...
	function findCheckbox_onClick() {  //从多选框到修改页面
		...
		form.action="<%=request.getContextPath()%>/CompanyAction.do?cmd=find&id=" + ids;
		form.submit();
	}  
	function deleteMulti_onClick(){  //从多选框物理删除多条记录
		...
		if(confirm("<fmt:message key='gap.authority.It_completely_remove_the_data_' bundle='${applicationAuResources}' />")) {  //如果用户在确认对话框按"确定"
	    	form.action="<%=request.getContextPath()%>/CompanyAction.do?cmd=deleteMulti&ids=" + ids;
	    	form.submit();
		}
	}
	function simpleQuery_onClick(){  //简单的模糊查询
		form.action="<%=request.getContextPath()%>/CompanyAction.do?cmd=simpleQuery";
    	form.submit();
  	}
	function queryAll_onClick(){  //查询全部数据列表
		form.action="<%=request.getContextPath()%>/CompanyAction.do?cmd=queryAll";
		form.submit();
	}	
	...
	function detail_onClick(thisId){  //实现转到详细页面
		...
		form.action="<%=request.getContextPath()%>/CompanyAction.do?cmd=detail&isReadOnly=1";
		form.submit();
	}
...
<div id="right">
<script language="javascript">
	writeTableTopFesco('公司档案管理','<%=request.getContextPath()%>/');  //显示本页的页眉
</script>
<div class="ringht_s">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
			...
			<td/>
			  <td>    </td>
				...
				<tr>
				<td width="15%" align="right"><fmt:message key='gap.authority.Company_Number' bundle='${applicationAuResources}' /></td>
					<td width="25%" align="left">
						<input type="text" class="text_field" validate="isSearch" name="company_no" inputName="<fmt:message key='gap.authority.Company_Number' bundle='${applicationAuResources}' />" maxLength="50"/>
					</td>
					<td width="15%" align="right"><fmt:message key='gap.authority.Type' bundle='${applicationAuResources}' /></td>
					<td width="25%" align="left"><input type="text" class="text_field" validate="isSearch" name="company_type" inputName="<fmt:message key='gap.authority.Type' bundle='${applicationAuResources}' />" maxLength="50"/></td>
			
			<td align="right"></td>
		</tr>
		<tr>
				<td align="right"><fmt:message key='gap.authority.Company_Name' bundle='${applicationAuResources}' /></td>
					<td align="left">
						<input type="text" class="text_field" validate="isSearch" name="company_name" inputName="<fmt:message key='gap.authority.Company_Name' bundle='${applicationAuResources}' />" maxLength="300"/>
					</td>
					<td align="right"><fmt:message key='gap.authority.Company_referred_to' bundle='${applicationAuResources}' /></td>
					<td align="left">
						<input type="text" class="text_field" validate="isSearch" name="short_name" inputName="<fmt:message key='gap.authority.Company_referred_to' bundle='${applicationAuResources}' />" maxLength="50"/>
					</td>
			<td align="right"></td>
		</tr>
		<tr>
		<td align="right"><fmt:message key='gap.authority.Your_area' bundle='${applicationAuResources}' /></td>
					<td align="left">
						<input type="text" class="text_field" validate="isSearch" name="area" inputName="<fmt:message key='gap.authority.Your_area' bundle='${applicationAuResources}' />" maxLength="300"/>
					</td>
					<td align="right"><fmt:message key='gap.authority.Contact' bundle='${applicationAuResources}' /></td>
					<td align="left">
						<input type="text" class="text_field" validate="isSearch" name="linkman" inputName="<fmt:message key='gap.authority.Contact0' bundle='${applicationAuResources}' />" maxLength="50"/>
					</td>
			<td align="right"></td>
		</tr>
		<tr>
...
<td><bean:write name="rmBean" property="company_no"/>   </td>
			
			<td><bean:write name="rmBean" property="company_name"/>   </td>
			
			<td><bean:write name="rmBean" property="short_name"/>   </td>
			<td><bean:write name="rmBean" property="company_type"/>   </td>
			<td><bean:write name="rmBean" property="area"/>   </td>
			<td><bean:write name="rmBean" property="linkman"/>   </td>
