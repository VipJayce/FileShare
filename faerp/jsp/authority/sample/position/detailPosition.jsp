...
<script language="javascript">
	...
	function find_onClick(){  //直接点到修改页面
		form.action="<%=request.getContextPath()%>/PositionAction.do";
		form.cmd.value = "find";
		form.submit();
	}
	function delete_onClick(){  //直接点删除单条记录
		...
		if(!confirm("<fmt:message key='gap.authority.This_operation_will_completely_remove_the_posts' bundle='${applicationAuResources}' />\n"
	...
	<tr>
		<td>
			...
			%>
				...
				<input name="button_change" class="button_ellipse" type="button" value="<fmt:message key='gap.authority.Tone_level' bundle='${applicationAuResources}' />" onClick="javascript:getChangeTree(new Array(),'<venus:base/>','radio','','1','','2','1');">
			<%
			}
			%>
		</td>
...
<table class="table_div_control">
	<tr> 
		<td>
			<img src="<%=request.getContextPath()%>/images/icon/07-0.gif" class="div_control_image" onClick="javascript:hideshow('ccChild0',this,'<%=request.getContextPath()%>/')"><%=venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Post")%><fmt:message key='gap.authority.Details0' bundle='${applicationAuResources}' />
		</td>
	</tr>
</table>
</div>
	...
	<tr>
		<td>
			<table class="table_div_content_inner">
				<tr>
					<td width="30%" align="right"><fmt:message key='gap.authority.Job_Title_' bundle='${applicationAuResources}' /></td>
				...
				<tr>
					...
					<td align="left"><%="1".equals(resultVo.getLeader_flag())?venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Be"):venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.No0")%></td>
				</tr>
				<tr>
