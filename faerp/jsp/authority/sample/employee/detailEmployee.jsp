...
<title><fmt:message key='gap.authority.Staff_Details' bundle='${applicationAuResources}' /></title>
<script language="javascript">
	...
	function find_onClick(){  //直接点到修改页面
		form.action="<%=request.getContextPath()%>/EmployeeAction.do";
		form.cmd.value = "find";
		form.submit();
	}
	function delete_onClick(){  //直接点删除单条记录
		if(!confirm("<fmt:message key='gap.authority.This_action_will_remove_the_employee_organization_relationship_information' bundle='${applicationAuResources}' />\n"
		+ "<fmt:message key='gap.authority.If_the_staff_does_not_schedule_any_current_positions_then_the_employee_s_basic_information_will_be_removed_together' bundle='${applicationAuResources}' />\n"
		+ "<fmt:message key='gap.authority.If_the_employee_currently_only_arranged_for_a_post_then_the_employees_will_be_placed_in_their_respective_departments_below' bundle='${applicationAuResources}' />\n"
	...
	<tr>
		<td>
			...
			%>
				...
				<input name="button_change" class="button_ellipse" type="button" value="<fmt:message key='gap.authority.Tone_level' bundle='${applicationAuResources}' />" onClick="javascript:getChangeTree(new Array(),'<venus:base/>','radio','','1','','3','1');">
			<%
			}
			%>
		</td>
...
<table class="table_div_control">
	<tr> 
		<td>
			<img src="<%=request.getContextPath()%>/images/icon/07-0.gif" class="div_control_image" onClick="javascript:hideshow('ccChild0',this,'<%=request.getContextPath()%>/')"><%=venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Staff0")%><fmt:message key='gap.authority.Details0' bundle='${applicationAuResources}' />
		</td>
	</tr>
</table>
</div>
	...
	<tr>
		<td>
			<table class="table_div_content_inner">
				<tr>
					<td width="30%" align="right"><fmt:message key='gap.authority.Name_0' bundle='${applicationAuResources}' /></td>
				...
				<tr>
					...
					<td align="left"><%="1".equals(resultVo.getSex())?venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Male"):venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Female")%></td>
				</tr>
				<tr>
