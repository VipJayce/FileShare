...
<%
	...
	SysParamVo multiTab = GlobalConstants.getSysParam("MULTITAB"); 
%>
<?xml version="1.0" encoding="GBK"?>
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	<!--忽略xml声明-->
	<xsl:output method="html" encoding="GBK" omit-xml-declaration="yes"/>
	<!--处理根节点-->
	<xsl:template match="/">
		<xsl:apply-templates/>
	</xsl:template>
	...
	<xsl:template match="TreeNode">
		<xsl:for-each select=".">
			...
			<div class="TreeNode" nowrap="true" name="deeptree_div">
				...
				<!--隐藏属性的容器结束-->
				<!--树的图片开始-->
				<img type="img" align="absmiddle" use="tree" name="deeptree_treeImage">
						...
						<xsl:choose>
							<xsl:when test="@hasChild &gt; 0">
								<xsl:choose>
									<xsl:when test="TreeNode">
										<xsl:choose>
											<xsl:when test="@defaultOpen &gt; 0"><%=treeImagePath%>/ftv2folderopen.gif</xsl:when>
											<xsl:otherwise><%=treeImagePath%>/ftv2folderclosed.gif</xsl:otherwise>
										</xsl:choose>
									</xsl:when>
									<xsl:otherwise><%=treeImagePath%>/ftv2folderclosed.gif</xsl:otherwise>
								</xsl:choose>
							</xsl:when>
							<xsl:otherwise><%=treeImagePath%>/ftv2link.gif</xsl:otherwise>
						</xsl:choose>
					</xsl:attribute>
				</img>		
				<!--树的图片结束-->
				...
				<!--checkbox开始-->
				<input type="checkbox" name="deeptree_checkbox">
				...
				<input type="radio" name="deeptree_radio">
					...
					<xsl:if test="@isDisable = '1'">
						<xsl:attribute name="style">disabled</xsl:attribute>
					</xsl:if>
				</input>
				<!--radio结束-->
				<%	} %>
				...
				<xsl:if test="@logoImagePath != ''">
					<img type="img" align="absmiddle" use="logo" name="deeptree_logoImage">
					...
					<xsl:choose>
						<xsl:when test="@hrefPath != ''">
                            ...
                            <% if (multiTab != null && Integer.parseInt(multiTab.getValue()) > 0) { //启用多页签%>
                                ...
                                <xsl:attribute name="title"><xsl:value-of select="@hrefPath"/></xsl:attribute>
                            <%} else {%>
	...
	<xsl:template match="message">
		<div class="TreeNode" type="load" nowrap="true">
			<img type="img" src="image/file.gif" align="absmiddle"/>
	...
	<xsl:template match="MoreTreeNode">
		<div class="TreeNode" type="more" nowrap="true">
			<span type="more" class="more" style="cursor:hand;">
				<xsl:attribute name="xmlSource"><xsl:value-of select="@xmlSource"/></xsl:attribute>
				<xsl:value-of select="@text"/>
			</span>
		</div>
