...
<title><fmt:message key='gap.authority.Body_type_list' bundle='${applicationAuResources}' /></title>
</head>
<body>
						...
						<table style="behavior:url('<venus:base/>/js/au/gap-treeColor.htc')" width="100%" border="0" cellpadding="0" cellspacing="0">
							...
							<tr>
								...
								<script language="javascript">								
									<%
									String htmlCode = "folderTree0 = gFld('&nbsp;"+venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Body_type_list")+"','','diffFolder.gif','diffFolder-0.gif');\n";
								    ...
								    for(Iterator it = lTree.iterator(); it.hasNext(); ) {
							            ...
							            htmlCode += "folderTree" + vo.getId() + " = insDoc(folderTree" + vo.getKeyword() + ", gLnk('0','&nbsp;" + vo.getName() + "', '" + request.getContextPath() + "/AuPartyAction.do?cmd=queryAll&typeId="+vo.getId()+"', 'ftv2link.gif'));\n";
								    }
									if(!hasRoot){
										htmlCode += "folderTree1 = insFld(folderTree0, gFld('&nbsp;"+venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.There_is_no_alternative_node")+"', '', 'ftv2folderopen.gif', 'ftv2folderclosed.gif'));\n";
									}
									out.print(htmlCode);
									%>
									try {
