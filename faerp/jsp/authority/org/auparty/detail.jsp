...
<script language="javascript">
	...
	function toAdd_onClick(thisId) {//添加团体关系
		...
		if(rtObj != undefined && rtObj.length > 0){
			...
			form.action="<venus:base/>/AuPartyRelationAction.do?cmd=insert&returnPage=party_detail" 
	...
	function delete_onClick(thisId, type) {//揪除团体关系
		...
		if(confirm("<fmt:message key='gap.authority.It_completely_remove_the_data_' bundle='${applicationAuResources}' />")) {
			var partyId = document.form.partyId.value;
	    	form.action="<venus:base/>/AuPartyRelationAction.do?cmd=delete&returnPage=party_detail&id="+partyId+"&relId="+relId;
    		form.submit();
		}
	}
	function goBack_onClick(typeId) {//添加团体
	    window.location="<venus:base/>/AuPartyAction.do?cmd=queryAll&typeId="+typeId;
	}	
	function refresh_onClick() {//刷新
		var partyId = document.form.partyId.value;
		form.action = "<venus:base/>/AuPartyAction.do?cmd=detailPage&id=" + partyId;
	    form.submit();
	}
...
<table class="table_noframe" width="97%" align="center">
	...
	<table border="1" bordercolordark="#FFFFFF" bordercolorlight="#7EBAFF" cellpadding="5" cellspacing="0" width="96%" align="center"> 	
		...
		<tr> 
			<td width="30%" align="right"><fmt:message key='gap.authority.Type_1' bundle='${applicationAuResources}' /></td> 
...
<table class="table_div_control">
	<tr > 
		...
		<td> 
			<table align="right">
				<tr> 
					<td class="button_ellipse" onClick="javascript:toAdd_onClick();" title="<fmt:message key='gap.authority.Select_the_parent_group' bundle='${applicationAuResources}' />"><img src="<venus:base/>/images/icon/add.gif" class="div_control_image"><fmt:message key='gap.authority.New_Relationship' bundle='${applicationAuResources}' /></td>
					<td class="button_ellipse" onClick="javascript:delete_onClick();"><img src="<venus:base/>/images/icon/delete.gif" class="div_control_image"><fmt:message key='gap.authority.Remove_Relations' bundle='${applicationAuResources}' /></td>
...
<table class="table_div_content">
	<tr>
		<td>
		<layout:collection name="list" id="wy1" styleClass="listCss" width="100%" indexId="orderNumber" align="center" sortAction="0" >
			...
			<layout:collectionItem width="100" title='<%=venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Group_type_of_relationship") %>' property="relationtype_name" />
            <layout:collectionItem  title='<%=venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Parent_groups") %>' property="parent_partyname" />
