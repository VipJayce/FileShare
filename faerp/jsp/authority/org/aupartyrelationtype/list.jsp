...
<script>
	...
	function view_onClick(id){//查看详细页面
		form.action = "<venus:base/>//AuPartyRelationTypeAction.do?cmd=detailPage&id=" + id;
		form.submit();
	}
	...
	function findCheckbox_onClick() {  //从多选框到修改页面
		...
		form.action = "<venus:base/>/AuPartyRelationTypeAction.do?cmd=find&ids=" + ids;
		form.submit();
	}
	function detail_onClick(thisId){
		form.action = "<venus:base/>/AuPartyRelationTypeAction.do?cmd=find&ids=" + thisId;
		form.submit();
	}
	function deleteMulti_onClick(){  //从多选框物理删除多条记录
		...
		if(confirm("<fmt:message key='gap.authority.It_completely_remove_the_data_' bundle='${applicationAuResources}' />")) {
	    	form.action="<venus:base/>/AuPartyRelationTypeAction.do?cmd=delete&ids=" + ids;
    		form.submit();
		}
	}
	function toAdd_onClick() {  //到增加记录页面
		window.location="<venus:base/>/jsp/authority/org/aupartyrelationtype/insert.jsp";
	}
	...
	function enable_onClick() {  //从多选框到修改页面
		...
		form.action="<venus:base/>/AuPartyRelationTypeAction.do?cmd=enable&ids=" + ids;
		form.submit();
	}   
	function disable_onClick() {  //从多选框到修改页面
		...
		form.action="<venus:base/>/AuPartyRelationTypeAction.do?cmd=disable&ids=" + ids;
		form.submit();
	}  
	function initFocus(){ 
...
<form name="form" method="post" action="<venus:base/>/AuPartyRelationTypeAction.do">
...
<tr>
	...
	<td width="30%"><input name="name" type="text" class="text_field" inputName="<fmt:message key='gap.authority.Name' bundle='${applicationAuResources}' />" validate="isSearch"></td>
