...
<script>
	...
	function getRowHiddenRelaId() {  //从事件取得本行的id
		var thisA = event.srcElement;  //定义事件来源对象
		var thisTr = thisA.parentElement.parentElement.parentElement;  //定义本行的tr对象
		var thisHidden = getObjectByNameRecursive(thisTr, "hiddenRelaId");  //从thisTr递归的取出name是hiddenId的对象
		if(thisHidden != undefined && thisHidden != null) {  //如果thisHidden不为空
			return thisHidden.value;
		} else {
			return null;
...
<!--
	function delete_onClick(){  //删除记录
		...
		if(confirm("<fmt:message key='gap.authority.It_completely_remove_the_data_' bundle='${applicationAuResources}' />")) {
	    	form.action="<venus:base/>/AuPartyRelationAction.do?cmd=delete&relId="+ids+"&partyrelationtype_id="+staticRelationTypeId;
    		form.submit();
		}
	}
	function toAdd_onClick() {  //到增加记录页面
		...
		if(rtObj != undefined){
			...
			form.action="<venus:base/>/AuPartyRelationAction.do?cmd=insertMulti&ids=" + partyIds + "&parentRelId=" + ids + "&relTypeId="+staticRelationTypeId;
    		form.submit();
		}
	}
...
<table class="table_div_control">
	<tr > 
		...
		<td> 
			<table align="right">
				<tr> 
					<td class="button_ellipse" onClick="javascript:view_onButton();"><img src="<venus:base/>/images/icon/modify.gif" class="div_control_image"><fmt:message key='gap.authority.Tree_View' bundle='${applicationAuResources}' /></td>
				</tr>
			</table>
		</td>
	</tr>
...
<table class="table_div_content">
	<tr>
		...
		<layout:collection name="wy" id="wy1" styleClass="listCss" width="100%" indexId="orderNumber" align="center" sortAction="0" >
			...
			<%="<a onclick='javascript:view_onClick(getRowHiddenId(),getRowHiddenRelaId())'>"%>	
			<layout:collectionItem width="130" title='<%=venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Name") %>' property="name" />	
			<layout:collectionItem width="130" title='<%=venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Group_type_of_relationship") %>' property="relationtype_name" />	
...
<table class="table_div_control">
	<tr > 
		<td>
			<img src="<venus:base/>/images/icon/07-0.gif" class="div_control_image" onClick="javascript:hideshow('auDivChild2',this,'<venus:base/>/')">&nbsp;<fmt:message key='gap.authority.Group_Tree' bundle='${applicationAuResources}' />
		</td>
		<td> 
			<table align="right">
				<tr> 
