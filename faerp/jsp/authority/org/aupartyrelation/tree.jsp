...
<title><fmt:message key='gap.authority.Group_Tree' bundle='${applicationAuResources}' /></title>
<script>
<!--
	function delete_onClick(){  //删除记录
		...
		if(ids=="") {
	  		alert("<fmt:message key='gap.authority.Please_select_the_records_you_want_to_delete' bundle='${applicationAuResources}' />!")
	  		return;
		}
		if(typeId=="0") {
			alert("<fmt:message key='gap.authority.Not_a_leaf_node_the_node_is_not_allowed_to_delete_' bundle='${applicationAuResources}' />")
	  		return;
		}
		if(confirm("<fmt:message key='gap.authority.Configuration_information_will_be_synchronized_to_delete_permissions_whether_completely_remove_the_data_' bundle='${applicationAuResources}' />")) {
	    	form.action="<venus:base/>/AuPartyRelationAction.do?cmd=delete&relId="+ids+"&partyrelationtype_id=<%=parentCode%>";
    		form.submit();
		}
	}
	function toAdd_onClick() {  //到增加记录页面
		...
		var ids = myTree.codeStrList;
		if(ids=="") {
	  		alert("<fmt:message key='gap.authority.Please_choose_which_node_s_parent_node' bundle='${applicationAuResources}' />!")
	  		return;
		}
		...
		if(rtObj != undefined){	
			...
			form.action="<venus:base/>/AuPartyRelationAction.do?cmd=insertMulti&ids=" + partyIds + "&parentRelId=" + ids + "&relTypeId=<%=parentCode%>";
    		form.submit();
		}
	}
	function refresh_onClick(){  //刷新本页
		form.action="<venus:base/>/AuPartyRelationAction.do?cmd=hasRoot&partyrelationtype_id=<%=parentCode%>";
		form.submit();
	}
...
<table class="table_div_control">
	<tr > 
		<td>
			<img src="<venus:base/>/images/icon/07-0.gif" class="div_control_image" onClick="javascript:hideshow('auDivChild1',this,'<venus:base/>/')">&nbsp;<fmt:message key='gap.authority.Group_Tree' bundle='${applicationAuResources}' />
		</td>
		<td> 
			<table align="right">
				<tr> 
					...
					<td class="button_ellipse" onClick="javascript:delete_onClick();"><img src="<venus:base/>/images/icon/delete.gif" class="div_control_image"><fmt:message key='gap.authority.Delete' bundle='${applicationAuResources}' /></td>
					...
					<td class="button_ellipse" onClick="javascript:getChangeTree4All(new Array(),'<venus:base/>','radio','','1','','2','1');"><img src="<venus:base/>/images/icon/mix.gif" class="div_control_image"><fmt:message key='gap.authority.Tone_level' bundle='${applicationAuResources}' /></td>
					<%}%>
				</tr>
			</table>
		</td>
