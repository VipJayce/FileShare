...
<script language="javascript">
	...
	function showMessageBox(id) {
		...
		if (id=='' || id == null || optItem == '') {
			org_content.innerText = "<fmt:message key='gap.authority.Not_given_parameters' bundle='${applicationAuResources}' />";
			return;
		}
		...
		org_content.innerText = "<fmt:message key='gap.authority.Data_Loading' bundle='${applicationAuResources}' />......";
		var thisHidden = getLayoutHiddenObjectById(id);	
		jQuery.ajax({
			type: "GET",
			url: "<%=request.getContextPath()%>/AuPartyRelationAction.do?cmd=getOrgName",
			...
			success: function(data,textStatus) {
				if(data == '' || data == null || data == undefined || data == 'null')
					org_content.innerText = "<fmt:message key='gap.authority.Did_not_find_their_respective_organizations' bundle='${applicationAuResources}' />";
				else if (data.indexOf("<fmt:message key='gap.authority.You_are_logged_in_the_system' bundle='${applicationAuResources}' />") != -1)
					org_content.innerText = "<fmt:message key='gap.authority.You_are_logged_in_the_system' bundle='${applicationAuResources}' />,<fmt:message key='gap.authority.Or_login_has_timed_out' bundle='${applicationAuResources}' />,<fmt:message key='gap.authority.Please_login_system' bundle='${applicationAuResources}' />!";
				else 
	...
	document.write("<div style='display:inline; width:150px; position:absolute'><fmt:message key='gap.authority.Organization' bundle='${applicationAuResources}' /></div>");
	document.write('<span onclick=closeMessageBox() style="float:right; display:inline; cursor:hand">×</span>');
	document.write('</div>');
	document.write("<div id='org_content' style='overflow: auto'><fmt:message key='gap.authority.Data_Loading' bundle='${applicationAuResources}' />......</div>");
	document.write('</div>');
	document.write('</div>');
</script>
