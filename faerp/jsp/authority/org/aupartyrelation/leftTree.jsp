...
<title><fmt:message key='gap.authority.Function_menu' bundle='${applicationAuResources}' /></title>
</head>
<body>
						...
						<table style="behavior:url('<venus:base/>/js/au/gap-treeColor.htc')" width="100%" border="0" cellpadding="0" cellspacing="0">
							...
							<tr>
								...
								<script language="javascript">								
									<%
									String htmlCode = "folderTree0 = gFld('&nbsp;"+venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Relationship_as_type_list")+"','','diffFolder.gif','diffFolder-0.gif');\n";
								    htmlCode += "foldersTree = folderTree0\n";
								    htmlCode += "folderTree3 = insFld(folderTree0, gFld('&nbsp;"+venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Agency")+"', '', 'ftv2folderopen.gif', 'ftv2folderclosed.gif'));\n";
								    htmlCode += "folderTree2 = insFld(folderTree0, gFld('&nbsp;"+venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Role")+"', '', 'ftv2folderopen.gif', 'ftv2folderclosed.gif'));\n";
								    htmlCode += "folderTree4 = insFld(folderTree0, gFld('&nbsp;"+venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Other")+"', '', 'ftv2folderopen.gif', 'ftv2folderclosed.gif'));\n";
								    for(Iterator it = lTree.iterator(); it.hasNext(); ) {
								        AuPartyRelationTypeVo vo = (AuPartyRelationTypeVo) it.next();
							            htmlCode += "folderTree" + vo.getId() + " = insDoc(folderTree" + vo.getKeyword() + ", gLnk('0','&nbsp;" + vo.getName() + "', '" + request.getContextPath() + "/AuPartyRelationAction.do?cmd=hasRoot&partyrelationtype_id="+vo.getId()+"', 'ftv2link.gif'));\n";
								    }
									out.print(htmlCode);
									%>
									try {
