...
<script>
	function insert_onClick(){  //增加记录
		...
		if(checkSelectValue.value==''){
			alert("<fmt:message key='gap.authority.Please_select_the_type_of_parent_groups' bundle='${applicationAuResources}' />!");
			return;
		}
		checkSelectValue = document.getElementById("child_partytype_id");
		if(checkSelectValue.value==''){
			alert("<fmt:message key='gap.authority.Please_select_the_type_of_sub_groups' bundle='${applicationAuResources}' />!");
			return;
		}
		if(checkAllForms()){
  ...
  // indexName指选中项,selectName指select的ID
    ...
    var me=this;
    obj1.length=0;
    obj1.options[0]=new Option("<fmt:message key='gap.authority.Please_select' bundle='${applicationAuResources}' />",'');
...
<table class="table_noframe">
	<tr>
		<td valign="middle"><input name="button_save" type="button"
			class="button_ellipse" value="<fmt:message key='gap.authority.Save' bundle='${applicationAuResources}' />"
			onClick="javascript:<%=isModify?"update_onClick()":"insert_onClick()"%>;">
		<input name="button_cancel" type="button" class="button_ellipse"
			value="<fmt:message key='gap.authority.Cancel' bundle='${applicationAuResources}' />" onClick="javascript:returnBack();"></td>
	</tr>
</table>
...
<table class="table_div_control">
	<tr>
		<td><img src="<venus:base/>/images/icon/07-0.gif"
			...
			onClick="javascript:hideshow('auDivChild1',this,'<venus:base/>/')"><%=isModify?venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Modify_the_rules_of_group_connection"):venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.New_groups_joining_rules")%>
		</td>
	</tr>
</table>
</div>
...
<table class="table_div_content">
	<tr>
		...
		</span><fmt:message key='gap.authority.Name' bundle='${applicationAuResources}' /></td>
		<td align="left"><input name="Id" type="hidden" value=""> <input
			name="name" type="text" class="text_field" inputName="<fmt:message key='gap.authority.Name' bundle='${applicationAuResources}' />"
			validate="notNull;isSearch" maxlength=50 value=""></td>
	</tr>
	<%if("1".equals(linkage)){ %>
	<tr>
		<td align="right"><fmt:message key='gap.authority.Group_type_of_relationship' bundle='${applicationAuResources}' /></td>
		<td align="left"><select id='relation_type_id' name="relation_type_id">
			<OPTION selected></OPTION>
		</select></td>
	</tr>
	<tr>
		<td align="right"><fmt:message key='gap.authority.Types_of_parent_groups' bundle='${applicationAuResources}' /></td>
	...
	<tr>
		<td align="right"><fmt:message key='gap.authority.Sub_group_type' bundle='${applicationAuResources}' /></td>
  ...
  <%  	
  	int index=0;
  	List subPartyTypeList=(List)chilePartyMap.get(typeMap.getValue(venus.frames.i18n.util.LocaleHolder.getMessage("gap.authority.Staff")));
	...
	%>
			       		array[<%=index++%>]=new Array("<%=tmp.getLabel()%>","<fmt:message key='gap.authority.Root' bundle='${applicationAuResources}' />","<%=tmp.getValue()%>");
			       		<%
  ...
  liandong.firstSelectChange("<fmt:message key='gap.authority.Root' bundle='${applicationAuResources}' />","relation_type_id"); //设置第一个选择框
  ...
  liandong.subSelectChange("parent_partytype_id","child_partytype_id");
	...
	<%} %>
	<tr>
		<td align="right" width="10%" nowrap><fmt:message key='gap.authority.Notes' bundle='${applicationAuResources}' /></td>
