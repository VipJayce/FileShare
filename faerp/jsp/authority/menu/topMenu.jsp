...
<%
    ...
    if (!list.isEmpty()) {
     ...
     if(loginSessionVo==null){
		...
		%>
		<script language="javascript">
		   alert("<fmt:message key='gap.authority.This_relation_does_not_have_any_rights' bundle='${applicationAuResources}' />") 
		    // window.top.location.href="<%=request.getContextPath()%>/jsp/login/login.jsp";
		</script>
		<%
		 }
...
<%
    ...
    try {
        ...
        IAuPartyRelationTypeBS relBS = (IAuPartyRelationTypeBS) Helper.getBean(gap.authority.org.aupartyrelationtype.util.IConstants.BS_KEY);
        ...
        //关系处理
        IAuPartyRelationBs relBs = (IAuPartyRelationBs) Helper.getBean(gap.authority.org.aupartyrelation.util.IConstants.BS_KEY);
...
<table width="100%" height="30px" border="0" cellpadding="0" cellspacing="0"  bgcolor="#F5f5f5">
	<tr>
        ...
        <td align="right">&nbsp;&nbsp;&nbsp;<img src="<%=request.getContextPath() %>/images/au/ren.gif" width="16" height="16" align="absmiddle">&nbsp;&nbsp;<fmt:message key='gap.authority.welcome' bundle='${applicationAuResources}' />, <%=LoginHelper.getLoginName(request) %>&nbsp;&nbsp;&nbsp;&nbsp;</td>
        <td align="right">
        <%if(chooseAuRel!=null&&"true".equals(chooseAuRel.getValue()) &&!LoginHelper.getIsAdmin(request)){ %>
        <fmt:message key='gap.authority.currentrelation' bundle='${applicationAuResources}' />：&nbsp;
        <%} %>
        </td>
        <td align="right">
        <form name="form" method="post" action="<%=request.getContextPath()%>/LoginAction.do" target="_top">
        <%
                ...
                <tr>
                    <td height="25">
                        <select name="relationtype_id" style={width:150px} onChange="javascript:form.submit()">
                            <option value="-1" <%=relationtype_id.equals("-1")?"selected":""%> ><fmt:message key='gap.authority.Whole' bundle='${applicationAuResources}' /></option>
                        <%
                            for(int i=0; i<relTypeList.size(); i++) {
                                LabelValueBean tmp = (LabelValueBean) relTypeList.get(i);
                        %>
