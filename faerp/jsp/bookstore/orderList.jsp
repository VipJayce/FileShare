...
<%@ page import="gap.template.bookstore.common.Constant" %>
<%@ page import="gap.template.bookstore.model.*" %>
<html>
...
<script language="javascript">
	writeTableTop('<fmt:message key="orderlist"/>','<venus:base/>/');
</script>
...
<table class="table_div_content">
	<tr>
		...
		<layout:collection name="orders" id="order" styleClass="listCss" width="100%" indexId="orderNumber" align="center" sortAction="0"  >
			...
			<layout:collectionItem width="15%" title='<%=LocaleHolder.getMessage("gap.template.bookstore.orderno") %>' property="id" sortable="true" style="text-align:center;"/>
			<layout:collectionItem width="10%" title='<%=LocaleHolder.getMessage("gap.template.bookstore.consignee") %>' property="name" sortable="true" style="text-align:center;"/>
			<layout:collectionItem width="10%" title='<%=LocaleHolder.getMessage("gap.template.bookstore.tel") %>' property="tel"  style="text-align:center;"/>
			<layout:collectionItem width="6%" title='<%=LocaleHolder.getMessage("gap.template.bookstore.postcode") %>' property="postcode"  style="text-align:center;"/>
			<layout:collectionItem width="8%" title='<%=LocaleHolder.getMessage("gap.template.bookstore.orderamount") %>' property="money" sortable="true" style="text-align:center;"/>
			<layout:collectionItem width="10%" title='<%=LocaleHolder.getMessage("gap.template.bookstore.ordertime") %>' property="orderTime" sortable="true" style="text-align:center;"/>
			<layout:collectionItem width="10%" title='<%=LocaleHolder.getMessage("gap.template.bookstore.orderstatus") %>' property="orderStatus" sortable="true" style="text-align:center;">
				<bean:define id="status" name="order" property="orderStatus"/>
				<%=Constant.ORDER_STATUS.get(status)%>
			</layout:collectionItem>
			<layout:collectionItem width="12%" title='<%=LocaleHolder.getMessage("gap.template.bookstore.remark") %>' property="result"  style="text-align:center;"/>
			<layout:collectionItem title='<%=LocaleHolder.getMessage("gap.template.bookstore.operation") %>' width="25%" style="text-align:center;">
