...
<script language="javascript">
	...
	function detail_Click(){
	   var securityId = document.getElementById("securityId").value;
        ...
        window.open("<%=request.getContextPath()%>/EmpsecurityAction.do?cmd=detailQuerySocialSecurity&securityId=" + securityId+"&types=complex_insurance",'详细信息','height=600px,width=1024px,top=100,left=200,help=no,resizable=yes,scrollbars=yes,location=no,status=no');
        //$id("datacell_formid").action="<%=request.getContextPath()%>/EmpsecurityAction.do?cmd=detailQuerySocialSecurity&securityId=" + securityId+"&types=complex_insurance";
        //$id("datacell_formid").submit();
	}
	
	//导出
    function exportToExcel(){
        $id("datacell_formid").action="<%=request.getContextPath()%>/EmpsecurityAction.do?cmd=exportQuerySocialSecurityToExcel&para=complex_insurance";
        $id("datacell_formid").submit();
    }
...
<div id="right">
<script language="javascript">
writeTableTopFesco('综保查询','<%=request.getContextPath()%>/');  //显示本页的页眉
</script>
<div class="ringht_s">
        ...
        <tr>
            ...
            <td align="left">
                 <select id="security_group_id" name="security_group_id" >
                    <option value="<%=IEmpsecurityConstants.SECURITY_GROUP_ID4 %>">上海综保</option>
                 </select>
            </td>
...
<div style="padding: 8 10 8 8;">
    <r:datacell 
        id="datacell1"
        queryAction="/FAERP/EmpsecurityAction.do?cmd=listQueryComplexInsurance"
        width="98%"
        height="320px"
