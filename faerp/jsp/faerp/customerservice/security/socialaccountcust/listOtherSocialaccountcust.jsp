...
function citySecurityGroupchange(seletvalue){
    ...
    function findCheckbox_onClick() {  //从多选框到修改页面
        ...
        var url="<%=request.getContextPath()%>/SocialaccountcustAction.do?id=" + ids+"&cmd=find&group_type="+group_type;
        url=url+'&_ts='+(new Date()).getTime();   //防止IE缓存，在每次打开时加个时间差的参数
        window.open(url,'','height=300px,width=800px,top =100,left=200,help=no,resizable=yes,scrollbars=yes,location=no,status=not,title=修改客户帐号');
    ...
    function toAdd_onClick() {  //到增加记录页面
       var group_type= document.getElementById('group_type').value;
        var url="<%=request.getContextPath()%>/jsp/faerp/customerservice/security/socialaccountcust/insertOtherSocialaccountcust.jsp?group_type="+group_type;
        url=url+'&_ts='+(new Date()).getTime();   //防止IE缓存，在每次打开时加个时间差的参数
        window.open(url,'','height=300px,width=800px,top =100,left=200,help=no,resizable=yes,scrollbars=yes,location=no,status=not,title=新增客户帐号');
    ...
    function detail_onClick(){  //实现转到详细页面
       var group_type= document.getElementById('group_type').value;
        ...
        var url="<%=request.getContextPath()%>/SocialaccountcustAction.do?id=" + ids+"&group_type="+group_type+"&cmd=detail";
        url=url+'&_ts='+(new Date()).getTime();   //防止IE缓存，在每次打开时加个时间差的参数
...
<div id="ccParent1" class="button"> 
 <div class="button_right">
         ...
         -->
         <input type="button" class="a" value="<fmt:message key="view"/>" <au:authorityBtn code="ydsbzh_view2" type="1"/>  onclick="detail_onClick();"/>
         <input type="button" class="c" value="<fmt:message key="insert"/>" <au:authorityBtn code="ydsbzh_add2" type="1"/>  onclick="toAdd_onClick();"/>
         <input type="button" class="d" value="<fmt:message key="delete"/>" <au:authorityBtn code="ydsbzh_del2" type="1"/>  onclick="deleteMulti_onClick();"/>
         <input type="button" class="b" value="<fmt:message key="modify"/>" <au:authorityBtn code="ydsbzh_modify2" type="1"/>  onclick="findCheckbox_onClick();"/>
         </div>
           <div class="clear"></div>            
</div>
