...
function citySecurityGroupchange(seletvalue){
	...
	function findCheckbox_onClick() {  //从多选框到修改页面
		...
		//form.cmd.value = "find";
		//form.submit();
		var url="<%=request.getContextPath()%>/SocialaccountpvdAction.do?id=" + ids+"&cmd=find&group_type="+group_type+"&security_type="+security_type;;
        url=url+'&_ts='+(new Date()).getTime();   //防止IE缓存，在每次打开时加个时间差的参数
        window.open(url,'','height=300px,width=800px,top =100,left=200,help=no,resizable=yes,scrollbars=yes,location=no,status=not,title=修改账号');
        ...
        jQuery.getJSON(vpath+"/SocialaccountpvdAction.do?cmd=checkSocialUnit&date="+new Date()+"",{"ids":ids},  function(json){
             if(json.saia!=null && json.saia.length>0){
                 for(var i = 0; i < json.saia.length; i++){
                    alert("账号："+json.saia[i].account+"对应的福利办理方存在在缴的员工记录，不能删除！");
                 }
             }else{
	...
	function toAdd_onClick() {  //到增加记录页面
	    var group_type= document.getElementById('group_type').value;
	    var security_type= document.getElementById('security_type').value;
		var url="<%=request.getContextPath()%>/jsp/faerp/customerservice/security/socialaccountpvd/insertSocialaccountpvd_zj.jsp?group_type="+group_type+"&security_type="+security_type;
        url=url+'&_ts='+(new Date()).getTime();   //防止IE缓存，在每次打开时加个时间差的参数
        window.open(url,'','height=300px,width=800px,top =100,left=200,help=no,resizable=yes,scrollbars=yes,location=no,status=not,title=新增客户账号');
	...
	function detail_onClick(){  //实现转到详细页面
	   var group_type= document.getElementById('group_type').value;
		...
		//form.action="<%=request.getContextPath()%>/SocialaccountpvdAction.do?id=" + ids+"&group_type="+group_type+"&security_type="+security_type;
		//form.cmd.value = "detail";
		//form.submit();
		var url="<%=request.getContextPath()%>/SocialaccountpvdAction.do?id=" + ids+"&group_type="+group_type+"&security_type="+security_type+"&cmd=detail";
        url=url+'&_ts='+(new Date()).getTime();   //防止IE缓存，在每次打开时加个时间差的参数
...
<div id="ccParent1" class="button"> 
 <div class="button_right">
		 ...
		 -->
		 <input class="a" type="button"   value="查看"  <au:authorityBtn code="sbzh_view1" type="1"/>  onClick="detail_onClick();">
         <input class="c" type="button"   value="新增"  <au:authorityBtn code="sbzh_add1" type="1"/>  onClick="toAdd_onClick();">
         <input class="d" type="button"   value="删除"  <au:authorityBtn code="sbzh_del1" type="1"/>  onClick="deleteMulti_onClick();">
         <input class="b" type="button"   value="修改"  <au:authorityBtn code="sbzh_modify1" type="1"/>  onClick="findCheckbox_onClick();">
         </div>
		 </div>
           <div class="clear"></div>			
</div>
...
<div style="padding: 8 10 8 8;">
    <r:datacell 
      ...
      <r:field fieldName="social_unit_id" label="福利办理方" width="100px" allowModify="false">
      ...
      <r:field fieldName="accounts" label="账号" width="250px" allowModify="false">
      </r:field>
