...
<input type="hidden" name="security_group_id" id="security_group_id" value="${param.security_group_id }">
    ...
    <r:datacell 
        ...
        >
      <r:field fieldName="product_name" label="产品" width="80px" allowModify="false">
      </r:field>
      
      <r:field fieldName="base" label="基数" width="80px" onRefreshFunc="(false||function(cellText, entity, rn, cn, cur){return formatNumber(cellText,'#,###.##');})">
        <c:if test="${param.isModify=='1'}">
        <h:text onchange="javascript:calMoneyAgain();"/>
        </c:if>
      </r:field>
      
      <r:field fieldName="money" label="金额" width="80px" onRefreshFunc="(false||function(cellText, entity, rn, cn, cur){return formatNumber(cellText,'#,###.##');})">
        <c:if test="${param.isModify=='1'}">
        <h:text onchange="javascript:calAllmoney();"/>
        </c:if>
      </r:field>
      
      <r:field fieldName="e_money" label="企业金额" width="80px" onRefreshFunc="(false||function(cellText, entity, rn, cn, cur){return formatNumber(cellText,'#,###.##');})">
            <c:if test="${param.isModify=='1'}">
            <h:text onchange="javascript:calMoneyAgain1();"/>
            </c:if>
      </r:field>
      
      <r:field fieldName="p_money" label="个人金额" width="80px" onRefreshFunc="(false||function(cellText, entity, rn, cn, cur){return formatNumber(cellText,'#,###.##');})">
            <c:if test="${param.isModify=='1'}">
            <h:text onchange="javascript:calMoneyAgain1();"/>
            </c:if>
      </r:field>
      
      <r:field fieldName="e_ratio" label="企业比例" width="80px" allowModify="false">
      </r:field>
      
      <r:field fieldName="p_ratio" label="个人比例" width="80px" allowModify="false">
      </r:field>
      
      <r:field fieldName="" label="回改订单" width="80px" allowModify="false">
      </r:field>
      
      <r:field fieldName="" label="是否改订单" width="80px" allowModify="false">
      </r:field>
    </r:datacell>
    </div>
</div>
...
<input type="button" class="foot_icon_1" value='确定'  onclick="javascript:back_onClick();" />
</div>
</form>
</fmt:bundle>
<script language="javascript">
    ...
    function calMoneyAgain(){
        ...
        if(entCur.getProperty("base")!=null && entCur.getProperty("base")!=''){
            var base    = getNumber(entCur.getProperty("base"));//基数
            var money   = getNumber(entCur.getProperty("money"));//金额
            var e_money = getNumber(entCur.getProperty("e_money"));//企业金额
            var p_money = getNumber(entCur.getProperty("p_money"));//个人金额
...
<script>
    ...
    function toAdd_onClick() {
        ...
        showModalCenter(url, window,toaddProductlistrollback,230,260,"添加产品");
    }
