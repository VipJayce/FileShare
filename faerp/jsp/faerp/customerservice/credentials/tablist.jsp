...
<title><fmt:message key="no_title"/></title>
 <script  language="javascript">
    var tabs;
     function updateData() { //定义修改状态时显示的标签页
             tabs  =  new Array(
                new Array ("<fmt:message key="add_credentials_type"/>","<%=request.getContextPath()%>/CredentialsAction.do?cmd=queryNewHandle"),
                new Array ("<fmt:message key="credentials_finish"/>","<%=request.getContextPath()%>/CredentialsAction.do?cmd=queryHandleComplete")
                );
      }
 updateData(); 
 </script>
