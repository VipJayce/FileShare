...
<%  //判断是否为修改页面
	...
	if(request.getParameter("isModify") != null) {  //如果从request获得参数"isModify"不为空
		...
		if(request.getAttribute(IPbServiceSmallTypeConstants.REQUEST_BEAN) != null) {  //如果request中取出的bean不为空
...
<script language="javascript">
	function insert_onClick(){  //插入单条数据
	    if(checkAllForms()){
		form.action="<%=request.getContextPath()%>/PbServiceSmallTypeAction.do?cmd=insert";
		form.submit();
		}
	}
  	...
  	function update_onClick(id){  //保存修改后的单条数据
	    ...
	    form.action="<%=request.getContextPath()%>/PbServiceSmallTypeAction.do?cmd=update";
    	form.submit();
    	}
	}
...
<table  width="100%" height="40" border="0" cellpadding="0" cellspacing="1" bgcolor="#c5dbe2">
		...
		<tr>
			...
			<td class="td_2 ">
				<input type="text" class="text_field" name="service_big_type_id" inputName="大分类名称" value="" maxLength="9.5" />
			</td>
			<td class="td_2" ></td>
			<td class="td_2" ></td>
		</tr>
		<tr>
			...
			<td class="td_2 ">
				<input type="text" class="text_field" name="name" inputName="小分类名称" value="" maxLength="50" />
			</td>
			<td class="td_2" ></td>
			<td class="td_2" ></td>
		</tr>
