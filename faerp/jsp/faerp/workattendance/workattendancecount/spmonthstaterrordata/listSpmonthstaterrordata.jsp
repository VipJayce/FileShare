...
<%@ page import="rayoo.workattendance.workattendancecount.spmonthstaterrordata.vo.SpmonthstaterrordataVo" %>
<%@ page import="rayoo.workattendance.workattendancecount.spmonthstaterrordata.util.ISpmonthstaterrordataConstants" %>
<%  //取出List
	List lResult = null;  //定义结果列表的List变量
	if(request.getAttribute(ISpmonthstaterrordataConstants.REQUEST_BEANS) != null) {  //如果request中的beans不为空
		lResult = (List)request.getAttribute(ISpmonthstaterrordataConstants.REQUEST_BEANS);  //赋值给resultList
	}
	Iterator itLResult = null;  //定义访问List变量的迭代器
	if(lResult != null) {  //如果List变量不为空
		itLResult = lResult.iterator();  //赋值迭代器
	}
	SpmonthstaterrordataVo resultVo = null;  //定义一个临时的vo变量
%>
<html>
...
<script language="javascript">
	...
	function findCheckbox_onClick() {  //从多选框到修改页面
		...
		form.action="<%=request.getContextPath()%>/SpmonthstaterrordataAction.do?id=" + ids;
		form.cmd.value = "find";
		form.submit();
	}  
	function deleteMulti_onClick(){  //从多选框物理删除多条记录
		...
		if(confirm('<fmt:message key="sure_to_delete"/>')) {  //如果用户在确认对话框按"确定"
	    	form.action="<%=request.getContextPath()%>/SpmonthstaterrordataAction.do?ids=" + ids;
	...
	function toAdd_onClick() {  //到增加记录页面
		form.action="<%=request.getContextPath()%>/jsp/faerp/workattendance/workattendancecount/spmonthstaterrordata/insertSpmonthstaterrordata.jsp";
		form.submit();
	}
...
<form name="form" method="post" action="<%=request.getContextPath()%>/SpmonthstaterrordataAction.do">
...
<tr><td>
	<table class="table_noFrame" width="100%">
		<tr>
			<td align="right">errorrow</td>
			<td align="left">
				<input type="text" class="text_field" name="errorrow" inputName="errorrow" maxLength="4"/>
			</td>
			<td align="right"></td>
			<td align="left"></td>
		</tr>
		<tr>
			<td align="right">errorcell</td>
			<td align="left">
				<input type="text" class="text_field" name="errorcell" inputName="errorcell" maxLength="4"/>
			</td>
			<td align="right"></td>
			<td align="left"></td>
		</tr>
		<tr>
			<td align="right">errormsg</td>
			<td align="left">
				<input type="text" class="text_field" name="errormsg" inputName="errormsg" maxLength="200"/>
			</td>
			<td align="right"></td>
			<td align="left"></td>
		</tr>
		...
		<tr>
			<td align="right">period_id</td>
			<td align="left">
				<input type="text" class="text_field" name="period_id" inputName="period_id" maxLength="9.5"/>
			</td>
			<td align="right"></td>
			<td align="left"></td>
		</tr>
...
<table class="table_div_content2">
	<tr>
		...
		<layout:collection name="beans" id="rmBean" styleClass="listCss" width="100%" indexId="orderNumber" align="center" sortAction="0">
			...
			<layout:collectionItem width="8%" title="errorrow" property="errorrow" sortable="true"/>
			
			<layout:collectionItem width="8%" title="errorcell" property="errorcell" sortable="true"/>
			
			<layout:collectionItem width="8%" title="errormsg" property="errormsg" sortable="true"/>
			...
			<layout:collectionItem width="8%" title="period_id" property="period_id" sortable="true"/>
...
<script language="javascript">
<%  //表单回写
	if(request.getAttribute(ISpmonthstaterrordataConstants.REQUEST_WRITE_BACK_FORM_VALUES) != null) {  //如果request中取出的表单回写bean不为空
		out.print(RmVoHelper.writeBackMapToForm((java.util.Map)request.getAttribute(ISpmonthstaterrordataConstants.REQUEST_WRITE_BACK_FORM_VALUES)));  //输出表单回写方法的脚本
	}
%>
</script>	
