...
<%  //判断是否为修改页面
	...
	if(request.getParameter("isModify") != null) {  //如果从request获得参数"isModify"不为空
		...
		if(request.getAttribute(IImportdataConstants.REQUEST_BEAN) != null) {  //如果request中取出的bean不为空
  		    resultVo = (ImportdataVo)request.getAttribute(IImportdataConstants.REQUEST_BEAN);  //从request中取出vo, 赋值给resultVo
  		}
	}
%>
<html>
...
<script language="javascript">
	function insert_onClick(){  //插入单条数据
		form.action="<%=request.getContextPath()%>/ImportdataAction.do?cmd=insert";
		form.submit();
	}
  	...
  	function update_onClick(id){  //保存修改后的单条数据
	    ...
	    form.action="<%=request.getContextPath()%>/ImportdataAction.do?cmd=update";
    	form.submit();
	}

    function cancel_onClick(){  //取消后返回列表页面
        form.action="<%=request.getContextPath()%>/ImportdataAction.do?cmd=queryAll&backFlag=true";
        form.submit();
    }
</script>
</head>
...
<tr><td> 
	<table class="table_div_content_inner">
		<tr>
			<td align="right">刷卡状态（签到、签退标志）</td>
			<td align="left">
				<input type="text" class="text_field" name="datastatus" inputName="刷卡状态（签到、签退标志）" value="" maxLength="4" />
			</td>
			<td align="right"></td>
			<td align="left"></td>
		</tr>
		<tr>
			<td align="right">日历时间(签卡时间)</td>
			<td align="left">
				<input type="text" class="text_field_reference_readonly" name="calendartime" id="calendartime" inputName="日历时间(签卡时间)" /><img class="refButtonClass"	src="<venus:base/>/images/icon/reference.gif" onClick="javascript:getYearMonthDay('calendartime','<venus:base/>/');"/>
			</td>
			<td align="right"></td>
			<td align="left"></td>
		</tr>
		<tr>
			...
			<td align="left">
				<input type="text" class="text_field" name="timecardid" inputName="考勤卡号" value="" maxLength="10" />
			</td>
			<td align="right"></td>
			<td align="left"></td>
		</tr>
		<tr>
			<td align="right">公司主键</td>
			<td align="left">
				<input type="text" class="text_field" name="cust_id" inputName="公司主键" value="" maxLength="9.5" />
			</td>
			<td align="right"></td>
			<td align="left"></td>
		</tr>
