...
<script language="javascript">
	function findCheckbox_onClick() {  //从多选框到修改页面
	    ...
	    var url="<%=request.getContextPath()%>/ElfhandAction.do?cmd=find2&id=" + ids;
        url=url+'&_ts='+(new Date()).getTime();   //防止IE缓存，在每次打开时加个时间差的参数
        showModalCenter(url, window, callBack, 650, 230, "修改非常规岗位");  
    }  
	...
	function deleteMulti_onClick(){  //从多选框物理删除多条记录
		...
		if(confirm('<fmt:message key="sure_to_delete"/>')) {  //如果用户在确认对话框按"确定"
		    form.action="<%=request.getContextPath()%>/ElfhandAction.do?cmd=batchDeleteMulti2&ids=" + ids;
            form.submit();
		}
	}

	function toAdd_onClick() {  //到增加记录页面
		var url="<%=request.getContextPath()%>/jsp/faerp/elfhand/unConventionalPost/insertUnConventionalPost.jsp";
        showModalCenter(url, window, callBack, 650, 230, "新增非常规岗位"); 
	}
	...
	function detail_onClick(){  //实现转到详细页面
        ...
        var url="<%=request.getContextPath()%>/ElfhandAction.do?cmd=detail2&id=" + ids;
        url=url+'&_ts='+(new Date()).getTime();   //防止IE缓存，在每次打开时加个时间差的参数
        showModalCenter(url, window, callBack, 650, 230, "查看非常规岗位");  
	}
...
<form name="form" method="post" id="datacell_formid">
    <div id="right">
		<!-- <script language="javascript">
			writeTableTopFesco('非常规岗位','<%=request.getContextPath()%>/');  //显示本页的页眉
		</script> -->
    	<div class="ringht_s"> 
			<div id="ccChild0" class="search"> 
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					...
					<tr>
				  		...
				  		<td align="left">
				       		<input type="text" class="text_field" name="empCode" inputName="唯一号" maxLength="64"/>
					  	</td>
					  	<td align="right">姓名&nbsp;&nbsp;&nbsp;</td>
			          	<td align="left">
			              	<input type="text" class="text_field" name="name" inputName="姓名" maxLength="50"/>
			          	</td>
			          	<td align="right"></td>
			...
			<div id="ccParent1" class="button"> 
 			...
 			<div style="padding: 8 10 8 8;">
    			<r:datacell 
		        id="datacell1"
		        queryAction="/FAERP/ElfhandAction.do?cmd=getAll2"
		        width="98%"
		        height="318px"
		        xpath="UnConventionalPostVo"
		        ...
		        >
		      	...
		      	<r:field fieldName="empCode" label="唯一号" width="300px"></r:field>
      			<r:field fieldName="name" label="姓名" width="300px"></r:field>
      			<r:field fieldName="unit" label="单位" width="200px"></r:field>
      			<r:field fieldName="typeOfWork" label="岗位工种" width="260px"></r:field>
			    </r:datacell>
    		</div>
		</div>
	</div>
