...
<input type="hidden" id="no_amount_flag" value="0" />
<input type="hidden" id="incoming_id" value="${bean.id}" />
<div id="right">
    <div class="ringht_s">
...
function subinvoice(){
        ...
        var incoming_id = _$("#incoming_id").val();//到款id
        //开票项目 以及金额  
        ...
        var un_claim_amout = _$("#un_claim_amout_h").val();//未认领金额
       ...
       if(_$("#no_amount_flag").val() == "0" && (parseFloat(invoice_amount) > parseFloat(un_claim_amout)) ){
            //如果有认领 则发票金额不允许大于
            alert("发票金额不能大于未认领金额!");
            return false; 
       }
       ...
       if(isRepeat(sValueArray) && isErrSelect(sValueArray) ){
            ...
            var _url = "<%=request.getContextPath()%>/FinclaimAction.do?cmd=noBillFininvoice&_ts="+(new Date()).getTime()+"&sTextArray="+sTextArray+"&sValueArray="+sValueArray+"&incoming_id="+incoming_id+"&receipt_title="+encodeURI(receipt_title)+"&contact_person="+encodeURI(contact_person)+"&contact_address="+encodeURI(contact_address)+"&contact_tel="+encodeURI(contact_tel)+"&bill_year_month="+bill_year_month+"&invoice_amount="+invoice_amount+"&amountArray="+amountArray+"&invoice_desc="+encodeURI(invoice_desc)+"&is_express="+is_express+"&attachName="+encodeURI(attachName)+"&attachAddress="+encodeURI(attachAddress)+"&apply_reason="+encodeURI(apply_reason); 
            _$.ajax({
                type : "post",
                url : _url,
                data : param,
