_$(document).ready(function() {
    ...
    if(datacell2) {
        ...
        datacell2.init();
        //加载数据
        datacell2.queryAction = "/FAERP/BillmodifyAction.do?cmd=getItemBybillid4DataCellPage";    
        datacell2.loadData();
        datacell2.refresh();
        //
...
function datacell4rightsum() {
	...
	_$(".eos-body-div .eos-table tr").each(function(i){
...
function list2item() {
    ...
    _$.ajax({
        type : "post",
        url : venusbase + "/FinbillAction.do?cmd=checklist2item&bill_id=" + bill_id,
        dataType : "html",
        success : function(data) {
        	if (data) {
                ...
                if (bill_id) {
                    window.location.href = venusbase + "/FinbillAction.do?cmd=list2item&bill_id=" + bill_id;
                }
        	}
        }
    });
...
function modifysub_callback(retVal) {
	...
	//dc2.addParam('applymsg', retVal);
	//dc2.loadData();
	//dc2.refresh();
    var xmlSubmit = dc2.dataset.toString();
    var myAjax = new Ajax(dc2.submitAction);
