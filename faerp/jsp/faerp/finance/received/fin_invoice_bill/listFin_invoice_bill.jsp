...
<%@ page import="rayoo.finance.received.fin_invoice_bill.vo.Fin_invoice_billVo" %>
<%@ page import="rayoo.finance.received.fin_invoice_bill.util.IFin_invoice_billConstants" %>
<%  //取出List
	List lResult = null;  //定义结果列表的List变量
	if(request.getAttribute(IFin_invoice_billConstants.REQUEST_BEANS) != null) {  //如果request中的beans不为空
		lResult = (List)request.getAttribute(IFin_invoice_billConstants.REQUEST_BEANS);  //赋值给resultList
	}
	Iterator itLResult = null;  //定义访问List变量的迭代器
	if(lResult != null) {  //如果List变量不为空
		itLResult = lResult.iterator();  //赋值迭代器
	}
	Fin_invoice_billVo resultVo = null;  //定义一个临时的vo变量
%>
<html>
...
<script language="javascript">
	...
	function findCheckbox_onClick() {  //从多选框到修改页面
		...
		form.action="<%=request.getContextPath()%>/Fin_invoice_billAction.do?id=" + ids;
		form.cmd.value = "find";
		form.submit();
	}  
	function deleteMulti_onClick(){  //从多选框物理删除多条记录
		...
		if(confirm('<fmt:message key="sure_to_delete"/>')) {  //如果用户在确认对话框按"确定"
	    	form.action="<%=request.getContextPath()%>/Fin_invoice_billAction.do?ids=" + ids;
	...
	function toAdd_onClick() {  //到增加记录页面
		form.action="<%=request.getContextPath()%>/jsp/faerp/finance/received/fin_invoice_bill/insertFin_invoice_bill.jsp";
		form.submit();
	}
...
<form name="form" method="post" action="<%=request.getContextPath()%>/Fin_invoice_billAction.do">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
		...
		<tr>
			<td align="right">fin_invoice_id</td>
			<td align="left">
				<input type="text" class="text_field" name="fin_invoice_id" inputName="fin_invoice_id" maxLength="9.5"/>
			</td>
			<td align="right"></td>
			<td align="left"></td>
			<td align="right"></td>
...
<th>
			  	...
			  	<a href="/FAERP/Fin_invoice_billAction.do?cmd=queryAll&VENUS_ORDER_KEY=<%=royoo.common.TagSortUitl.getSortColumn(request,"company_id")%>">
			...
			<th>
			  	...
			  	<a href="/FAERP/Fin_invoice_billAction.do?cmd=queryAll&VENUS_ORDER_KEY=<%=royoo.common.TagSortUitl.getSortColumn(request,"fin_invoice_id")%>">
            	<img src="/FAERP/images/<%=royoo.common.TagSortUitl.getSortImage(request,"fin_invoice_id")%>" border="0" title="<%=royoo.common.TagSortUitl.getSortTitle(request,"fin_invoice_id")%>">
           	</a>	
       
			 fin_invoice_id</th>
			<th>
				
			  	<a href="/FAERP/Fin_invoice_billAction.do?cmd=queryAll&VENUS_ORDER_KEY=<%=royoo.common.TagSortUitl.getSortColumn(request,"bill_id")%>">
			...
			<th>
			  	...
			  	<a href="/FAERP/Fin_invoice_billAction.do?cmd=queryAll&VENUS_ORDER_KEY=<%=royoo.common.TagSortUitl.getSortColumn(request,"is_valid")%>">
			...
			<th>
			  	...
			  	<a href="/FAERP/Fin_invoice_billAction.do?cmd=queryAll&VENUS_ORDER_KEY=<%=royoo.common.TagSortUitl.getSortColumn(request,"is_del")%>">
			...
			<th>
			  	...
			  	<a href="/FAERP/Fin_invoice_billAction.do?cmd=queryAll&VENUS_ORDER_KEY=<%=royoo.common.TagSortUitl.getSortColumn(request,"last_update_user_id")%>">
			...
			<th>
				 	...
				 	<a href="/FAERP/Fin_invoice_billAction.do?cmd=queryAll&VENUS_ORDER_KEY=<%=royoo.common.TagSortUitl.getSortColumn(request,"last_update_date")%>">
...
<td><bean:write name="data" property="company_id"/>   </td>
			...
			<td><bean:write name="data" property="fin_invoice_id"/>   </td>
...
<script language="javascript">
<%  //表单回写
	if(request.getAttribute(IFin_invoice_billConstants.REQUEST_WRITE_BACK_FORM_VALUES) != null) {  //如果request中取出的表单回写bean不为空
		out.print(RmVoHelper.writeBackMapToForm((java.util.Map)request.getAttribute(IFin_invoice_billConstants.REQUEST_WRITE_BACK_FORM_VALUES)));  //输出表单回写方法的脚本
	}
%>
</script>	
