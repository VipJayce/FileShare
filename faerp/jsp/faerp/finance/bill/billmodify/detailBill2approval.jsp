	                   ...
	                   <logic:iterate name="smalltitles" id="title" scope="page">
						  <c:choose>
						    <c:when test="${fn:contains(title, '_city') }">
						        <r:field fieldName="${title }" label="城市" groupval="${smalltitles[9] }" allowModify="false" width="70">
						        </r:field>
						    </c:when>
						    <c:when test="${fn:contains(title, '_base') }">
						        <r:field fieldName="${title }" label="基数" groupval="${smalltitles[9] }"  allowModify="false" width="70">
						        </r:field>
						    </c:when>
						    <c:when test="${fn:contains(title, '_amount') }">
						      <r:field fieldName="${title }" label="总额" groupval="${smalltitles[9] }" allowModify="false" width="70">
						      </r:field>
						    </c:when>
						    <c:when test="${fn:contains(title, '_eratio') }">
						       <r:field fieldName="${title }" label="单位比例" groupval="${smalltitles[9] }" allowModify="false" width="70">
						       </r:field>
						    </c:when>
						    <c:when test="${fn:contains(title, '_emoney') }">
						        <r:field fieldName="${title }" label="单位金额" groupval="${smalltitles[9] }" allowModify="false" width="70">
						        </r:field>
						    </c:when>
						    <c:when test="${fn:contains(title, '_pratio') }">
						        <r:field fieldName="${title }" label="个人比例" groupval="${smalltitles[9] }" allowModify="false" width="70">
						        </r:field>
						    </c:when>
						    <c:when test="${fn:contains(title, '_pmoney') }">
						        <r:field fieldName="${title }" label="个人金额" groupval="${smalltitles[9] }" allowModify="false" width="70">
						        </r:field>
						    </c:when>
						    <c:when test="${fn:contains(title, '_bill_imp_type_db') }">
						        <r:field fieldName="${title }" label="收费属性" groupval="${smalltitles[9] }" allowModify="false" width="70">
						        </r:field>
						    </c:when>
						  </c:choose>
					   </logic:iterate>
	...
	<div class="box_tab">
      <table width="100%" cellspacing="1" cellpadding="0" border="0" bgcolor="#c5dbe2">
        <tr>
          ...
          <td class="td_2" style="width: 30%;">
                <table style="line-height:15px;">
                    <tr>
                        <td style="width: 30%;text-align: left;">原因大类 : </td>
                        <td style="width: 70%;text-align: left;">${billVo.bill_reason_big_type_str }</td>
                    </tr>
                    <tr>
                        <td style="width: 10%;text-align: left;">原因小类 : </td>
                        <td style="width: 70%;text-align: left;">${billVo.bill_reason_small_type_str }</td>
                    </tr>
                </table>
          </td>
          <td class="td_1">其他原因</td>
          <td class="td_2" >
            <textarea id="text23" rows="3"  cols="20" name="text22" readonly="readonly">${billVo.bill_other_reason }</textarea>
          </td>
          <td class="td_1">备注</td>
          <td class="td_2" >
            <textarea id="text22" rows="3"  cols="20" name="text22" readonly="readonly">${billVo.applyfor }</textarea>
          </td>
          </tr>
      </table>
