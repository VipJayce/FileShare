...
<%  //判断是否为修改页面
	...
	if(request.getParameter("isModify") != null) {  //如果从request获得参数"isModify"不为空
		...
		if(request.getAttribute(IBillitemhisConstants.REQUEST_BEAN) != null) {  //如果request中取出的bean不为空
...
<script language="javascript">
	function insert_onClick(){  //插入单条数据
	    if(checkAllForms()){
		form.action="<%=request.getContextPath()%>/BillitemhisAction.do?cmd=insert";
		form.submit();
		}
	}
  	...
  	function update_onClick(id){  //保存修改后的单条数据
	    ...
	    form.action="<%=request.getContextPath()%>/BillitemhisAction.do?cmd=update";
    	form.submit();
    	}
	}
...
<table  width="100%" height="40" border="0" cellpadding="0" cellspacing="1" bgcolor="#c5dbe2">
		...
		<tr>
			...
			<td class="td_2 ">
				<input type="text" class="text_field" name="old_bill_item_id" inputName="old_bill_item_id" value="" maxLength="9.5" />
			</td>
			<td class="td_2" ></td>
			<td class="td_2" ></td>
		</tr>
