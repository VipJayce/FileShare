...
function generateBill(){
     ...
     */
      ...
      if(bill_type_bd=="7"){
         ...
         jQuery("#cost_center_id_hidden").val(cost_center_id);
         jQuery("#ibdRepeatFlag").val("1");
         window.open("<%=request.getContextPath()%>/jsp/faerp/finance/bill/salarybatch/ibd_generate_bill_selectproduct.jsp?contract_id="+ jQuery("#contract_id").val());
				    ...
				    var retstr=ajax.getValue("root/data/returnValue");
				     ...
				     if(returnNode) {
				         if(retsinfo[0]=="1") {
				             ...
				             alert(retsinfo[1]);
				         }else if(retsinfo[0].substring(0,2)=="57"){
				            //未红冲 邮件通知AR
				            if(confirm("账单数据财务已入账，是否邮件通知财务AR组取消入账！")){
                                       _$.ajax({
                                            type : "post",
                                            url :"/FAERP/FinbillAction.do?cmd=sendEmailToARTeam&bill_id=" + retsinfo[0].substring(3),
                                            dataType : "html",
                                            success : function(dt) {
                                                                   alert(dt);
                                                             }
...
if(generatetype=='generateBill'){ //直接生成
	var ajax = new Ajax($id("datacell1").submitAction);
			    ajax.addParam("ids", ids);
			    ajax.submitForm(frm);
			    showProgressBar();
		    ...
		    var ajax = new Ajax('/FAERP/BillCreateAction.do?cmd=generateBillThread2');
			    ajax.addParam("ids", ids);
			    ajax.addParam("batch_ids","");
			    ajax.submitForm(frm);
			    showProgressBar();
			    ...
			    var retstr=ajax.getValue("root/data/returnValue");
			     ...
			     if(returnNode) {
			         if(retsinfo[0]=="1") {
			             ...
			             alert("<fmt:message key='alert_invoke_fail'/>");
			             alert(retsinfo[1]);
			         }else if(retstr.substring(0,2)=="57"){
                            //未红冲 邮件通知AR
                            if(confirm("账单数据财务已入账，是否邮件通知财务AR组取消入账！")){
                                       _$.ajax({
                                            type : "post",
                                            url :"/FAERP/FinbillAction.do?cmd=sendEmailToARTeam&bill_id=" + retstr.substring(3),
                                            dataType : "html",
                                            success : function(dt) {
                                                                   alert(dt);
                                                             }
...
//tab切换
   ...
   function onchange_billtypedb(){
           ...
           $id("button_search").disabled="" ;
           $id("button_reset").disabled="" ;
           jQuery("[name=scopetype_ibd]").hide();
           if(bill_type_bd =='7'){  //IBD证件账单
	           $id("button_search").disabled="disabled" ;
	           $id("button_reset").disabled="disabled" ;
               jQuery("[name=scopetype_ibd]").show();
               jQuery("[notFinout=true]").hide();
    ...
    function toFindContract(obj) {
	    jQuery("#contract_id").val(obj[0]);
	    jQuery("#contract_type_bd").val(obj[6]);
	    var lookup = $id("ibd_contract_name");
          ...
          success: function(text){
                     if(text== "get_success"){
                        ...
                        flag = true;
                           ...
                           if(confirm("该成本中心需要的XPress系统内当前年月的账单明细还未生成，是否生成？")){
                               jQuery.ajax({
                                      url: url,
                                      type: 'post',
                                      dataType: 'html',
...
<body>
        ...
        <div id="bg" class="xixi1" >
            <div id="font1" class="tab1" onMouseDown="setTab03Syn(1);document.getElementById('bg').className='xixi1'"><fmt:message key='tab_re_xixi1'/></div>
...
<table width="100%" border="0">
    ...
    <tr>
        ...
        <td align="left"><d:select dictTypeId="FB_BILL_TYPE_BD" id="bill_type_bd"
                ...
                filterOp="in" filterStr="1,2,7" onchange="onchange_billtypedb();"/></td>
    ...
    <tr>
        <td align="right">
            <fmt:message key='select_customer'/>
            <input type="hidden" id="ibdRepeatFlag" />
        </td>
	...
	<tr>
		<td align="right"><fmt:message key='recreate_reason'/></td>
		<td align="left"><input type="checkbox" value="1" name="recreate_reason"><fmt:message key='recreate_reason_'/>
        ...
        <td >&nbsp;</td>
        <td> 
            <input name="button_ok22" id="button_search" class="icon_1"  <au:authorityBtn code="cszd_cxxz" type="1"/>    type="button" value="<fmt:message key='button_search'/>" onClick="javascript:searchSalary()">
            <input name="button_ok" id="button_ok" class="icon_1"  <au:authorityBtn code="cszd_cszd" type="1"/>    type="button" value="<fmt:message key='tab_re_xixi1'/>" onClick="javascript:generateBill()">
            <input name="button_ok" id="button_ok" class="icon_3"  <au:authorityBtn code="cszd_Thread_cszd" type="1"/>    type="button" value="<fmt:message key='button_generateThreadBill'/>" onClick="javascript:generateThreadBill()">
            <input name="button_reset" id="button_reset" class="icon_1" <au:authorityBtn code="cszd_jdsz" type="1"/>  type="button" value="<fmt:message key='title_precision'/>" onClick="javascript:openNewWin();">
        </td>
	</tr>
...
<r:datacell id="datacell1"  queryAction="/FAERP/BillCreateAction.do?cmd=searchSalary4again"
	 width="98%" height="304px" xpath="SalarybatchVo" submitAction="/FAERP/BillCreateAction.do?cmd=generateBill2"
	...
	<r:field fieldName="pay_name" messagekey="pay_name" width="250px" >
	</r:field>
	<r:field fieldName="pay_year_month" messagekey="pay_year_month" allowModify="false"
		width="150px">
		<h:text />
	</r:field>
	<r:field fieldName="tax_month" messagekey="tax_month" width="190px"
		allowModify="false">
		<h:text />
	</r:field>
	<r:field fieldName="bill_year_month" messagekey="dc_bill_year_month" width="205px"
		allowModify="false">
		<h:text />
	</r:field>
...
<table width="100%" border="0">
    ...
    <tr>
        ...
        <div align="center">
        <input name="button_ok2" class="icon_1"   <au:authorityBtn code="cszd_cxrz" type="1"/>   type="button" value="<fmt:message key='button_searchCreateLog'/>" onClick="javascript:searchCreateLog()">
        <input name="button_ok2" class="icon_1"  <au:authorityBtn code="cszd_zdxx" type="1"/>    type="button" value="<fmt:message key='button_viewBillDetail'/>" onClick="javascript:viewBillDetail()">
        </div>
        </td>
        <td align="right"></td>
    </tr>
