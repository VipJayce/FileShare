...
<%@ page import="rayoo.finance.queryarrearage.queryfinbill.util.IQueryfinbillConstants" %>
<%@ include file="/jsp/include/global.jsp" %>
<%@ include file="/jsp/include/rmGlobal_insert.jsp" %>
<%  //判断是否为修改页面
	QueryfinbillVo resultVo = null;  //定义一个临时的vo变量
	boolean isModify = false;  //定义变量,标识本页面是否修改(或者新增)
	if(request.getParameter("isModify") != null) {  //如果从request获得参数"isModify"不为空
		isModify = true;  //赋值isModify为true
		if(request.getAttribute(IQueryfinbillConstants.REQUEST_BEAN) != null) {  //如果request中取出的bean不为空
  		    resultVo = (QueryfinbillVo)request.getAttribute(IQueryfinbillConstants.REQUEST_BEAN);  //从request中取出vo, 赋值给resultVo
  		}
	}
%>
<html>
...
<script language="javascript">
	function insert_onClick(){  //插入单条数据
		form.action="<%=request.getContextPath()%>/QueryfinbillAction.do?cmd=insert";
		form.submit();
	}
  	...
  	function update_onClick(id){  //保存修改后的单条数据
	    ...
	    form.action="<%=request.getContextPath()%>/QueryfinbillAction.do?cmd=update";
    	form.submit();
	}

    function cancel_onClick(){  //取消后返回列表页面
        form.action="<%=request.getContextPath()%>/QueryfinbillAction.do?cmd=queryAll&backFlag=true";
        form.submit();
    }
</script>
</head>
