       ...
       var dataset1 = datacell1.dataset;//取得页面的datacell
        for(var i=0; i<dataset1.getLength(); i++){
            ...
            if(datasettr.status == Entity.STATUS_NEW || datasettr.status == Entity.STATUS_MODIFIED){
	            ...
	            //2016-06-29   by zhouxiaolong  增加“开票类型”的保存    Begin
	            ...
	            //2016-06-29   by zhouxiaolong  增加“开票类型”的保存    End
...
<table  width="100%" height="40" border="0" cellpadding="0" cellspacing="1" bgcolor="#c5dbe2">
    ...
    <%  if(!"detail".equals(detail)) {%>
       ...
       <input class="bc"  type="<%=isModify?"button":"hidden"%>" style="padding-left:1px"  value="<fmt:message key="save"/>"  <au:authorityBtn code="service_product_save" type="1"/>  onClick="toSaveProduct_onClick();">
<%} %>
</div>    
   <div class="clear"></div>
    </div> 
       ...
       <r:field fieldName="id" label="操作" width="35px" onRefreshFunc="setCheckboxStatus"  align="center" ></r:field>
        <r:field fieldName="product_id" label="产品id" width="0px" allowModify="false" >
         ...
         <r:field fieldName="product_name" label="服务项名称" width="150px" allowModify="false" >
            ...
            <h:text validateAttr="allowNull=false;maxLength=100;"/>
        </r:field>
        <r:field fieldName="product_price" label="价格" width="70px" allowModify="true" >
        	<h:text validateAttr="allowNull=false;type=formatStr;regExpr=^\d+.{0,1}\d*$;message=价格输入错误;"/>
        </r:field>
        ...
        <r:field fieldName="is_sales_tax" label="是否可收增值税" width="130px" >
         <d:select dictTypeId="FINOUT_IS_SALES_TAX" extAttr="allowNull=false;"/> 
        </r:field>
        <r:field fieldName="business_type_bd" label="业务类型" width="130px" allowModify="true" >
         <d:select dictTypeId="FINOUT_TYPE_BD" nullLabel=""  extAttr="allowNull=false;"/>
        </r:field>
        ...
        <h:text validateAttr="type=formatStr;regExpr=[0][.]{1}\d+$;message=营业税比例输入错误;"/>
        </r:field>
         -->
    </r:datacell>
     <!-- 服务项  end-->
 ...
 <div class="mx_button" align="center">
     <%  if(null!=detail&&!"detail".equals(detail)) {%>
          ...
          if(!"1".equals(create_type_bd)&&!isModify){
           %>
                 <input type="button" class="icon_4" value='保存并提交审批' onClick="javascript:<%=isModify?"update_onClick()":"insert_onClick('yes_nogapwf')"%>" />
       <%} %>
