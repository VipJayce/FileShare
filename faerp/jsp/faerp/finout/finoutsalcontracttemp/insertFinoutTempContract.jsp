...
<script language="javascript">
    ...
    var lapse_date = document.getElementById("lapse_date").value;
       ...
       var param = jQuery("#tempcontractform").serialize();
        var url="<%=request.getContextPath()%>/Finout_contract_serviceAction.do?cmd=insert_tempcontract&"+param;
    ...
    var lapse_date = document.getElementById("lapse_date").value;
       ...
       if(effect_date!=""||effect_date.trim().length!=0&&lapse_date!=""||lapse_date.trim().length!=0){
        ...
        var param = jQuery("#tempcontractform").serialize();
        var url="<%=request.getContextPath()%>/Finout_contract_serviceAction.do?cmd=update_tempcontract&"+param;
        jQuery.post(url,function(data){
        if(data){
              alert("更新合同模版成功!");
                    window.close();
...
<script language="javascript">
    if(<%=isModify%>)
        writeTableTopFesco('修改财务外包合同模板','<%=request.getContextPath()%>/');
    else
        writeTableTopFesco('新增财务外包合同模板','<%=request.getContextPath()%>/');
</script>
 <div class="ringht_x">
                    ...
                    <table  width="100%" height="40" border="0" cellpadding="0" cellspacing="1" bgcolor="#c5dbe2">
                            ...
                            <tr>
                             <td class="td_1" >合同模板类型</td>                                                                
                                ...
                                <td class="td_2">
                                    <d:select dictTypeId="FINOUT_TYPE_BD" name="business_type_bd" id="business_type_bd" property="bean/business_type_bd" style="width:208px"/>
                                </td>
                            </tr> 
                              <tr>
