...
<script language="javascript">
    ...
    if(create_type_bd==1){
        var contract_template_id=document.form.contract_template_id.value;
          ...
          form.action="<%=request.getContextPath()%>/jsp/faerp/sales/salcontract/zpinserttab_list.jsp?create_type_bd="+create_type_bd+"&contract_type_bd="+contract_type_bd+"&contract_template_id="+contract_template_id+"&agent_b_id=<%=agent_b_id%>"+"&agent_b_name=<%=agent_b_name%>"+"&detail="+"<%=detail%>"+"&f="+"<%=f%>";
          }else{
         form.action="<%=request.getContextPath()%>/jsp/faerp/sales/salcontract/tab_list.jsp?create_type_bd="+create_type_bd+"&contract_type_bd="+contract_type_bd+"&contract_template_id="+contract_template_id+"&agent_b_id=<%=agent_b_id%>"+"&agent_b_name=<%=agent_b_name%>"+"&detail="+"<%=detail%>"+"&f="+"<%=f%>"+"&isQA=<%=isQA%>";
         }
    }else{
      if(contract_type_bd=="9"){
          form.action="<%=request.getContextPath()%>/jsp/faerp/sales/salcontract/zpinserttab_list.jsp?create_type_bd="+create_type_bd+"&contract_type_bd="+contract_type_bd+"&detail="+"<%=detail%>"+"&agent_b_id=<%=agent_b_id%>"+"&agent_b_name=<%=agent_b_name%>"+"&f="+"<%=f%>";
          }else{
        form.action="<%=request.getContextPath()%>/jsp/faerp/sales/salcontract/tab_list.jsp?create_type_bd="+create_type_bd+"&contract_type_bd="+contract_type_bd+"&detail="+"<%=detail%>"+"&agent_b_id=<%=agent_b_id%>"+"&agent_b_name=<%=agent_b_name%>"+"&f="+"<%=f%>"+"&isQA=<%=isQA%>";
     }
     //        form.action="<%=request.getContextPath()%>/jsp/faerp/sales/salcontract/insertSalcontract.jsp?create_type_bd="+create_type_bd+"&contract_type_bd="+contract_type_bd+"&agent_b_id=<%=agent_b_id%>"+"&agent_b_name=<%=agent_b_name%>";
...
<table  width="100%" height="40" border="0" cellpadding="0" cellspacing="1" bgcolor="#c5dbe2">
        <tr>
            ...
            <td class="td_2" >
            <%if("zp".equals(f)) {%>
            <d:select dictTypeId="CONTRACT_TYPE_BD" name="contract_type_bd" id="contract_type_bd" nullLabel="请选择" value="9" onchange="javascript:getContract_template(this,'contract_template','code','name','${bean.contract_type_bd}')"/>
            <%}else{ %>
            <d:select dictTypeId="CONTRACT_TYPE_BD" name="contract_type_bd" id="contract_type_bd" nullLabel="请选择" onchange="javascript:getContract_template(this,'contract_template','code','name','${bean.contract_type_bd}')"/>
            <%} %>
