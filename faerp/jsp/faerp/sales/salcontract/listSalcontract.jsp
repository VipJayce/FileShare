...
<script language="javascript">
    ...
    function findCheckbox_onClick() {  //从多选框到修改页面
        ...
        if("zp" != "<%=f%>"){
	   //     if(type==2&&(constatus==0||constatus==2||constatus==5)){
	        //WfCommonAjax.checkCurrentWf(wfname,contract_id,function(reData){
	                //if(reData=='0'){
	                    //没有进行发起过审批
	                    //form.action="<%=request.getContextPath()%>/SalcontractAction.do?id=" + ids
	                    //form.cmd.value = "find";
	                    //form.submit();
			            var url="<%=request.getContextPath()%>/SalcontractAction.do?cmd=find&id=" + ids+"&detail=modify";
			            ...
			            //showModalCenter(url, window, showListData, 1000, 800, "修改合同"); 
	                //}   
	                //else{
	         ...
	         //);        
	//    } else{
	//      alert(type+":"+constatus);
	 //        alert("<fmt:message key="contract_type_upload_attachments"/>");
	 //         return;
   // }  
    }else{
         if(constatus==0||constatus==2||constatus==5){
                    var url="<%=request.getContextPath()%>/SalcontractAction.do?cmd=find&id=" + ids+"&detail=modify&f=<%=f%>";
                    url=url+'&_ts='+(new Date()).getTime();   //防止IE缓存，在每次打开时加个时间差的参数
                    window.open(url,'','height=500px,width=900px,top =100,left=200,help=no,resizable=yes,scrollbars=yes,location=no,status=not,title=新增合同');
		    } else{
		         alert("<fmt:message key="contract_type_create_modify"/>");
		          return;
		    }  
    }
    ...
    function toAdd_onClick() {  //到增加记录页面
        var url="<%=request.getContextPath()%>/SalcontractAction.do?cmd=add&detail=add&f=<%=f%>";
        url=url+'&_ts='+(new Date()).getTime();   //防止IE缓存，在每次打开时加个时间差的参数
        ...
        //showModalCenter(url, window, callBack, 900, 250, "新增合同");  
    ...
    function detail_onClick(){  //实现转到详细页面
        ...
        //showModalCenter(url, window, "", 1000, 400, "查看详细信息");  
    }
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
     <tr>
        ...
        <tr>
            ...
            <td align="left">
                    <!--<d:select dictTypeId="CONTRACT_TYPE_BD" name="contract_type_bd" value="" />-->
                    <%if("zp".equals(f)) {%>
                    <d:select dictTypeId="CONTRACT_TYPE_BD" name="contract_type_bd" id="contract_type_bd" nullLabel="请选择" value="9"/>
                    <%}else{ %>
                    <d:select dictTypeId="CONTRACT_TYPE_BD" name="contract_type_bd" id="contract_type_bd" nullLabel="请选择"/>
                    <%} %>
                    <%//=gap.rm.tools.helper.RmJspHelper.getSelectField("contract_type_bd", -1,"CONTRACT_TYPE_BD",""," id=contract_type_bd",false) %> 
            </td>
            ...
            <td align="left">
                <w:lookup onReturnFunc="rtnFunc" lookupWidth="189px" readonly="true" id="lk_cust" lookupUrl="/CustomersimpleAction.do?cmd=getAllCustomerByCondition1&f=${f}" messagekey="choose_customer" height="500" width="810"/>
...
<div id="ccParent1" class="button"> 
 <div class="button_right">
                ...
                <!--     <li class="d"><a  onClick="javascript:deleteMulti_onClick();"><fmt:message key="delete"/> </a> </li>  -->
                    ...
                    <li class="approdetial"><a  onClick="javascript:workflow_onClick();">合同审批 </a> </li> -->
                    <!--<li class="approdetial"><a  onClick="javascript:monitor_onClick();">审批详细 </a> </li>杜辉阳修改分配经办人审批流程 -->
                     <!--<li class="approdetial"><a  onClick="javascript:spinto_onClick();">审批详细 </a> </li> -->
                    <!--  
                    <li class="approdetial"><a  onClick="javascript:workflowaa_onClick();">分配经办人 </a> </li>
          杜辉阳修改分配经办人审批流程-->
                   <!--  <li class="approdetial"><a  onClick="javascript:workflowplay_onClick();">支付审批 </a> </li>
                    <li class="approdetial"><a  onClick="javascript:workflowbill_onClick();">修改账单 </a> </li>
                    <li class="approdetial"><a  onClick="javascript:workflowadjust_onClick();">欠款调整 </a> </li>
                    <li class="approdetial"><a  onClick="javascript:workflowkaipaio_onClick();">开票审核</a> </li>
                    <li class="approdetial"><a  onClick="javascript:workflowreportTaxation_onClick();">分配报税专员</a> </li> -->
                 <!--</ul>  -->
                 <input class="a" type="button"   value="<fmt:message key="view"/>"  <au:authorityBtn code="cont_view" type="1"/>  onClick="detail_onClick();">
				 <input class="c" id="addButton" type="button"   value="<fmt:message key="insert"/>"  <au:authorityBtn code="cont_add" type="1"/>  onClick="toAdd_onClick();">
				 <input class="b" type="button"   value="<fmt:message key="modify"/>"  <au:authorityBtn code="cont_modify" type="1"/>  onClick="findCheckbox_onClick();">
				 <input class="approdetial" type="button"   value="<fmt:message key="contract_examine"/>"  <au:authorityBtn code="cont_appr" type="1"/>  onClick="workflow_onClick();">              
				 <input class="approdetial" type="button"   value="<fmt:message key="examine_info"/>"  <au:authorityBtn code="cont_apprInfo" type="1"/>  onClick="spinto_onClick();">
				    <%if(flag.equals("true")){ %>
				    <input class="m_1" type="button"  value="<fmt:message key="goback_first"/>"    onClick="back_to();">
				    <input class="approdetial" type="button"   value="<fmt:message key="yes"/>"  onClick="tonotNeedRemind_onClick();">
                    <%} %> 
         </div>
           <div class="clear"></div>            
</div>
<div style="padding: 8 10 8 8;">
    <r:datacell 
        id="datacell1"
        queryAction="/FAERP/SalcontractAction.do?cmd=searchSalcontractData&f=${f}"
        width="98%"
        height="318px"
        xpath="SalcontractVo"
      ...
      <r:field fieldName="contract_type_bd" messagekey="contract_type" allowModify="false" width="100px">
      <d:select dictTypeId="CONTRACT_TYPE_BD" />
      </r:field>
      <r:field fieldName="contract_name" messagekey="contract_name" width="196px">
      </r:field>
      <r:field fieldName="main_part_bd" messagekey="contract_s" allowModify="false" width="100px">
      </r:field>
      <r:field fieldName="customer_name" messagekey="contract_jiafang" width="196px">
      </r:field>
      ...
      <r:field fieldName="create_user_id" messagekey="create_user_name" width="80px">
      </r:field>
      <r:field fieldName="contract_status_bd" messagekey="contract_status" allowModify="false" width="100px">
