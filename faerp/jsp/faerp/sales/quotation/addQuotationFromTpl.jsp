...
window.onload=function(){
	var contract_type_bd = <%=contract_type_bd %>;
	if(contract_type_bd!=""){
		$name("contract_type_bd").value = contract_type_bd;
	}else{
		contract_type_bd = $name("contract_type_bd").value;
	}
	if(contract_type_bd==null||contract_type_bd==""){//找不到合同
		alert("该合同类型未配置业务类型，请联系管理员！");
		return false;
	}
	jQuery.getJSON("<venus:base/>/QuotationAction.do?cmd=getTaxrate",{"contract_type_bd":$name("contract_type_bd").value},   function(json){
...
function insert_onClick() { // 插入单条数据
    if (checkAllForms()) {
        ...
        if(toAddFromOldSpQuotation == "true"){
	        ...
	        //form.find("[name=contract_type_bd]").val(<%=contract_type_bd%>);
...
function cancel_onClick() { // 取消后返回列表页面
    //window.location = "<%=request.getContextPath()%>/QuotationAction.do?cmd=queryAll&backFlag=true";
    window.close();
}
...
<form action="" id="insert_sal_form" method="post">
    ...
    <table width="100%" border="0" cellspacing="1" cellpadding="0"
        ...
        <tr>
            ...
            <input type="hidden" name="contract_type_bd" value="<%=contract_type_bd %>" />
            ...
            <input type="hidden" name="add_taxrate"  id="add_taxrate" value="" />
        ...
        <tr>
            ...
            <td class="td_2" class="td_2"><input type='checkbox' <%="1".equals(is_renewal_sign)?"checked":"" %>
        ...
        <tr>
                ...
                <td class="td_2"><textarea rows="3" class="textarea_limit_words" name="remark">${bean.remark }</textarea></td>
                ...
                "<venus:base/>/QuotationitemAction.do?cmd=addProduct&date="
                    ...
                    if (json.beans != null && json.beans != "") {
                        ...
                        for (var i = 0; i < json.beans.length; i++) {
                        		...
                        		var newEntity = $id("datacell_product").styleEntity.clone(true);// 只克隆行样式，不带值
...
function toSave_onClick_product() {
    ...
    /* var dc1 = $id("datacell_product");
    var action = "/FAERP/QuotationAction.do?cmd=saveQuotationitemList";
    //将两个datacell的xml内容合并
...
<script language="javascript">
	...
	$id("datacell_product").afterRefresh = function (){
		jQuery.getJSON("<venus:base/>/QuotationAction.do?cmd=getTaxrate",{"contract_type_bd":$name("contract_type_bd").value},   function(json){
			...
			$name("taxrateBu").value=json.taxrateBu;//增值税税率
			$name("add_taxrateBu").value=json.add_taxrateBu;
			//checkPrice('product_price','vat_amount','add_vat_amount','产品价格');
			checkSum();
