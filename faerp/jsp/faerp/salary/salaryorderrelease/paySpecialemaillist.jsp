...
function sendEmail_chked(num){
	...
	if(ids){
		showProgressBar();
		jQuery.post("<%=request.getContextPath()%>/SalaryorderreleaseAction.do?cmd=sendEmailPayroll&emp_ids="+ids+"&batchId="+batchId+"&clzId="+clzId+"&ChineseOrEnglish="+ChineseOrEnglish+"&Special=1",
		function(data){
           if(data>0){
           	hideProgressBar();
            ...
            $id("dc_emaillistid").reload();
           	...
           	hideProgressBar();
           	alert("<fmt:message key='alert_email_fail'/>");
           }
       });
	}
}
...
<div id="right">
    <div class="right_title_bg">
      <div class=" right_title"><fmt:message key="title_Specialemail"/></div></div>
  <div class="ringht_s">
    <!--查询 -->
  <div class="search">
  ...
  <!--按钮-->
    ...
    <div class="" id="values_div">
      <r:datacell id="dc_emaillistid"
            queryAction="/FAERP/SalaryorderreleaseAction.do?cmd=seachSalaryOrderReleaseEmail&special_flag=1"
