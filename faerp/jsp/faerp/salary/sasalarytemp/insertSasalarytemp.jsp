...
<%  //判断是否为修改页面
	...
	if(request.getParameter("isModify") != null) {  //如果从request获得参数"isModify"不为空
		...
		if(request.getAttribute(ISasalarytempConstants.REQUEST_BEAN) != null) {  //如果request中取出的bean不为空
  		    resultVo = (SasalarytempVo)request.getAttribute(ISasalarytempConstants.REQUEST_BEAN);  //从request中取出vo, 赋值给resultVo
  		}
	}
%>
<html>
...
<script language="javascript">
	function insert_onClick(){  //插入单条数据
	    if(checkAllForms()){
		form.action="<%=request.getContextPath()%>/SasalarytempAction.do?cmd=insert";
		form.submit();
		}
	}
  	...
  	function update_onClick(id){  //保存修改后的单条数据
	    ...
	    form.action="<%=request.getContextPath()%>/SasalarytempAction.do?cmd=update";
    	form.submit();
    	}
	}

    function cancel_onClick(){  //取消后返回列表页面
        form.action="<%=request.getContextPath()%>/SasalarytempAction.do?cmd=queryAll&backFlag=true";
        form.submit();
    }
</script>
</head>
...
<table  width="100%" height="40" border="0" cellpadding="0" cellspacing="1" bgcolor="#c5dbe2">
		...
		<tr>
			<td class="td_1" >sa_item_desc</td>
			<td class="td_2 ">
				<input type="text" class="text_field" name="sa_item_desc" inputName="sa_item_desc" value="" maxLength="75" />
			</td>
			<td class="td_2" ></td>
			<td class="td_2" ></td>
		</tr>
		...
		<tr>
			...
			<td class="td_2 ">
				<input type="text" class="text_field" name="is_display" inputName="is_display" value="" maxLength="64" />
			</td>
			<td class="td_2" ></td>
			<td class="td_2" ></td>
		</tr>
		...
		<tr>
			<td class="td_1" >data_field</td>
			<td class="td_2 ">
				<input type="text" class="text_field" name="data_field" inputName="data_field" value="" maxLength="64" />
			</td>
			<td class="td_2" ></td>
			<td class="td_2" ></td>
		</tr>
