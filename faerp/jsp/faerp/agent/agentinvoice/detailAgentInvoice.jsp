...
<%@ page import="rayoo.agent.agentinvoice.vo.AgentInvoiceVo" %>
<%@ page import="rayoo.agent.agentinvoice.util.IAgentInvoiceConstants" %>
<%  //取出本条记录
	AgentInvoiceVo resultVo = null;  //定义一个临时的vo变量
	resultVo = (AgentInvoiceVo)request.getAttribute(IAgentInvoiceConstants.REQUEST_BEAN);  //从request中取出vo, 赋值给resultVo
...
<script language="javascript">
    function back_onClick(){  //返回列表页面
        form.action="<%=request.getContextPath()%>/AgentInvoiceAction.do?cmd=queryAll&backFlag=true";
        form.submit();
    }
</script>
</head>
...
<div id="ccChild0" class="box_3"> 
	<table width="100%" height="40" border="0" cellpadding="0" cellspacing="1" bgcolor="#c5dbe2">
		...
		<tr>
			<td class="td_1" >agent_name：</td>
			<td class="td_2" ><%=RmStringHelper.prt(resultVo.getAgent_name())%></td>
		</tr>
		<tr>
		...
		<tr>
			<td class="td_1" >invoice_date：</td>
			<td class="td_2" ><%=RmStringHelper.prt(resultVo.getInvoice_date(), 10)%></td>
		</tr>
		<tr>
			<td class="td_1" >bill_date：</td>
			<td class="td_2" ><%=RmStringHelper.prt(resultVo.getBill_date(), 10)%></td>
		</tr>
		<tr>
			<td class="td_1" >agent_code：</td>
			<td class="td_2" ><%=RmStringHelper.prt(resultVo.getAgent_code())%></td>
		</tr>
		<tr>
