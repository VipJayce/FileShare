...
//    _$.ajax({
//            type : "post",
//            url : venusbase + "/AgentfinbillAction.do?cmd=getOldItemByAgentbillid4DataCell&agent_bill_id="+_$("#bid").val(),
//            data : _$("#datacell_formid").serialize(),
//            dataType : "html",
//            success : function(data) {
//                    _$("#bill_id_old").val(data);
//            }
//        });
	
	var queryAction = venusbase + "/AgentfinbillAction.do?cmd=getOldItemByAgentbillid4DataCell&agent_bill_id="+_$("#bid").val() ;
...
function approvsub() {
    ...
    if(!confirm(confirmmsg + '?')){
    	...
    	_$.ajax({
            type : "post",
            url : venusbase + "/AgentfinbillmodifyAction.do?cmd=approvalsub&taskuid=" +taskuid,
            data : _$("#datacell_formid").serialize(),
            dataType : "html",
            success : function(data) {
            	if (data == '1') {
            		alert("提交成功。");
            		window.location.href = venusbase + "/jsp/faerp/agent/agentfinbillmodify/listAgentfinbillmodify.jsp";
            	} else {
            		alert("提交失败。");
            	}
            }
