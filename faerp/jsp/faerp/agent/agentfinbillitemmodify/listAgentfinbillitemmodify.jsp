...
<%  //取出List
	...
	if(request.getAttribute(IAgentfinbillitemmodifyConstants.REQUEST_BEANS) != null) {  //如果request中的beans不为空
		lResult = (List)request.getAttribute(IAgentfinbillitemmodifyConstants.REQUEST_BEANS);  //赋值给resultList
	}
...
<script language="javascript">
	...
	function findCheckbox_onClick() {  //从多选框到修改页面
		...
		form.action="<%=request.getContextPath()%>/AgentfinbillitemmodifyAction.do?id=" + ids;
		form.cmd.value = "find";
		form.submit();
	}  
	function deleteMulti_onClick(){  //从多选框物理删除多条记录
		...
		if(confirm('<fmt:message key="sure_to_delete"/>')) {  //如果用户在确认对话框按"确定"
	    	form.action="<%=request.getContextPath()%>/AgentfinbillitemmodifyAction.do?ids=" + ids;
	...
	function toAdd_onClick() {  //到增加记录页面
		form.action="<%=request.getContextPath()%>/jsp/faerp/agent/agentfinbillitemmodify/insertAgentfinbillitemmodify.jsp";
		form.submit();
	}
...
<form name="form" method="post" action="<%=request.getContextPath()%>/AgentfinbillitemmodifyAction.do">
...
<tr><td>
	<table class="table_noFrame" width="100%">
		...
		<tr>
			<td align="right">agent_bill_modify_id</td>
			<td align="left">
				<input type="text" class="text_field" name="agent_bill_modify_id" inputName="agent_bill_modify_id" maxLength="9.5"/>
			</td>
			<td align="right"></td>
			<td align="left"></td>
		</tr>
		...
		<tr>
			<td align="right">agent_fin_bill_item_id</td>
			<td align="left">
				<input type="text" class="text_field" name="agent_fin_bill_item_id" inputName="agent_fin_bill_item_id" maxLength="9.5"/>
			</td>
			<td align="right"></td>
			<td align="left"></td>
		</tr>
...
<table class="table_div_content2">
	<tr>
		...
		<layout:collection name="beans" id="rmBean" styleClass="listCss" width="100%" indexId="orderNumber" align="center" sortAction="0">
			...
			<layout:collectionItem width="8%" title="agent_bill_modify_id" property="agent_bill_modify_id" sortable="true"/>
			...
			<layout:collectionItem width="8%" title="agent_fin_bill_item_id" property="agent_fin_bill_item_id" sortable="true"/>
...
<script language="javascript">
<%  //表单回写
	if(request.getAttribute(IAgentfinbillitemmodifyConstants.REQUEST_WRITE_BACK_FORM_VALUES) != null) {  //如果request中取出的表单回写bean不为空
		out.print(RmVoHelper.writeBackMapToForm((java.util.Map)request.getAttribute(IAgentfinbillitemmodifyConstants.REQUEST_WRITE_BACK_FORM_VALUES)));  //输出表单回写方法的脚本
	}
%>
</script>	
