...
<%  //判断是否为修改页面
	...
	if(request.getParameter("isModify") != null) {  //如果从request获得参数"isModify"不为空
		...
		if(request.getAttribute(IAgentcontactpersonConstants.REQUEST_BEAN) != null) {  //如果request中取出的bean不为空
  		    resultVo = (AgentcontactpersonVo)request.getAttribute(IAgentcontactpersonConstants.REQUEST_BEAN);  //从request中取出vo, 赋值给resultVo
  		}
	}
%>
<html>
...
<script language="javascript">
	function insert_onClick(){  //插入单条数据
	    if(checkAllForms()){
		form.action="<%=request.getContextPath()%>/AgentcontactpersonAction.do?cmd=insert";
		form.submit();
		}
	}
  	...
  	function update_onClick(id){  //保存修改后的单条数据
	    ...
	    form.action="<%=request.getContextPath()%>/AgentcontactpersonAction.do?cmd=update";
    	form.submit();
    	}
	}

    function cancel_onClick(){  //取消后返回列表页面
        form.action="<%=request.getContextPath()%>/AgentcontactpersonAction.do?cmd=queryAll&backFlag=true";
        form.submit();
    }
</script>
</head>
...
<table  width="100%" height="40" border="0" cellpadding="0" cellspacing="1" bgcolor="#c5dbe2">
		...
		<tr>
			<td class="td_1" >contact_name</td>
			<td class="td_2 ">
				<input type="text" class="text_field" name="contact_name" inputName="contact_name" value="" maxLength="50" />
			</td>
			<td class="td_2" ></td>
			<td class="td_2" ></td>
		</tr>
		<tr>
			<td class="td_1" >contact_tel</td>
			<td class="td_2 ">
				<input type="text" class="text_field" name="contact_tel" inputName="contact_tel" value="" maxLength="6.5" />
			</td>
			<td class="td_2" ></td>
			<td class="td_2" ></td>
		</tr>
		<tr>
			<td class="td_1" >contact_email</td>
			<td class="td_2 ">
				<input type="text" class="text_field" name="contact_email" inputName="contact_email" value="" maxLength="250" />
			</td>
			<td class="td_2" ></td>
			<td class="td_2" ></td>
		</tr>
		<tr>
			<td class="td_1" >contact_post</td>
			<td class="td_2 ">
				<input type="text" class="text_field" name="contact_post" inputName="contact_post" value="" maxLength="50" />
			</td>
			<td class="td_2" ></td>
			<td class="td_2" ></td>
		</tr>
		<tr>
			<td class="td_1" >备注</td>
			<td class="td_2 ">
				<input type="text" class="text_field" name="remark" inputName="备注" value="" maxLength="250" />
			</td>
			<td class="td_2" ></td>
			<td class="td_2" ></td>
		</tr>
