...
<script language="javascript">
    ...
    function detail_onClick(){  //实现转到详细页面
        ...
        var url="<%=request.getContextPath()%>/JobcandidateAction.do?cmd=detail&id=" + ids;
        url=url+'&_ts='+(new Date()).getTime();   //防止IE缓存，在每次打开时加个时间差的参数
        window.open(url,'','height=700px,width=1200px,top =10,left=100,help=no,resizable=yes,scrollbars=yes,location=no,status=not,title=候选人维护');
    }
...
<div id="right">
<script language="javascript">
if("<%=source%>"=="relevance_position"){
    writeTableTopFesco('招聘职位','<%=request.getContextPath()%>/');  //显示本页的页眉
}else{
    writeTableTopFesco('候选人管理','<%=request.getContextPath()%>/');  //显示本页的页眉
}
</script>
<div class="ringht_s">
...
<div id="ccParent1" class="button"> 
 <div class="button_right">
        <ul>
            <%
            if("relevance_position".equals(source)){
            %>
            <li class="g"><a onClick="javascript:ok();">确定</a></li>
            <%}else{ %>
            <li class="i"><a onClick="javascript:detail_onClick();">候选人维护</a></li>
            <%} %>
        </ul>
</div>
           <div class="clear"></div>
...
<div style="padding: 8 10 8 8;">
 <r:datacell id="datacell1" queryAction="/FAERP/Job_adviserAction.do?cmd=simpleQueryByAuthority" width="98%" height="318px" xpath="JobPositionVo" paramFormId="datacell_formid" >
