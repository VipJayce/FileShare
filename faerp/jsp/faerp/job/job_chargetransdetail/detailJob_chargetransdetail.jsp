<%@ page contentType="text/html; charset=UTF-8" %>
<%@ include file="/jsp/include/global.jsp" %>
<%@ page import="gap.rm.tools.helper.*" %>
<%@ page import="rayoo.job.job_chargetransdetail.vo.Job_chargetransdetailVo" %>
<%@ page import="rayoo.job.job_chargetransdetail.util.IJob_chargetransdetailConstants" %>
<%  //取出本条记录
	Job_chargetransdetailVo resultVo = null;  //定义一个临时的vo变量
	resultVo = (Job_chargetransdetailVo)request.getAttribute(IJob_chargetransdetailConstants.REQUEST_BEAN);  //从request中取出vo, 赋值给resultVo
	RmVoHelper.replaceToHtml(resultVo);  //把vo中的每个值过滤
%>
<html>
<fmt:bundle basename="gap.rm.rm_resource" prefix="gap.rm.">
<head>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
<title>codegen</title>
<script language="javascript">
    function back_onClick(){  //返回列表页面
        form.action="<%=request.getContextPath()%>/Job_chargetransdetailAction.do?cmd=queryAll&backFlag=true";
        form.submit();
    }
</script>
</head>
<body>
 
<form name="form" method="post">
 
	
       <div id="right">
		<div class="right_title_bg">
	 <div class=" right_title">	<fmt:message key="detail_page"/>
	 </div>
	 </div>
    	<div class="ringht_s">
 

<div id="ccParent0" class="box_3"> 
<table class="table_div_control">
	<tr> 
		<td>
			<img src="<%=request.getContextPath()%>/images/icon/07-0.gif" class="div_control_image" onClick="javascript:hideshow('ccChild0',this,'<%=request.getContextPath()%>/')"><fmt:message key="details"/>
		</td>
	</tr>
</table>
</div>

<div id="ccChild0" class="box_3"> 
	<table width="100%" height="40" border="0" cellpadding="0" cellspacing="1" bgcolor="#c5dbe2">
		<tr>
			<td class="td_1" >费用划转单主表：</td>
			<td class="td_2" ><%=RmStringHelper.prt(resultVo.getChargetransferbill_id())%></td>
		</tr>
		<tr>
			<td class="td_1" >职位ID：</td>
			<td class="td_2" ><%=RmStringHelper.prt(resultVo.getPosition_id())%></td>
		</tr>
		<tr>
			<td class="td_1" >客户ID：</td>
			<td class="td_2" ><%=RmStringHelper.prt(resultVo.getCust_id())%></td>
		</tr>
		<tr>
			<td class="td_1" >PM主键：</td>
			<td class="td_2" ><%=RmStringHelper.prt(resultVo.getPm_id())%></td>
		</tr>
		<tr>
			<td class="td_1" >pmteam：</td>
			<td class="td_2" ><%=RmStringHelper.prt(resultVo.getPmteam())%></td>
		</tr>
		<tr>
			<td class="td_1" >顾问ID：</td>
			<td class="td_2" ><%=RmStringHelper.prt(resultVo.getCounselor_id())%></td>
		</tr>
		<tr>
			<td class="td_1" >员工ID：</td>
			<td class="td_2" ><%=RmStringHelper.prt(resultVo.getEmployee_id())%></td>
		</tr>
		<tr>
			<td class="td_1" >外派开始日期：</td>
			<td class="td_2" ><%=RmStringHelper.prt(resultVo.getSendstartdate(), 10)%></td>
		</tr>
		<tr>
			<td class="td_1" >外派截止日期：</td>
			<td class="td_2" ><%=RmStringHelper.prt(resultVo.getSendenddate(), 10)%></td>
		</tr>
		<tr>
			<td class="td_1" >外派期限：</td>
			<td class="td_2" ><%=RmStringHelper.prt(resultVo.getSendperiod())%></td>
		</tr>
		<tr>
			<td class="td_1" >费用项目：</td>
			<td class="td_2" ><%=RmStringHelper.prt(resultVo.getExpenseitem())%></td>
		</tr>
		<tr>
			<td class="td_1" >费用金额：</td>
			<td class="td_2" ><%=RmStringHelper.prt(resultVo.getExpenseamount())%></td>
		</tr>
		<tr>
			<td class="td_1" >划入部门：</td>
			<td class="td_2" ><%=RmStringHelper.prt(resultVo.getCutindept_id())%></td>
		</tr>
		<tr>
			<td class="td_1" >是否显示：</td>
			<td class="td_2" ><%=RmStringHelper.prt(resultVo.getShowed())%></td>
		</tr>
		<tr>
			<td class="td_1" >显示序号：</td>
			<td class="td_2" ><%=RmStringHelper.prt(resultVo.getSerialno())%></td>
		</tr>
		<tr>
			<td class="td_1" >客服确认人：</td>
			<td class="td_2" ><%=RmStringHelper.prt(resultVo.getServiceconfirmor())%></td>
		</tr>
		<tr>
			<td class="td_1" >顾问确认人：</td>
			<td class="td_2" ><%=RmStringHelper.prt(resultVo.getCounselconfirmor())%></td>
		</tr>
		<tr>
			<td class="td_1" >财务确认人：</td>
			<td class="td_2" ><%=RmStringHelper.prt(resultVo.getFinanceconfirmor())%></td>
		</tr>
		<tr>
			<td class="td_1" >备注：</td>
			<td class="td_2" ><%=RmStringHelper.prt(resultVo.getDescription())%></td>
		</tr>
		<tr>
			<td class="td_1" >是否有效：</td>
			<td class="td_2" ><%=RmStringHelper.prt(resultVo.getIs_valid())%></td>
		</tr>
		<tr>
			<td class="td_1" >是否删除：</td>
			<td class="td_2" ><%=RmStringHelper.prt(resultVo.getIs_del())%></td>
		</tr>
		<tr>
			<td class="td_1" >分公司ID：</td>
			<td class="td_2" ><%=RmStringHelper.prt(resultVo.getCompany_id())%></td>
		</tr>
		<tr>
			<td class="td_1" >创建用户名称：</td>
			<td class="td_2" ><%=RmStringHelper.prt(resultVo.getCreate_user_name())%></td>
		</tr>
		<tr>
			<td class="td_1" >最后更改用户ID：</td>
			<td class="td_2" ><%=RmStringHelper.prt(resultVo.getLast_update_user_id())%></td>
		</tr>
		<tr>
			<td class="td_1" >最后更改用户名称：</td>
			<td class="td_2" ><%=RmStringHelper.prt(resultVo.getLast_update_user_name())%></td>
		</tr>
		<tr>
			<td class="td_1" >最后更改日期：</td>
			<td class="td_2" ><%=RmStringHelper.prt(resultVo.getLast_update_date(), 10)%></td>
		</tr>
		<tr>
			<td class="td_1" align="right"></td>
			<td class="td_2" align="left"></td>
		</tr>
	</table>
</div>


  	
<div class="foot_button">
<input type="button" class="foot_icon_1" value='<fmt:message key="go_back"/>'  onclick="javascript:back_onClick();" />
</div>


</div>
<input type="hidden" name="id" value="<%=RmStringHelper.prt(resultVo.getId())%>">

	 </div>

</form>
</fmt:bundle>
<script language="javascript">
	writeTableBottom('<%=request.getContextPath()%>/');  //显示本页的页脚
</script>
</body>
</html>
