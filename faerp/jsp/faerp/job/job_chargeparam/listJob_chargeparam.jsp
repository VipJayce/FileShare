...
<%  //取出List
	...
	if(request.getAttribute(IJob_chargeparamConstants.REQUEST_BEANS) != null) {  //如果request中的beans不为空
		lResult = (List)request.getAttribute(IJob_chargeparamConstants.REQUEST_BEANS);  //赋值给resultList
	}
...
<script language="javascript">
	...
	function findCheckbox_onClick() {  //从多选框到修改页面
		...
		form.action="<%=request.getContextPath()%>/Job_chargeparamAction.do?id=" + ids;
		form.cmd.value = "find";
		form.submit();
	}  
	function deleteMulti_onClick(){  //从多选框物理删除多条记录
		...
		if(confirm('<fmt:message key="sure_to_delete"/>')) {  //如果用户在确认对话框按"确定"
	    	form.action="<%=request.getContextPath()%>/Job_chargeparamAction.do?ids=" + ids;
	...
	function toAdd_onClick() {  //到增加记录页面
		form.action="<%=request.getContextPath()%>/jsp/faerp/job/job_chargeparam/insertJob_chargeparam.jsp";
		form.submit();
	}
...
<form name="form" method="post" action="<%=request.getContextPath()%>/Job_chargeparamAction.do">
...
<tr><td>
	<table class="table_noFrame" width="100%">
		<tr>
			...
			<td align="left">
				<input type="text" class="text_field" name="paramname" inputName="参数名称" maxLength="50"/>
			</td>
			<td align="right"></td>
			<td align="left"></td>
		</tr>
		<tr>
			...
			<td align="left">
				<input type="text" class="text_field" name="paramcode" inputName="参数代码" maxLength="50"/>
			</td>
			<td align="right"></td>
			<td align="left"></td>
		</tr>
		...
		<tr>
			...
			<td align="left">
				<input type="text" class="text_field" name="datatype" inputName="数据类型" maxLength="50"/>
			</td>
			<td align="right"></td>
			<td align="left"></td>
		</tr>
		<tr>
			...
			<td align="left">
				<input type="text" class="text_field" name="defaultvalue" inputName="默认值" maxLength="50"/>
			</td>
			<td align="right"></td>
			<td align="left"></td>
		</tr>
		<tr>
			<td align="right">参数序号</td>
			<td align="left">
				<input type="text" class="text_field_half" name="paramnumber_from" inputName="参数序号" value="" columnSize="" decimalDigits="0" />&nbsp;到&nbsp;<input type="text" class="text_field_half" name="paramnumber_to" inputName="参数序号" value="" columnSize="" decimalDigits="0" />
			</td>
			<td align="right"></td>
			<td align="left"></td>
		</tr>
...
<table class="table_div_content2">
	<tr>
		...
		<layout:collection name="beans" id="rmBean" styleClass="listCss" width="100%" indexId="orderNumber" align="center" sortAction="0">
			...
			<layout:collectionItem width="8%" title="参数名称" property="paramname" sortable="true"/>
			
			<layout:collectionItem width="8%" title="参数代码" property="paramcode" sortable="true"/>
			...
			<layout:collectionItem width="8%" title="数据类型" property="datatype" sortable="true"/>
			
			<layout:collectionItem width="8%" title="默认值" property="defaultvalue" sortable="true"/>
			
			<layout:collectionItem width="8%" title="参数序号" property="paramnumber" sortable="true"/>
...
<script language="javascript">
<%  //表单回写
	if(request.getAttribute(IJob_chargeparamConstants.REQUEST_WRITE_BACK_FORM_VALUES) != null) {  //如果request中取出的表单回写bean不为空
		out.print(RmVoHelper.writeBackMapToForm((java.util.Map)request.getAttribute(IJob_chargeparamConstants.REQUEST_WRITE_BACK_FORM_VALUES)));  //输出表单回写方法的脚本
	}
%>
</script>	
