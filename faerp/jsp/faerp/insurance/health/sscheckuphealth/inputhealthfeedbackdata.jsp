...
<body>
<form name="form" method="post" action="<%=request.getContextPath()%>/SscheckuphealthAction.do?cmd=inputFeedbackHealth" onsubmit="return insert_submit() ;">
<div id="right">
  ...
  <tr>
    ...
    <td class="td_2" width="50%">
    <a href="#" onclick="toDownLoad_onclick()">点击下载模板</a>
    </td>
  </tr>
  </table>
<div class="xz_button">
...
<script type="text/javascript"><!--
   ...
   function insert_submit(){
    ...
    function toDownLoad_onclick(){
          window.location="<%=request.getContextPath()%>/SscheckuphealthAction.do?cmd=downLoad&inputtype=0" ;
    }
    //消息
    if (${msg != null } && ${msg != "" }) {
        alert('${msg}');
    ...
    function toinput(){
         if(getConfirm()) {  //如果用户在确认对话框中点"取消"
                ...
                if(attach_address == null){
                    ...
                    var url="<%=request.getContextPath()%>/SscheckuphealthAction.do?cmd=inputFeedbackHealth&attach_address="+encodeURI(attach_address);
                      jQuery.ajax({
                       url: url,
                      type: 'post',
                      dataType: 'html',
