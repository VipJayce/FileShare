...
<%  //判断是否为修改页面
	...
	if(request.getParameter("isModify") != null) {  //如果从request获得参数"isModify"不为空
		...
		if(request.getAttribute(IU8_userConstants.REQUEST_BEAN) != null) {  //如果request中取出的bean不为空
...
<script language="javascript">
	function insert_onClick(){  //插入单条数据
		form.action="<%=request.getContextPath()%>/U8_userAction.do?cmd=insert";
		form.submit();
	}
  	...
  	function update_onClick(id){  //保存修改后的单条数据
	    ...
	    form.action="<%=request.getContextPath()%>/U8_userAction.do?cmd=update";
    	form.submit();
	}

    function cancel_onClick(){  //取消后返回列表页面
        form.action="<%=request.getContextPath()%>/U8_userAction.do?cmd=queryAll&backFlag=true";
        form.submit();
    }
</script>
</head>
...
<tr><td> 
	<table class="table_div_content_inner">
		<tr>
			...
			<td align="left">
				<input type="text" class="text_field" name="cuse_name" inputName="cuse_name" value="" maxLength="25" />
			</td>
			<td align="right"></td>
			<td align="left"></td>
		</tr>
		<tr>
			...
			<td align="left">
				<input type="text" class="text_field" name="cdept" inputName="cdept" value="" maxLength="25" />
			</td>
			<td align="right"></td>
			<td align="left"></td>
		</tr>
