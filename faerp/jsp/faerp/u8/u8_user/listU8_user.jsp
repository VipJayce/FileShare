...
<%  //取出List
	...
	if(request.getAttribute(IU8_userConstants.REQUEST_BEANS) != null) {  //如果request中的beans不为空
		lResult = (List)request.getAttribute(IU8_userConstants.REQUEST_BEANS);  //赋值给resultList
	}
...
<script language="javascript">
	...
	function findCheckbox_onClick() {  //从多选框到修改页面
		...
		form.action="<%=request.getContextPath()%>/U8_userAction.do?id=" + ids;
		form.cmd.value = "find";
		form.submit();
	}  
	function deleteMulti_onClick(){  //从多选框物理删除多条记录
		...
		if(confirm('<fmt:message key="sure_to_delete"/>')) {  //如果用户在确认对话框按"确定"
	    	form.action="<%=request.getContextPath()%>/U8_userAction.do?ids=" + ids;
	...
	function toAdd_onClick() {  //到增加记录页面
		form.action="<%=request.getContextPath()%>/jsp/faerp/u8/u8_user/insertU8_user.jsp";
		form.submit();
	}
...
<form name="form" method="post" action="<%=request.getContextPath()%>/U8_userAction.do">
...
<tr><td>
	<table class="table_noFrame" width="100%">
		<tr>
			<td align="right">cuse_name</td>
			<td align="left">
				<input type="text" class="text_field" name="cuse_name" inputName="cuse_name" maxLength="25"/>
			</td>
			<td align="right"></td>
			<td align="left"></td>
		</tr>
		<tr>
			<td align="right">cdept</td>
			<td align="left">
				<input type="text" class="text_field" name="cdept" inputName="cdept" maxLength="25"/>
			</td>
			<td align="right"></td>
			<td align="left"></td>
		</tr>
...
<table class="table_div_content2">
	<tr>
		...
		<layout:collection name="beans" id="rmBean" styleClass="listCss" width="100%" indexId="orderNumber" align="center" sortAction="0">
			...
			<layout:collectionItem width="8%" title="cuse_name" property="cuse_name" sortable="true"/>
			
			<layout:collectionItem width="8%" title="cdept" property="cdept" sortable="true"/>
...
<script language="javascript">
<%  //表单回写
	if(request.getAttribute(IU8_userConstants.REQUEST_WRITE_BACK_FORM_VALUES) != null) {  //如果request中取出的表单回写bean不为空
		out.print(RmVoHelper.writeBackMapToForm((java.util.Map)request.getAttribute(IU8_userConstants.REQUEST_WRITE_BACK_FORM_VALUES)));  //输出表单回写方法的脚本
	}
%>
</script>	
