...
<%  //取出List
	...
	if(request.getAttribute(IErp_userConstants.REQUEST_BEANS) != null) {  //如果request中的beans不为空
		lResult = (List)request.getAttribute(IErp_userConstants.REQUEST_BEANS);  //赋值给resultList
	}
...
<script language="javascript">
	...
	function findCheckbox_onClick() {  //从多选框到修改页面
		...
		form.action="<%=request.getContextPath()%>/Erp_userAction.do?id=" + ids;
		form.cmd.value = "find";
		form.submit();
	}  
	function deleteMulti_onClick(){  //从多选框物理删除多条记录
		...
		if(confirm('<fmt:message key="sure_to_delete"/>')) {  //如果用户在确认对话框按"确定"
	    	form.action="<%=request.getContextPath()%>/Erp_userAction.do?ids=" + ids;
	...
	function toAdd_onClick() {  //到增加记录页面
		form.action="<%=request.getContextPath()%>/jsp/faerp/u8/erp_user/insertErp_user.jsp";
		form.submit();
	}
...
<form name="form" method="post" action="<%=request.getContextPath()%>/Erp_userAction.do">
...
<script language="javascript">
<%  //表单回写
	if(request.getAttribute(IErp_userConstants.REQUEST_WRITE_BACK_FORM_VALUES) != null) {  //如果request中取出的表单回写bean不为空
		out.print(RmVoHelper.writeBackMapToForm((java.util.Map)request.getAttribute(IErp_userConstants.REQUEST_WRITE_BACK_FORM_VALUES)));  //输出表单回写方法的脚本
	}
%>
</script>	
