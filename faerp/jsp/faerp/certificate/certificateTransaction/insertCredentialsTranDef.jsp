 ...
 //办理证件类别、办理证件小类联动
   function findType(){
    ...
    function rtnFuncCompany(arg){
        ...
        jQuery("#companyNumber").text(arg[2]);
        return false;
    }
    //公共校验
 ...
 %>
	...
	<table width="100%" height="40" border="0" cellpadding="0"
		...
		<tr>
			...
			<td class="td_2 " width="25%"><c:choose>
				<c:when test="<%=isModify%>">
					<w:lookup name="companyId" allowInput="false"
						onReturnFunc="rtnFuncCompany" id="company_id"
						lookupUrl="/CertificateTransactionAction.do?cmd=getCustomer"
						messagekey="choose_company" height="440" width="600"
						style="width:170px" />
				</c:when>
				<c:otherwise>
                            ${bean.companyName}
			...
			<td class="td_1" width="25%"><span class="style_required_red">*</span><fmt:message key='companyNumber' /></td>
			<td class="td_2 " width="25%" id="companyNumber"><c:if
				test="<%=!isModify%>">
                     ${bean.companyNumber}
                </c:if></td>
		</tr>
		<tr>
			...
			<td class="td_1" width="25%"><span class="style_required_red">*</span><fmt:message key='credentials_name' /></td>
			<td class="td_2 " width="25%"><c:choose>
				<c:when test="<%=isModify%>">
		...
		<tr>
			<td class="td_1" width="25%"><span class="style_required_red">*</span><fmt:message key='companyAccount' /></td>
			<td class="td_2 " width="25%"><input type="text"
				class="text_field" name="companyAccount" maxLength="50"
				id="companyAccount" value="${bean.companyAccount}" /></td>
			<td class="td_1" width="25%"><span class="style_required_red">*</span><fmt:message
				key='companyAccountPassword' /></td>
			<td class="td_2 " width="25%"><input type="text"
				class="text_field" name="companyAccountPassword" maxLength="50"
				id="companyAccountPassword" value="${bean.companyAccountPassword}" />
			</td>
		</tr>
		<tr>
			<td class="td_1" width="25%"><fmt:message key='acceptance' /></td>
			<td class="td_2 " width="25%"><%=gap.rm.tools.helper.RmJspHelper.getSelectField("acceptance", -1, "ACCEPTANCE", acceptance, " id='acceptance'", false)%>
			</td>
			<td class="td_1" width="25%"><fmt:message key='companyLinkman' /></td>
			<td class="td_2 " width="25%"><input type="text"
				class="text_field" name="companyLinkman" maxLength="50"
				id="companyLinkman" value="${bean.companyLinkman}" /></td>
		</tr>
	</table>
	</div>
	<div></div>
