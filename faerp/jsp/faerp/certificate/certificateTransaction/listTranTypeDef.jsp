...
//办理证件类别、办理证件小类联动
   function findType(){
	...
	function findCheckbox_onClick() {  //从多选框到修改页面
        ...
        var url="<%=request.getContextPath()%>/CertificateTransactionAction.do?cmd=findTranTypeDef&id=" + ids;
         jQuery.ajax({
                             url:"<%=request.getContextPath()%>/CertificateTransactionAction.do?cmd=vailDataTranTypeDef1&id="+ids,
                             type: 'POST',
                             dataType: 'html',
                             async: false,
                             cache:false,
	...
	function deleteMulti_onClick(){  //从多选框物理删除多条记录
		...
		if(confirm('<fmt:message key="sure_to_delete"/>')) {  //如果用户在确认对话框按"确定"
	    	...
	    	form.cmd.value = "deleteMultiTranTypeDef";
	    	form.submit();
		}
	}
	   function restoreMulti_onClick(){  //从多选框物理删除多条记录
        ...
        if(confirm('<fmt:message key="sure_to_restore"/>')) {  //如果用户在确认对话框按"确定"
            form.action="<%=request.getContextPath()%>/CertificateTransactionAction.do?ids=" + ids;
            form.cmd.value = "restoreMultiTranTypeDef";
            form.submit();
        }
    }
	...
	function toAdd_onClick() {  //到增加记录页面
        var url="<%=request.getContextPath()%>/jsp/faerp/certificate/certificateTransaction/insertTranTypeDef.jsp";
        window.open(url,'','height=275px,width=820px,top =100,left=200,help=no,resizable=yes,scrollbars=yes,location=no,status=not,title=证件办理材料新增');
	}
	function detail_onClick(){  //实现转到详细页面
		...
		var url="<%=request.getContextPath()%>/CertificateTransactionAction.do?cmd=detailTranTypeDef&id=" + ids;
        window.open(url,'',"height=275px,width=820px,top =100,left=200,help=no,resizable=yes,scrollbars=yes,location=no,status=not,title=<fmt:message key='credentialsType_maintenance'/>");
	}
//-------------------------------
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
		...
		<tr>
		 <td align="right"><fmt:message key='isChargeAudit'/></td>
            <td align="left">
                 <h:select name="isChargeAudit" property="isChargeAudit">
                <h:option value="" label="请选择"></h:option>
                <h:option value="0" label="否"></h:option>
                <h:option value="1" label="是"></h:option>
           </h:select>
            </td>
             <td align="right"><fmt:message key='isVerifySuccess'/></td>
            <td align="left">
              <h:select name="isVerifySuccess" property="isVerifySuccess">
               <h:option value="" label="请选择"></h:option>
                <h:option value="0" label="否"></h:option>
                <h:option value="1" label="是"></h:option>
           </h:select>
		...
		<tr>           
           <td align="right"><fmt:message key='afterTranGet'/></td>
          <td align="left" colspan="3">
            <input type="text"     class="text_field" name="afterTranGet" maxLength="50"  id="afterTranGet" value="" />
           </td>
...
<div id="ccParent1" class="button"> 
 <div class="button_right">
					<input class="a" type="button"  onClick="javascript:detail_onClick();" value="<fmt:message key="view"/>"<au:authorityBtn code="view_tranTypeDef" type="1"/>>
					<input class="c" type="button" onClick="javascript:toAdd_onClick();" value="<fmt:message key="insert"/>"<au:authorityBtn code="insert_tranTypeDef" type="1"/>>
					<input class="b" type="button"  onClick="javascript:findCheckbox_onClick();" value="<fmt:message key="modify"/>"<au:authorityBtn code="modify_tranTypeDef" type="1"/>>
			        <input class="d" type="button"  onClick="javascript:deleteMulti_onClick();" value="<fmt:message key="delete"/>"<au:authorityBtn code="delete_tranTypeDef" type="1"/>>
			        <input class="g" type="button"  onClick="javascript:restoreMulti_onClick();" value="<fmt:message key="restore"/>"<au:authorityBtn code="restore_tranTypeDef" type="1"/>>
		 </div>
           <div class="clear"></div>			
</div>
...
<div style="padding: 8 10 8 8;">
    <r:datacell id="datacell1" queryAction="/FAERP/CertificateTransactionAction.do?cmd=simpleQueryTranTypeDef"  width="98%" height="303px" xpath="CertificateTranTypeDefVO" paramFormId="datacell_formid" readonly="true">
        <r:toolbar location="bottom" tools="nav,pagesize,info"/>
        <r:field fieldName="id" messagekey="Operation" width="50px" onRefreshFunc="setCheckboxStatus" align="center" ></r:field>
        <r:field fieldName="credentialsTypeBdID" messagekey="tranTypeBD" width="200px"  allowModify="false">
          <d:select  dictTypeId="CREDENTIALS_TYPE" />    
        </r:field>
        <r:field fieldName="credentialsTypeName" messagekey="tranType" width="200px"></r:field>
        <r:field fieldName="isChargeAudit" messagekey="isChargeAudit" width="100px" allowModify="false">
            <d:select  dictTypeId="TrueOrFalse"  />    
        </r:field>
        <r:field fieldName="isVerifySuccess" messagekey="isVerifySuccess" width="100px" allowModify="false">
        <d:select  dictTypeId="TrueOrFalse"/>    
        </r:field>
        <r:field fieldName="afterTranGet" messagekey="afterTranGet" width="200px"></r:field>
