...
<%  
    ...
    String quotation_id = (String)request.getAttribute("quotation_id");
...
<script language="javascript">
    ...
    function toSecondStep(type){  //前往下一步
        if(checkAllCont()){
            var dc1 = $id("grdCityEnsureTbody");
            var action = "<%=request.getContextPath()%>/EmppostAction.do?cmd=queryPostAll";
            ...
            if (form) {
             ...
             if( returnNode ) {
                 if( myAjax.getValue("root/data/returnValue") == 1 ) {
                      if(type=='1'){//变更所有
                        window.location.href="<%=request.getContextPath()%>/jsp/faerp/employeepost/empentry/emppost/initUpdateAll2.jsp";
                      }
                      if(type=='3'){//只变更社保公积金
                        window.location.href="<%=request.getContextPath()%>/jsp/faerp/employeepost/empentry/emppost/initUpdateShebao2.jsp";
                      }
    ...
    function DelSite() {  
       ...
       if(confirm("<fmt:message key="sure_to_delete_for_accu_product"/>")) {  //如果用户在确认对话框按"确定"
            ...
            //当某个组的产品全部被删除了，则删除该组
            var group_ids = document.getElementsByName("select_group_id");//所有的组
            var security_group_ids = null;//需要删除的组
            for(var i=0;i<group_ids.length;i++){
                var isexits = '1';
                var group_id = group_ids[i].value;
                for(var j=0; j<dataset.getLength(); j++){
                    var datasettr = dataset.get(j);
                    var security_group_id= datasettr.getProperty("group_id");
                    if(group_id==security_group_id){//列表中有这个组的产品
                        isexits = '0';
                        break;
                    }
                }
                if(isexits =='1'){//没有产品了就需要删除该组
                    if(security_group_ids == null) {
                        security_group_ids = new Array(0);
                    }
            ...
            //正式删除组
            if(security_group_ids != null) { 
                for(var i=0;i<security_group_ids.length;i++){
                    var tbody = jQuery("#selectgroups");
                    jQuery(tbody).find("div[id='"+security_group_ids[i]+"']").remove();
                }
            }
...
<div id="right">
    <div class="right_title_bg">
        <div class=" right_title"><fmt:message key="entry_change"/></div>
    </div>
</div>
<div>
    ...
    <div id="values_div">
      <r:datacell 
                         ...
                         queryAction="/FAERP/EmppostAction.do?cmd=initUpdateOneSecurity"
                         submitAction="/FAERP/EmppostAction.do?cmd=queryPostAll"
                         width="100%"
                         xpath="SecurityAllVo"
    ...
    <div class="foot_button">
          <input type="button" class="foot_icon_1" value="<fmt:message key="next_step"/>" onclick="toSecondStep('<%=type %>');" />
