...
<%
String txType = (String)request.getParameter("txType");
...
<script language="javascript">
    if("${txType}" == "1"){
        writeTableTopFesco("Welcome call未完成提醒",'<%=request.getContextPath()%>/');  //显示本页的页眉
    }else if("${txType}" == "2"){
        writeTableTopFesco("回访电话提醒",'<%=request.getContextPath()%>/');  //显示本页的页眉
    }else if("${txType}" == "3"){
        writeTableTopFesco("福利办理无操作提醒",'<%=request.getContextPath()%>/');  //显示本页的页眉
    }else if("${txType}" == "4"){
        writeTableTopFesco("福利未完成提醒",'<%=request.getContextPath()%>/');  //显示本页的页眉
    }else if("${txType}" == "5"){
        writeTableTopFesco("公积金未停缴提醒",'<%=request.getContextPath()%>/');  //显示本页的页眉
    }else if("${txType}" == "6"){
        writeTableTopFesco("未办理退工提醒",'<%=request.getContextPath()%>/');  //显示本页的页眉
    }
...
<!--<div id="ccParent1" class="button"> 
 ...
 <div style="padding: 8 10 8 8;">
    <r:datacell 
        id="datacell1"
        queryAction="/FAERP/EmppostAction.do?cmd=listEmpPostTx&txType=${txType}"
        width="98%"
        height="320px"
        xpath="EmppostVo"
        ...
        width="35px" sortAt="none" onRefreshFunc="setCheckBoxStatus"  align="center"></r:field>-->
          ...
          <r:field fieldName="emp_code" messagekey="emp_unique_num">
           ...
           <c:if test="${txType == '2'}">
	           <r:field fieldName="call_time" messagekey="call_time">
	               <h:text/>   
	           </r:field>
           </c:if>
           ...
           <r:field fieldName="on_post_date" label="报入职时间">
               <w:date format="YYYY-MM-DD" disabled="true"/>
           </r:field>
           <r:field fieldName="apply_on_post_date" label="入职时间">
               <w:date format="YYYY-MM-DD" disabled="true"/>
           </r:field>
           <r:field fieldName="off_post_date_str" label="报离职时间">
              <w:date format="YYYY-MM-DD" disabled="true"/>
           </r:field>
           <r:field fieldName="apply_off_post_date" label="离职时间">
               <w:date format="YYYY-MM-DD" disabled="true"/>
           </r:field>
