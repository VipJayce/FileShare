...
<%  
    ...
    //报价单
    String quo_id = (String)request.getAttribute("quo_id");
    //员工ID  
    String emp_ids = (String)request.getAttribute("emp_ids");
    //员工入离职ID ----------------------------------- 多个
    String emppostIds = (String)request.getAttribute("emppostIds");
    //客户
    String customer_id = (String)request.getAttribute("customer_id");
    //接受单位
    String cust_receive_id = (String)request.getAttribute("cust_receive_id");
    //委派单ID
    String send_id = (String)request.getAttribute("send_id");
%>
<html>
...
<script language="javascript">
    ...
    function back_onClick(){  //返回列表页面
        form.action="<%=request.getContextPath()%>/EmppostAction.do?cmd=queryzhuanyiList1&backFlag=true";
        form.submit();
    }
    ...
    function toSecondStep(){  //前往下一步
        if(checkAllCont()){
            ...
            var action = "<%=request.getContextPath()%>/EmppostAction.do?cmd=queryPostBatForZhuanYi";
            //将两个datacell的xml内容合并
            ...
            if (form) {
             ...
             if( returnNode ) {
                 if( myAjax.getValue("root/data/returnValue") == 1 ) {
                      window.location.href="<%=request.getContextPath()%>/jsp/faerp/employeepost/empentry/emppost/initInsertbat2ForZhuanYi.jsp?isLimitUpperAndLower=${isLimitUpperAndLower}";
...
<div>
    ...
    <input id="quo_id" name="quo_id" type="hidden" value="<%=quo_id%>"/>
    <input id="emp_ids" name="emp_ids" type="hidden" value="<%=emp_ids%>"/>
    <input id="emppostIds" name="emppostIds" type="hidden" value="<%=emppostIds%>"/>
    <input id="customer_id" name="customer_id" type="hidden" value="<%=customer_id%>"/>
    <input id="cust_receive_id" name="cust_receive_id" type="hidden" value="<%=cust_receive_id%>"/>
    <input id="send_id" name="send_id" type="hidden" value="<%=send_id%>"/>
    <!--社保公积金-->
    <div class="zbox">
    ...
    <div id="values_div">
      <r:datacell 
                         ...
                         queryAction="/FAERP/EmppostAction.do?cmd=initInsertOneSecurityForZhuanYi"
