...
<%  //判断是否为修改页面
	...
	if(request.getParameter("isModify") != null) {  //如果从request获得参数"isModify"不为空
		...
		if(request.getAttribute(IEmpsiblingConstants.REQUEST_BEAN) != null) {  //如果request中取出的bean不为空
...
<script language="javascript">
	function insert_onClick(){  //插入单条数据
	    if(checkAllForms()){
		form.action="<%=request.getContextPath()%>/EmpsiblingAction.do?cmd=insert";
		form.submit();
		}
	}
  	...
  	function update_onClick(id){  //保存修改后的单条数据
	    ...
	    form.action="<%=request.getContextPath()%>/EmpsiblingAction.do?cmd=update";
    	form.submit();
    	}
	}
...
<table  width="100%" height="40" border="0" cellpadding="0" cellspacing="1" bgcolor="#c5dbe2">
		...
		<tr>
			...
			<td class="td_2 ">
				<input type="text" class="text_field" name="sib_name" inputName="亲属姓名" value="" maxLength="50" />
			</td>
			<td class="td_2" ></td>
			<td class="td_2" ></td>
		</tr>
		...
		<tr>
			...
			<td class="td_2 ">
				<input type="text" class="text_field" name="sib_id_card_num" inputName="证件号码" value="" maxLength="64" />
			</td>
			<td class="td_2" ></td>
			<td class="td_2" ></td>
		</tr>
		...
		<tr>
			...
			<td class="td_2">
				<input type="text" class="text_field_reference_readonly" name="sib_birthday" id="sib_birthday" inputName="出生日期" /><img class="refButtonClass"	src="<venus:base/>/images/icon/reference.gif" onClick="javascript:getYearMonthDay('sib_birthday','<venus:base/>/');"/>
			</td>
			<td  class="td_1" ></td>
			<td class="td_2" ></td>
		</tr>
		<tr>
			...
			<td colspan="3" class="td_2" >
				<textarea class="textarea_limit_words" cols="60" rows="5" name="sib_desc" inputName="备注" maxLength="1000" ></textarea>
			</td>
		</tr>
		<tr>
			<td class="td_1"></td>
