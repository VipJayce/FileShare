...
<table width="100%" cellspacing="0">
	...
	<tr>
        <td width="20%">员工选择递交资料的统计表格(按客户) </td>
		<td align="right">年：</td>
 ...
 if (window.navigator.appName == "Microsoft Internet Explorer")
{
    ...
    var MyHighchartsPie = new MyHighcharts({
        ...
        url:"/FAERP/AnalysisAction.do?cmd=getDjclPieAnalysis"     //ajax URL
    ...
    $(document).ready(function() {
       ...
       $("#pie_year_idList_search_input").bind("propertychange", function() { 
		...
		//table-jqgrid
		jQuery("#list2").jqGrid(
		{ url:'/FAERP/AnalysisAction.do?cmd=getDjclTableAnalysis', 
		datatype: "json", 
		colNames:['客户code','客户名称','上门', '上门占比', '快递','快递占比','onstie','onstie占比','不需要提交','不需要提交占比','总人数','year','month'], 
		colModel:[
		{name:'customerCode',index:'customerCode', width:80,sortable:false},
		 ...
		 {name:'visit',index:'visit', width:50,  align:"right",formatter: visitLink,sortable:false}, 
		 {name:'visitPercent',index:'visitPercent',  align:"right",width:50,sortable:false}, 
		 {name:'express',index:'express', width:50, align:"right",formatter: expressLink,sortable:false}, 
		 {name:'expressPercent',index:'expressPercent', width:50, align:"right",sortable:false}, 
		 {name:'onsite',index:'onsite', width:50, align:"right",formatter: onsiteLink,sortable:false}, 
		 {name:'onsitePercent',index:'onsitePercent', width:50, align:"right",sortable:false}, 
		 {name:'noNeed',index:'noNeed', width:50, align:"right",formatter: noNeedLink,sortable:false}, 
		 {name:'noNeedPercent',index:'noNeedPercent', width:60, align:"right",sortable:false}, 
        ...
        function visitLink(cellvalue, options, rowObject) {
             return "<a target='_blank' href =' /FAERP/AnalysisAction.do?cmd=getDjclShowDetail&type=1&month="+ rowObject['month']+"&year="+ rowObject['year']+"&customerCode="+ rowObject['customerCode']+"'>" + cellvalue +"</a>";
         }
        function onsiteLink(cellvalue, options, rowObject) {
             return "<a target='_blank' href =' /FAERP/AnalysisAction.do?cmd=getDjclShowDetail&type=2&month="+ rowObject['month']+"&year="+ rowObject['year']+"&customerCode="+ rowObject['customerCode']+"'>" + cellvalue +"</a>";
         }
        function expressLink(cellvalue, options, rowObject) {
             return "<a target='_blank' href =' /FAERP/AnalysisAction.do?cmd=getDjclShowDetail&type=0&month="+ rowObject['month']+"&year="+ rowObject['year']+"&customerCode="+ rowObject['customerCode']+"'>" + cellvalue +"</a>";
         }
        function noNeedLink(cellvalue, options, rowObject) {
             return "<a target='_blank' href =' /FAERP/AnalysisAction.do?cmd=getDjclShowDetail&type=3&month="+ rowObject['month']+"&year="+ rowObject['year']+"&customerCode="+ rowObject['customerCode']+"'>" + cellvalue +"</a>";
         }
