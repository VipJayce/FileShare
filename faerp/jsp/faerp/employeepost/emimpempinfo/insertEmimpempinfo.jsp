 ...
 function insert_onClick(){  //插入单条数据
    ...
    if(checkAllForms()){
        ...
        if(filetype==4){
            ...
            file1=file1.substring(file1.lastIndexOf(".")+1);
            if(file1!="xls"){
            	alert('批量转移仅支持格式为2003的EXCEL');
                return ;
            }
        	//alert('批量导入上传成功后后台将统一调度处理任务，请勿点击页面按钮！处理完成后的结果将以邮件方式通知！');  HYL
        }
        form.action="<%=request.getContextPath()%>/EmimpempinfoAction.do?cmd=insert&filetype="+filetype;
        form.submit();
    }
}
    ...
    function cancel_onClick(){  //取消后返回列表页面
        form.action="<%=request.getContextPath()%>/EmimpempinfoAction.do?cmd=queryAll1&backFlag=true";
        form.submit();
    }
</script>
</head>
...
<table  width="100%" height="40" border="0" cellpadding="0" cellspacing="1" bgcolor="#c5dbe2">
        <tr>
            ...
            <td class="td_2 ">
                <input type="radio" name="filetype" value="1" checked> 上海
                <input type="radio" name="filetype" value="2" > 全国
                <input type="radio" name="filetype" value="4" > 批量转移
            </td>
        </tr>
        <tr>
