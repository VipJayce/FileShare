...
<script language="javascript">
    ...
    function findCheckbox_onClick() {  //从多选框到修改页面
        ...
        for(var i =0 ;i<id_check.length;i++){
            ...
            if(constatus==0||constatus==2){
                var url="<%=request.getContextPath()%>/Finout_contract_productAction.do?cmd=find_ibdcontract&id=" + ids+"&detail=modify";
    ...
    function toAdd_onClick() {  //到增加记录页面
        form.action="<%=request.getContextPath()%>/Finout_contract_productAction.do?cmd=add_ibdsalcontract&_ts="+(new Date()).getTime();
        form.submit();
    }
    ...
    function detail_onClick(){  //实现转到详细页面
        ...
        var url="<%=request.getContextPath()%>/Finout_contract_productAction.do?cmd=detail_ibdcontract&id=" + ids+"&detail=detail";
        url=url+'&_ts='+(new Date()).getTime();   //防止IE缓存，在每次打开时加个时间差的参数
    ...
    function workflow_onClick(){  //发起工作流
     ...
     if(constatus!="0"&&constatus!="2"){
        ...
        WfCommonAjax.checkCurrentWf(wfname,contract_id,function(reData){
                if(reData=='0'){
                    //没有进行发起过审批
               form.action="<%=request.getContextPath()%>/SalcontractAction.do?contract_id=" + ids+"&detail=detail&service_company=no_company";
    ...
    function spinto_onClick(){
        ...
        WfCommonAjax.checkCurrentWf_Info(wfname,wfname1,contract_id,function(reData){
                ...
                //alert('测试审批详细');
                var url = "/FAERP/jsp/faerp/sales/salcontract/processdetailpage.jsp?contract_id=" + contract_id+"&wfname="+wfname+"&wfname1="+wfname+"&_ts="+(new Date()).getTime();
...
<div id="right">
<script language="javascript">
    writeTableTopFesco('合同管理','<%=request.getContextPath()%>/');  //显示本页的页眉
</script>
<div class="ringht_s">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
     <tr>
        ...
        <tr>
            ...
            <td align="left">
                    <d:select dictTypeId="IBD_CONTRACT_TYPE_BD" name="contract_type_bd" id="contract_type_bd"/>
            </td>
             <td align="right">合同状态</td>
            <td align="left">
                 <d:select dictTypeId="CONTRACT_STATUS_BD" id="contract_status_bd" name="contract_status_bd" style="width:183px" nullLabel="--请选择--" filterOp="in" filterStr="0,1,2,4,5,7" /> 
            </td>
...
<div id="ccParent1" class="button"> 
 <div class="button_right">
				 ...
				 <input class="approdetial" type="button"   value="修改"   onClick="findCheckbox_onClick();">
				 <input class="approdetial" type="button"   value="提交审批"  <au:authorityBtn code="cont_appr" type="1"/>  onClick="workflow_onClick();">              
                 <input class="approdetial" type="button"   value="审批详细"  <au:authorityBtn code="cont_apprInfo" type="1"/>  onClick="spinto_onClick();">
         </div>
           <div class="clear"></div>            
</div>
<div style="padding: 8 10 8 8;">
    <r:datacell 
      ...
      <d:select dictTypeId="IBD_BUSINESS_TYPE_BD" />
      </r:field>
