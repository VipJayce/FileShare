...
<script language="javascript">
    ...
    if(create_type_bd==1){
        var contract_template_id=document.form.contract_template_id.value;
         ...
         form.action="<%=request.getContextPath()%>/jsp/faerp/ibd/ibdsalcontract/ibdsaltab_list.jsp?create_type_bd="+create_type_bd+"&contract_type_bd=8&business_type_bd="+business_type_bd+"&contract_template_id="+contract_template_id+"&agent_b_id=<%=agent_b_id%>"+"&agent_b_name=<%=agent_b_name%>"+"&detail="+"<%=detail%>";
    }else{
        form.action="<%=request.getContextPath()%>/jsp/faerp/ibd/ibdsalcontract/ibdsaltab_list.jsp?create_type_bd="+create_type_bd+"&contract_type_bd=8&business_type_bd="+business_type_bd+"&detail="+"<%=detail%>"+"&agent_b_id=<%=agent_b_id%>"+"&agent_b_name=<%=agent_b_name%>";
        }
        form.submit();
    }

    function cancel_onClick(){  //取消后返回列表页面
       form.action="<%=request.getContextPath()%>/Finout_contract_productAction.do?cmd=queryAll_tolistIbdsal";
       form.submit();
    }
...
<input  type="hidden" id="contract_type_bd" name="contract_type_bd"  value="8"><!-- IBD合同类型 -->
<table  width="100%" height="40" border="0" cellpadding="0" cellspacing="1" bgcolor="#c5dbe2">
        ...
        <!--  
            ...
            -->
            <td class="td_1" ><span class="style_required_red">* </span>IBD业务类型</td>
            <td class="td_2" >
            <d:select dictTypeId="IBD_BUSINESS_TYPE_BD" name="business_type_bd" id="business_type_bd" nullLabel="--请选择--"   onchange="javascript:getContract_template(this,'contract_template','code','name','${bean.contract_type_bd}')"/>
            </td>
...
function getContract_template(selectObject,targetId,key,value,selectvalue){
        var contract_type_bd = "8";//IBD合同类型
