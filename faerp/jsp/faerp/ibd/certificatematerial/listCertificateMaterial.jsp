...
<%  //取出List
	...
	if(request.getAttribute(ICertificateMaterialConstants.REQUEST_BEANS) != null) {  //如果request中的beans不为空
		lResult = (List)request.getAttribute(ICertificateMaterialConstants.REQUEST_BEANS);  //赋值给resultList
	}
...
<script language="javascript">
	...
	function findCheckbox_onClick() {  //从多选框到修改页面
		...
		form.action="<%=request.getContextPath()%>/CertificateMaterialAction.do?id=" + ids;
		form.cmd.value = "find";
		form.submit();
	}  
	function deleteMulti_onClick(){  //从多选框物理删除多条记录
		...
		if(confirm('<fmt:message key="sure_to_delete"/>')) {  //如果用户在确认对话框按"确定"
	    	form.action="<%=request.getContextPath()%>/CertificateMaterialAction.do?ids=" + ids;
	...
	function toAdd_onClick() {  //到增加记录页面
		form.action="<%=request.getContextPath()%>/jsp/faerp/ibd/certificatematerial/insertCertificateMaterial.jsp";
		form.submit();
	}
...
<form name="form" method="post" action="<%=request.getContextPath()%>/CertificateMaterialAction.do">
...
<tr><td>
	<table class="table_noFrame" width="100%">
		<tr>
			<td align="right">员工证件信息表ID</td>
			<td align="left">
				<input type="text" class="text_field" name="credentials_id" inputName="员工证件信息表ID" maxLength="9.5"/>
			</td>
			<td align="right"></td>
			<td align="left"></td>
		</tr>
		<tr>
			<td align="right">材料ID</td>
			<td align="left">
				<input type="text" class="text_field" name="credentials_materia_id" inputName="材料ID" maxLength="9.5"/>
			</td>
			<td align="right"></td>
			<td align="left"></td>
		</tr>
		<tr>
			...
			<td align="left">
				<input type="text" class="text_field_half_reference_readonly" name="receive_time_from" id="receive_time_from" inputName="接收时间"/><img class="refButtonClass" src="<venus:base/>/images/icon/reference.gif" onClick="javascript:getYearMonthDay('receive_time_from','<venus:base/>/');"/>&nbsp;到&nbsp;<input type="text" class="text_field_half_reference_readonly" name="receive_time_to" id="receive_time_to" inputName="接收时间"/><img class="refButtonClass" src="<venus:base/>/images/icon/reference.gif" onClick="javascript:getYearMonthDay('receive_time_to','<venus:base/>/');"/>
			</td>
			<td align="right"></td>
			<td align="left"></td>
		</tr>
		<tr>
			...
			<td align="left">
				<input type="text" class="text_field" name="status" inputName="状态" maxLength="2.5"/>
			</td>
			<td align="right"></td>
			<td align="left"></td>
		</tr>
...
<table class="table_div_content2">
	<tr>
		...
		<layout:collection name="beans" id="rmBean" styleClass="listCss" width="100%" indexId="orderNumber" align="center" sortAction="0">
			...
			<layout:collectionItem width="8%" title="员工证件信息表ID" property="credentials_id" sortable="true"/>
			
			<layout:collectionItem width="8%" title="材料ID" property="credentials_materia_id" sortable="true"/>
			
			<layout:collectionItem width="8%" title="接收时间" property="receive_time" sortable="true">
				<bean:define id="rmValue" name="rmBean" property="receive_time"/>
				<%=gap.rm.tools.helper.RmStringHelper.prt(rmValue, 10)%>
			</layout:collectionItem>
			
			<layout:collectionItem width="8%" title="状态" property="status" sortable="true"/>
...
<script language="javascript">
<%  //表单回写
	if(request.getAttribute(ICertificateMaterialConstants.REQUEST_WRITE_BACK_FORM_VALUES) != null) {  //如果request中取出的表单回写bean不为空
		out.print(RmVoHelper.writeBackMapToForm((java.util.Map)request.getAttribute(ICertificateMaterialConstants.REQUEST_WRITE_BACK_FORM_VALUES)));  //输出表单回写方法的脚本
	}
%>
</script>	
