...
<script language="javascript">
	...
	function findCheckbox_onClick() {  //从多选框到修改页面
		...
		var url="<%=request.getContextPath()%>/Ibd_contract_serviceAction.do?cmd=find_tempIbdcontract&id=" + ids;
	...
	function deleteMulti_onClick(){  //从多选框物理删除多条记录
	    ...
	    jQuery.ajax({
        url: "<%=request.getContextPath()%>/Ibd_contract_serviceAction.do?cmd=deleteIbdTempMulti&ids="+ids+"&date="+new Date(),
        type: 'GET',
        dataType: 'html',
	...
	function toAdd_onClick() {  //到增加记录页面
	    var url="<%=request.getContextPath()%>/jsp/faerp/ibd/ibdsalcontracttemp/insertIbdTempContract.jsp";
        url=url+'?_ts='+(new Date()).getTime();   //防止IE缓存，在每次打开时加个时间差的参数
	...
	function detail_onClick(){  //实现转到详细页面
		...
		var url="<%=request.getContextPath()%>/Ibd_contract_serviceAction.do?cmd=detail_tempIbdcontract&id=" + ids;
        url=url+'&_ts='+(new Date()).getTime();   //防止IE缓存，在每次打开时加个时间差的参数
...
<div id="right">
<script language="javascript">
	writeTableTopFesco('IBD合同模板','<%=request.getContextPath()%>/');  //显示本页的页眉
</script>
<div class="ringht_s">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	<tr>
		...
		<td align="right">合同业务类型</td>
            <td align="left">
                    <d:select dictTypeId="IBD_BUSINESS_TYPE_BD" name="business_type_bd" id="business_type_bd" nullLabel="--请选择--"/>
            </td>
		 <td>
...
<div style="padding: 8 10 8 8;">
    <r:datacell 
        id="datacell1"
        queryAction="/FAERP/Ibd_contract_serviceAction.do?cmd=searchIbdContractTempData"
        width="98%"
        height="318px"
        xpath="ContractVo"
      ...
      <r:field fieldName="business_type_bd" label="合同业务类型" width="170px" allowModify="false">
            <d:select dictTypeId="IBD_BUSINESS_TYPE_BD"></d:select>
        </r:field>
    </r:datacell>
    </div>
</div>
