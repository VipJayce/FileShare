...
<script>
	function addChild_onClick() { //弹出新增窗口
		...
		var obj = window.showModalDialog('<venus:base/>/jsp/complexTemplate/tableFrame.jsp?tableId=B&empId=' + empId,'','dialogTop:' + (window.screen.height / 3) + 'px;dialogLeft:' + (window.screen.width / 3) + 'px;dialogHeight:300px;dialogWidth:490px;help:no;resizable:no;scroll:no;status:0;');
	...
	function deleteMulti_onClick(){  //从多选框物理删除多条记录
		...
		if(confirm('<fmt:message key="SureToDelete"/>')) {
	    	form.action="<venus:base/>/ChildTableBAction.do?cmd=delete&empId=" + empId + "&id=" + id;
    		form.submit();
		}
	}
	...
	function modifyChild_onClick() { //弹出修改窗口
		...
		var obj = window.showModalDialog('<venus:base/>/jsp/complexTemplate/tableFrame.jsp?tableId=B&isModify=true&empId=' + empId + '&id=' + id,'','dialogTop:' + (window.screen.height / 3) + 'px;dialogLeft:' + (window.screen.width / 3) + 'px;dialogHeight:300px;dialogWidth:490px;help:no;resizable:no;scroll:no;status:0;');
		if(obj != null || obj != undefined) {
			refresh();
		}
	}
...
<form name="form" method="post" action="<venus:base/>/ChildTableBAction.do?empId=<%=empId%>">
...
<table class="table_div_content">
	<tr>
		...
		<layout:collection name="demo" id="demo1" styleClass="listCss" width="100%" indexId="orderNumber" align="center" sortAction="0" >
			...
			<layout:collectionItem width="10%" title='<%=LocaleHolder.getMessage("gap.template.complex.Company") %>'  property="empCompany" sortable="true"/>
			<layout:collectionItem width="10%" title='<%=LocaleHolder.getMessage("gap.template.complex.Post") %>'  property="empProfession" sortable="true"/>		
			<layout:collectionItem width="10%" title='<%=LocaleHolder.getMessage("gap.template.complex.WorkTime") %>'  property="jobPeriod" sortable="true"/>
			<layout:collectionItem width="10%" title='<%=LocaleHolder.getMessage("gap.template.complex.StaffProfile") %>'  property="empIntroduce"/>	
		</layout:collection>
