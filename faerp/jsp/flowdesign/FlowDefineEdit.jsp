...
<script language="javascript">
    writeTableTop('<fmt:message key="modify" bundle="${applicationResources}"/>','<%=request.getContextPath()%>/');
</script>
...
<table class="table_div_content">
    <tr> 
        ...
        <fmt:message key="modify_process_definition"/></td>
    </tr>
</table>
</div>
...
<form name="form" method="post">
    ...
    <input type="hidden" name="WF_ID" value="<%=request.getParameter("id")%>">
    <input type="hidden" name="WF_TIME" value="<%=vo.getFileInfo().getWF_TIME()%>">
    <!--马啸注释<input type="hidden" name="WF_SECTION" value="<%=typeId%>">-->
    <input type="hidden" name="WF_PACKAGE" value="<%=wf_PACKAGE%>">
    <input type="hidden" name="WF_STATE" value="<%=vo.getFileInfo().getWF_STATE()%>">
    
    <tr>
        <td nowrap class="th_1"><fmt:message key="process_code"/></td>
        <td><%=request.getParameter("id")%></td>
        <td nowrap class="th_1"><fmt:message key="process_state"/></td>
  ...
  <tr nowrap> 
    ...
    <td>
    <input class="text_field" type="text" value="<%=vo.getFileInfo().getWF_NAME()%>" name="WF_NAME" value="" size="25" validators="isNormalStrOnWeb;notNull" maxlength="50">
    </td>
  ...
  <tr nowrap>
    ...
    <!--马啸添加start-->
      <tr nowrap>
        ...
        <td>
            <input type="hidden" name="WF_SECTION" value="<%=typeId %>" validators="notNull"/>
   ...
   <!--马啸添加end-->
    <tr nowrap>
        <td colspan="4">
            <input type="button" name="Submit0" value='<fmt:message key="save_process_as_draft"/>' class="button_ellipse" onClickto="toSaveFlow('designEditSubmit.do')">
            <input type="button" name="Submit1" value='<fmt:message key="save_and_inspection_process"/>' class="button_ellipse" onClickto="toSaveAndPublish('designEditSubmit.do')">
