...
function nextElement(field) {
  ...
  var e=0;
  var next;
  if(form.elements==undefined || form.elements=="")return;
  for(; e <form.elements.length; e++)
    if(field == form.elements[e])
      break;
  //找下一个输入域
...
function FindSelectObject(name)
{
  ...
  for (i=0;i<selectObjs.length;i++)
    ...
    { //  alert("i:"+i+selectObjs[i].name+ "标签名称"+name);
    	return selectObjs[i];
    }
...
{
  var obj;
 // alert('_GetSelectOnDiv name:'+name);
...
{
//	alert('ShowSelect'+selobj.toString()+"curSelectObj"+curSelectObj);
  if (curSelectObj!=null)
    HideSelect(curSelectObj);
  var obj = _GetSelectOnDiv(selobj.idOn);
//  alert('获取的对象'+obj.style);
