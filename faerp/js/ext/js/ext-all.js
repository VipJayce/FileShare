...
Date.formatCodes = {
	...
	c : function() {
		for (var E = "Y-m-dTH:i:sP", C = [], B = 0, A = E.length; B < A; ++B) {
			var D = E.charAt(B);
			C.push(D == "T" ? "'T'" : Date.getFormatCode(D))
		}
		return C.join(" + ")
	},
...
Date.getFormatCode = function(B) {
	var A = Date.formatCodes[B];
	if (A) {
		A = Ext.type(A) == "function" ? A() : A;
		Date.formatCodes[B] = A
	}
	return A || ("'" + String.escape(B) + "'")
};
...
Date.parseCodes = {
	...
	D : function() {
		for (var A = [], B = 0; B < 7; A.push(Date.getShortDayName(B)), ++B) {
		}
		return {
			g : 0,
			c : null,
	...
	M : function() {
		for (var A = [], B = 0; B < 12; A.push(Date.getShortMonthName(B)), ++B) {
		}
		return Ext.applyIf({
			s : "(" + A.join("|") + ")"
		}, Date.parseCodes["F"])
	...
	c : function() {
		var E = Date.formatCodeToRegex, C = [];
		var A = [E("Y", 1), E("m", 2), E("d", 3), E("h", 4), E("i", 5),
				E("s", 6), {
					c : "if(results[7] == 'Z'){\no = 0;\n}else{\n"
							+ E("P", 7).c + "\n}"
				}];
...
Date.formatCodeToRegex = function(B, A) {
	var C = Date.parseCodes[B];
	if (C) {
		C = Ext.type(C) == "function" ? C() : C;
		Date.parseCodes[B] = C;
		if (C.c) {
			C.c = String.format(C.c, A)
		}
	}
	return C || {
		g : 0,
...
Date.prototype.getGMTOffset = function(A) {
	return (this.getTimezoneOffset() > 0 ? "-" : "+")
			+ String.leftPad(Math
					.abs(Math.floor(this.getTimezoneOffset() / 60)), 2, "0")
			+ (A ? ":" : "")
...
Ext.Panel = Ext.extend(Ext.Container, {
	...
	addTool : function() {
		...
		for (var E = 0, C = arguments, B = C.length; E < B; E++) {
			...
			var D = this.toolTemplate.insertFirst((A.align !== "left")
	...
	onCollapse : function(A, B) {
		if (A) {
			this[this.collapseEl].slideOut(this.slideAnchor, Ext.apply(this
					.createEffect(B || true, this.afterCollapse, this),
	...
	onExpand : function(A, B) {
		if (A) {
			this[this.collapseEl].slideIn(this.slideAnchor, Ext.apply(this
					.createEffect(B || true, this.afterExpand, this),
					this.expandDefaults))
		} else {
...
Ext.DataView = Ext.extend(Ext.BoxComponent, {
	...
	initComponent : function() {
		...
		this.addEvents("beforeclick", "click", "containerclick", "dblclick",
...
Ext.DatePicker = Ext.extend(Ext.Component, {
	...
	onRender : function(A, F) {
		...
		var I = new Ext.KeyNav(this.eventEl, {
			"left" : function(J) {
				J.ctrlKey ? this.showPrevMonth() : this.update(this.activeDate
						.add("d", -1))
			},
			"right" : function(J) {
				J.ctrlKey ? this.showNextMonth() : this.update(this.activeDate
						.add("d", 1))
			},
			"up" : function(J) {
				J.ctrlKey ? this.showNextYear() : this.update(this.activeDate
						.add("d", -7))
			},
			"down" : function(J) {
				J.ctrlKey ? this.showPrevYear() : this.update(this.activeDate
						.add("d", 7))
			},
			"pageUp" : function(J) {
				this.showNextMonth()
	...
	onMonthClick : function(D, B) {
		...
		if (C.is("button.x-date-mp-cancel")) {
			...
			if (C.is("button.x-date-mp-ok")) {
				var E = new Date(this.mpSelYear, this.mpSelMonth,
						(this.activeDate || this.value).getDate());
				if (E.getMonth() != this.mpSelMonth) {
					E = new Date(this.mpSelYear, this.mpSelMonth, 1)
							.getLastDateOfMonth()
				}
				this.update(E);
				this.hideMonthPicker()
...
if (Ext.dd.DropZone) {
	...
	Ext.extend(Ext.tree.TreeDropZone, Ext.dd.DropZone, {
		...
		onNodeDrop : function(C, I, E, D) {
			...
			var G = D.node
					|| (I.getTreeNode ? I.getTreeNode(D, F, H, E) : null);
			var B = {
				tree : this.tree,
				target : F,
				data : D,
...
Ext.extend(Ext.menu.Item, Ext.menu.BaseItem, {
	...
	onRender : function(B, A) {
		...
		C.innerHTML = String.format(
				...
				this.icon || Ext.BLANK_IMAGE_URL, this.itemText || this.text,
				this.iconCls || "");
		this.el = C;
...
Ext.form.ComboBox = Ext.extend(Ext.form.TriggerField, {
	...
	restrictHeight : function() {
		...
		var D = Math.max(A, F, this.minHeight || 0) - this.list.shadowOffset
				- E - 5;
			...
			B.add("-", {
				...
				menu : new Ext.menu.ColorMenu({
					...
					selectHandler : function(F, E) {
						this.execCmd("forecolor", Ext.isSafari || Ext.isIE
								? "#" + E
								: E);
						this.deferFocus()
					},
					...
					selectHandler : function(F, E) {
						if (Ext.isGecko) {
							...
							this.execCmd(Ext.isOpera
									? "hilitecolor"
									: "backcolor", Ext.isSafari || Ext.isIE
									? "#" + E
									: E);
							this.deferFocus()
						}
		...
		if (C == E.TAB) {
			...
			if (C == E.ENTER) {
				...
				if (this.moveEditorOnEnter !== false) {
					if (A) {
						G = D.walkCells(B.row - 1, B.col, -1, this.acceptsNav,
								this)
					} else {
						G = D.walkCells(B.row + 1, B.col, 1, this.acceptsNav,
								this)
...
Ext.Slider = Ext.extend(Ext.BoxComponent, {
	...
	initComponent : function() {
		...
		this
				.addEvents("beforechange", "change", "dragstart", "drag",
						"dragend");
		if (this.vertical) {
	...
	onClickChange : function(A) {
		if (A.top > this.clickRange[0] && A.top < this.clickRange[1]) {
			this.setValue(Math.round(A.left / this.getRatio()))
		}
	},
	...
	translateValue : function(A) {
		return (A * this.getRatio()) - this.halfThumb
	},
	...
	onDrag : function(A) {
		...
		this.setValue(Math.round(B.left / this.getRatio()), false);
...
Ext.Slider.Vertical = {
	...
	onClickChange : function(B) {
		if (B.left > this.clickRange[0] && B.left < this.clickRange[1]) {
			...
			this.setValue(Math.round(A / this.getRatio()))
		}
	}
};
Ext.StatusBar = Ext.extend(Ext.Toolbar, {
	...
	setStatus : function(C) {
		...
		if (C.clear) {
			var D = C.clear, B = this.autoClear, A = {
				useDefaults : true,
				anim : true
			};
			if (typeof D == "object") {
