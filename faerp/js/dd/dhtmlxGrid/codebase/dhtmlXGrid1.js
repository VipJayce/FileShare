...
/************************************************************************
*定义dhtmlxArray数组，对Array进行扩展 begin
*************************************************************************/
...
/************************************************************************
*定义dhtmlxArray数组，对Array进行扩展 end
*************************************************************************/
...
function dhtmlXGridObject(id) {
	...
	this.row = null;
	this.editor = null;
	this._f2kE = true;//是否允许F2键
	this._dclE = true;//是否允许双击
	...
	//表头（支持双层表头）文本 xHdr分为上下两行，第一行保存this.hdr，第二行默认为空行
	...
	this.loadXMLString = function (url, afterCall) {
		if (this._dload) {
			this._dload = url;
			this._askRealRows(null, afterCall);
			return true;
	...
	this.init = function (fl) {
		...
		for (var i = 0; i < this.hdrLabels.length; i++) {
			...
			//alert(dwr.util.toDescriptiveString(hdrRow._childIndexes,3));
	...
	this._doClick = function (ev) {
		...
		if (this.selMultiRows != false) {
			if ((ev.shiftKey || ev.metaKey) && this.row != null) {
				selMethod = 1;
			}
			if (ev.ctrlKey) {
				selMethod = 2;
	...
	this.scrollPage = function (dir) {
		...
		if (this._dload && (!this.rowsCol[new_ind])) {
			this._askRealRows(new_ind, function () {
				try {
					self.selectCell(new_ind, (this.cell ? this.cell._cellIndex : 0), true);
				}
				catch (e) {
				}
			});
	...
	this.addRowsFromBuffer = function (stopBeforeServerCall) {
		if (this.rowsBuffer[0].length == 0) {
			if (!this.recordsNoMore && !stopBeforeServerCall) {
				if (this.limit && this.rowsCol.length >= this.limit) {
					return false;
				}
	...
	this.getColumnLabel = function (cin) {
		return this.getHeaderCol(cin);
	};
	...
	this.cells = function (row_id, col) {
		...
		if (!c && !window.eXcell_math) {
			dhtmlxError.throwError("cell", "Row not exists", [row_id, col]);
		}
	...
	this.setHeaderCol = function (c, label, ind) {
		...
		//alert(dwr.util.toDescriptiveString(z._childIndexes,3));
	...
	this.sortField = function (ind, repeatFl, r_el) {
		...
		if (this._dload && !this.callEvent("onBeforeSorting", [ind, this])) {
			return true;
		}
	...
	if (!this._dload) {
		if ((ind > this.rowsCol.length && ind < (this.rowsCol.length + this.rowsBuffer[0].length)) || (typeof ind == "undefined" && this.rowsBuffer[0].length)) {
			if (typeof ind == "undefined") {
				var inBufInd = this.rowsBuffer[0].length;
			} else {
...
dhtmlXGridObject.prototype.parseXML = function (xml, startIndex) {
	...
	if (this._innerParse(rowsCol, startIndex, tree, pid) == -1) {
		return (this._ahgr ? this.setSizes() : "");
	}
	if (zpid) {
	...
	this.callEvent("onXLE", [this, rowsCol.length, pid]);
};
...
dhtmlXGridObject.prototype._fillRowFromXML = function (r, xml, tree, pId) {
	...
	for (var j = 0; j < cellsCol.length; j++) {
		...
		if (css1) {
			r.childNodes[j].style.cssText += css1;
		}
...
dhtmlXGridObject.prototype.dhx_Event = function () {
	...
	this.eventCatcher = function (obj) {
		...
		//alert(dwr.util.toDescriptiveString(dhx_catch,3));
		var m_obj = obj;
		...
		z.addEvent = function (ev) {
			...
			if (ev) {
				//alert(dwr.util.toDescriptiveString(dhx_catch.push(ev),3));
...
dhtmlXGridObject.prototype.setData=function(data)
{
	...
	{
		r=this.addRow(data[i][0],data[i],1);
	}
}
