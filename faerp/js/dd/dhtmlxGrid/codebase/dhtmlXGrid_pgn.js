...
dhtmlXGridObject.prototype.enablePagingWT = function (fl, pageSize, pagesInGrp, parentObjId) {
	...
	this.setOnPageChanged(function (page, startRowInd, lastRowInd) {
		...
		if (this._WTDef[1]) {
			...
			iButton.setText("当前" + (this.currentPage) + "页/共" + (self.totalPages)+"页 第"+(startRowInd + 1)+"-"+(lastRowInd + 1)+"条/共"+(this.getRowsNum())+"条");
		}
	});
	//
	...
	if (!parentObjId.setOnClickHandler) {
		this.aToolBar = new dhtmlXToolbarObject(parentObjId, "100%", 22, "");
	} else {
	...
	this.aToolBar.setBarAlign("right");
	if (this._WTDef[1]) {
		this.aToolBar.addItem(new dhtmlXImageTextButtonObject(this.imgURL + "results.gif", "分页信息", 220, 18, 0, "results"));
	}
	if (this._WTDef[2]) {
		this.aToolBar.addItem(new dhtmlXSelectButtonObject("pages", "null", "null", f1, 120, 18, "toolbar_select"));
	}
	if (this._WTDef[3]) {
		this.aToolBar.addItem(new dhtmlXSelectButtonObject("perpagenum", "1,10,20,50,100,200,500", "1,10,20,50,100,200,500", f1, 60, 80, "toolbar_select"));
	}
	if (this._WTDef[0]) {
		this.aToolBar.addItem(new dhtmlXImageButtonObject(this.imgURL + "icon_page_frist.gif", 20, 18, f1, "leftabs", "首页"));
		this.aToolBar.addItem(new dhtmlXImageButtonObject(this.imgURL + "icon_page_prev.gif", 20, 18, f1, "left", "上一页"));
		this.aToolBar.addItem(new dhtmlXImageButtonObject(this.imgURL + "icon_page_next.gif", 20, 18, f1, "right","下一页"));
		this.aToolBar.addItem(new dhtmlXImageButtonObject(this.imgURL + "icon_page_last.gif", 20, 18, f1,"rightabs", "末页"));
	}
