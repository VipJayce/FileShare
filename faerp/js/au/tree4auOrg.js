...
WebFXTree.prototype.toString = function () {
	...
	str += "<td valign=\"bottom\" nowrap>&nbsp;<a href=\"####\" id=\"" + this.id + "-anchor\" onclick=\"folder_Click();\" onfocus=\"webFXTreeHandler.focus(this);\">" + this.text + "</a></td>";
	...
	for(var i=0; i<this.selNum; i++) {
		str += "<input type=\"Checkbox\" name=\"selectAll\" value=\""+this.selVal[i]+"\" onclick=\"selAll(this);\">"+this.selTxt[i];
	}
	str +="</td></tr></table></div>";
...
function WebFXTreeItem(sText,sTitle,treeID,selCode,selTxt,selVal,selWhoEx,selWho,isDisable,sType,isFolder) {
	this._subItems  = [];
	this._wasLast   = false;
	this.text       = sText || defaultText;
...
WebFXTreeItem.prototype.focus = function () {
	...
	//document.getElementById(this.id + '-anchor').style.border = '1px dotted threedshadow';
	document.getElementById(this.id + '-anchor').style.color = '#E77B39';
	//OrgAuthorize 20080925 add link 20081088disabled
...
WebFXTreeItem.prototype.toString = function (nItem,nItemCount) {
	...
	if (this.folder) {
		...
		str += "&nbsp;<a href=\"#\" ";
		str += "onclick=\"treeitem_Click('"+this.action + "','"+this.treeID+"');\"";
		str += " id=\"" + this.id + "-anchor\" onfocus=\"webFXTreeHandler.focus(this);\" title=\""+this.title+"\">" + this.text + "</a></td>";
		str += "<td width=\"150\" nowrap valign=\"bottom\" align=\"left\">&nbsp;"+gap.authority.Inherited_permissions_;
		//继承的授权情况
		if(this.selWhoEx[0]=="" && this.selWhoEx[1]=="") {
				str += gap.authority.No;
		}else {
			for(var i=0; i<this.selNum; i++) {
		...
		for(var i=0; i<this.selNum; i++) {
			...
			str += "<input type=\"Checkbox\" name=\""+this.treeID+"\" value=\""+this.selVal[i]+"\" code=\""+this.selCode+"\" sType=\""+this.sType+"\" "+disableFlag+" onclick=\"seltree(this);\"";
...
function folder_Click(){
	alert(gap.authority.Please_select_the_leaf_node_operation_);
}
