...
function writeTableTopFesco(pageTitle, path) {
	if(pageTitle == undefined) {
	pageTitle="示范页面";
	}
	
			document.write("<div class='right_title_bg'>");
		document.write( "<div class='right_title'>"+pageTitle+"</div></div>");	
...
function getYearMonthDay(textDate,path){
	...
	var event = window.event?window.event:getYearMonthDay.caller.arguments[0];
...
function writeTableBottomFesco(path) {
	if(path == undefined) {
		path = "../../";
	}
	jQueryload();
