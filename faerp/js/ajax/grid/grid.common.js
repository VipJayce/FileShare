...
function createModal(aIDs, content, p, insertSelector, posSelector, appendsel) {
	...
	if(p.display){
    	jQuery(mw).css({top: p.top+"px",left: p.left+"px",width: p.width+"px",height: p.height+"px", zIndex:p.zIndex,visibility:p.display});
    }
    else
...
function createEl(eltype,options,vl,elm) {
   	...
   	switch (eltype)
   	{
        ...
        case "text" :
            ...
            elem.value = vl;
			if(!options.size && elm) {
                jQuery(elem).css("width",$(elm).width()-4);
			}
            jQuery(elem).attr(options);
            break;
        case "password" :
            ...
            elem.type = "password";
            elem.value = vl;
			if(!options.size) jQuery(elem).css("width",$(elm).width()-4)
            jQuery(elem).attr(options);
            break;
		case "image" :
			elem = document.createElement("input");
