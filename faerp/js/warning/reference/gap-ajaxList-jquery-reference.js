...
function loadDelay(path,tbodyid){
	...
	cell.align = "center";
	cell.bgColor = "#FFFFFF";
	cell.innerHTML = '<img src="'+path+'/images/icon/lodding.gif">加载中...';
}
...
function getAjaxURL(tableBody){
	if(tableBody == "objectTableBody"){
		WarningAjax.queryAllWarningObjects(pageVo,pageVo.getOrderSortStr(),getMapResult);
	} 
}
  /*
  *加载参照页面时候调用,需要用户修改
  */
  function loadPage(dialogDivId,tablebody){
  	  if(tablebody == "objectTableBody"){
		jQuery("#"+dialogDivId).load(contextpath+'/WarningTemplateAction.do',{cmd:'objectReference'},function(){getActionMap();});
  	  }
  }
/*
...
function getPageVojs(tbodyid){
	if(oldTableBody != tbodyid){
		if(tbodyid=="objectTableBody"){
			pageVo  = new PageVoJs();
			//设置默认排序方式
			pageVo.setOrderStr("id");
		}
	}
	setSerial();//重新设置序号
}
...
function getObjectListFields(){
	...
	var column1 = function(data){
		var checkObj = jQuery("<input type='radio'  name='checkbox_template' value='"+data.id+':'+data.returnType+"' />").get(0);
		return checkObj;
	};
	var column2=  function(data) { return data['objectName']};
   	var column3 = function(data) { return data['functionName']};
