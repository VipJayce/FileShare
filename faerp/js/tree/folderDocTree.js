...
{ 
  ...
  doc.write("</a>") 
  doc.write("</td><td oncontextmenu='showmenuie5()' >") 
  if (USETEXTLINKS) 
...
function showmenuie5() {
	...
	*/
	...
	/*
	if (rightedge < ie5menu.offsetWidth)					//如果鼠标位置太靠右
		ie5menu.style.left = document.body.scrollLeft + event.clientX - ie5menu.offsetWidth;	//则在鼠标箭头左边显示菜单
	else									//否则
		ie5menu.style.left = document.body.scrollLeft + event.clientX;		//在鼠标右边显示菜单
	if (bottomedge < ie5menu.offsetHeight)					//如果鼠标位置太靠底部
		ie5menu.style.top = document.body.scrollTop + event.clientY - ie5menu.offsetHeight;	//则在鼠标箭头上边显示菜单
	else									//否则
		ie5menu.style.top = document.body.scrollTop + event.clientY;		//在鼠标箭头下面显示菜单
	*/
	...
	ie5menu.style.visibility = "visible";					//将菜单设为可见
	return false;								//返回false值，取消本来应该显示的右键菜单
}
//隐藏菜单
function hidemenuie5() {
	ie5menu.style.visibility = "hidden";					//直接将层属性设为不可见即可
}
//加亮菜单项
...
function init_onLoad() {
	if (document.all && window.print) {		//如果是
		//var thisObj = document.getElementById('ie5menu');
		//thisObj.className = menuskin;			//将模拟菜单的层class属性设为"menuskin"
		//document.oncontextmenu = doNothing;		//当弹出菜单的时候调用showmenuie5显示自定义菜单并且取消原先应该显示的右键菜单
		//document.onmousedown = showmenuie5;
		//document.body.onclick = hidemenuie5;		//当点击页面时隐藏菜单
	}
}
...
//document.body.ondocumentready=init_onLoad;
/*
document.write('<div id="ie5menu" class="skin0" style="display:block;visivility:hidden" onMouseover="highlightie5()" onMouseout="lowlightie5()" onClick="jumptoie5();">');
//document.write('	<div class="menuitems" url="#">&nbsp;</div>');
document.write('	<div class="menuitems" url="javascript:history.back();">返回</div>');
document.write('	<div class="menuitems" url="#">回首页</div>');
document.write('	<hr>');
document.write('	<div class="menuitems" url="#">添加快捷</div>');
document.write('	<div class="menuitems" url="#">察看权限</div>');
document.write('</div>');
*/