...
function runExpression(C, F) {
	var B = C.snippets, G = [];
	for (var A = 0, E = 0, D = C.length; A < D; A++)
		if (B[E] == A) {
...
function f_alert_hidden_all_message() {
	f_alert_resetMessagePos();
	for (var A = 0; A < Eos_All_Message.length; A++)
		Eos_All_Message[A].Eos_Message.hidden()
}
...
Datacell.prototype.initHead = function() {
	...
	for (var I = 0; I < J; I++) {
		...
		E.innerHTML = ["<div class=\"eos-hd-inner ", (window.isGecko ? "eos-hd-inner-ff " : ""), K.styleClass, "\" unselectable=\"on\" style=\"text-align:left;\" >", "<span>", K.label, "</span>", "<div class=\"eos-hd-tool\" ><span class=\"eos-hd-icon\"></span><span class=\"eos-hd-split\">&#160;</span></div>", "</div>"].join("");
		A.appendChild(E);
		K.headCell = E;
