...
var EOSKey = {
    ...
    NUMLOCK : 144,
    SCROLLLOCK : 145
}, EOSKeyPress = {}, blankURL = contextPath + "/common/blank.jsp";
(function() {
    ...
    window.isIE = A.indexOf("trident") > -1;
    ...
    if (window.isMoz || !window.isIE) {
        ...
        Document.prototype.loadXML = function(E) {
            ...
            var D = new DOMParser(), B = D.parseFromString(E, "text/xml");
    ...
    if (document.implementation.hasFeature("XPath", "3.0")) {
        XMLDocument.prototype.selectNodes = function(F, C) {
            if (!C)
                C = this;
            var E = this.createNSResolver(this.documentElement), D = this.evaluate(F, C, E, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null), B = [];
            for (var A = 0; A < D.snapshotLength; A++)
                B[A] = D.snapshotItem(A);
            return B
        };
...
function isCrossDomain(B, C) {
    C = C || window;
    if (B && typeof (B) != "string") {
        try {
            B = B.location ? B.location.href : B
        } catch(D) {
            return true
        }
    }
    ...
    if (!A[1])
        return false;
    return (A[1] != C.location.protocol) || (A[2] != C.location.host)
}

function _get_top_window(E, H) {
    H = H || window;
    var C = [H], A = H, F = H;
    while (F != F.parent) {
        F = F.parent;
    ...
    function G(A, B) {
        try {
            return (A && !isCrossDomain(A, B) && A.document && (E || A.document.getElementsByTagName("frameset").length < 1))
        } catch(C) {
            return false
        }
    }
...
function setMaskSize(C) {
    var A = document.getElementsByTagName("BODY")[0], D = getViewportHeight(), B = getViewportWidth();
    if (D > A.scrollHeight)
        popHeight = D;
    else
        popHeight = A.scrollHeight;
    if (B > A.scrollWidth)
        popWidth = B;
    else
        popWidth = A.scrollWidth;
...
PageControl.add = function(E, B) {
    if (!__page__components[E])
        __page__components[E] = B;
    else if (__page__components[E + "__is__array__"])
        __page__components[E].push(B);
    else {
        var D = [];
    ...
    if (B.registerSubComponent && C[E])
        for (var A = 0; A < C[E].length; A++)
            if (C[E][A]) {
...
PageControl.registerRelation = function(D, E) {
    if (!D || !E)
        return;
    var C = PageControl.getOne(D), A = PageControl.getOne(E);
    if (C && C.registerSubComponent)
        C.registerSubComponent(E);
...
PageControl.setFocus = function(B, E) {
    ...
    if (B)
        D = B.id;
    var A = topWin.currStack.length - 1;
    for (; A > -1; A--) {
        var C = topWin.currStack[A];
        if (C)
            if (C == B)
                break;
            else if (C == E)
...
PageControl.addtoStack = function(A) {
    if (!$contains(topWin.currStack, A))
        topWin.currStack.push(A)
};
PageControl.get = function(A) {
    return __page__components[A]
};
PageControl.getOne = function(A) {
    if (__page__components[A + "__is__array__"])
        return __page__components[A][0] || null;
    else
        return __page__components[A] || null
};
...
if (navigator.product == "Gecko") {
    Document.prototype.elementFromPoint = function(E, D) {
        ...
        var C = this.createEvent("MouseEvents"), B = this.getBoxObjectFor(this.documentElement), A = {
            x : B.screenX,
            y : B.screenY
        };
...
if (!document.all) {
    try {
        if (window.Node)
            Node.prototype.contains = function(D) {
                if (!D)
                    return false;
                if (this == D)
                    return true;
                var C = D.tagName, A = this.getElementsByTagName(C), E = A.length;
                for (var B = 0; B < E; B++)
                    if (A[B] == D)
                        return true;
                return false
...
eventManager.add = function(A, B, D, C) {
    ...
    if (A == null || D == null)
        return false;
    if (B == "mousewheel" || B == "mousescroll")
        B = (window.isGecko) ? "DOMMouseScroll" : "mousewheel";
    else if (B == "load")
        return eventManager.onLoad(A, D, C);
...
eventManager.CleanUp = function() {
    if (_registryEvent)
        for (var A = 0; A < _registryEvent.length; A++) {
...
eventManager.getPageScroll = function(E) {
    E = E || window;
    var A = E.document.documentElement, B = E.document.body, D = 0, C = 0;
...
eventManager.getPointX = function(C) {
    C = C || window;
    var A = eventManager.getEvent(C), B = A.pageX;
    if (!B && 0 !== B) {
        B = A.clientX || 0;
        if (isIE)
            B += eventManager.getPageScroll(C)[0]
    }
    return B
};
eventManager.getPointY = function(C) {
    C = C || window;
    var A = eventManager.getEvent(C), B = A.pageY;
    if (!B && 0 !== B) {
        B = A.clientY || 0;
        if (isIE)
            B += eventManager.getPageScroll(C)[1]
    }
    return B
};
...
eventManager.getEvent = function(C) {
    ...
    if (C.event)
        ...
        while (A != null) {
            var B = A.arguments[0];
            if (B)
                if (window["Event"] && B instanceof Event)
                    return B;
            A = A.caller
        }
        return null
...
function ReleaseEvents() {
    return;
    var B = ["focus", "blur", "click", "mousedown", "mouseup", "mouseover", "keydown", "keypress", "keyup", "mouseout", "change"], A = function(C) {
        var A;
...
function isDate(D, A) {
    ...
    if (!(new RegExp(C)).test(D))
        return false;
    var B = isFormatDate(D, A), E = isFormatTime(D, A);
    if (B || E)
        return true;
    return false
}
...
function isTimeFormat(A) {
    if ( typeof (A) == "string")
        return A.indexOf("HH") != -1 && A.indexOf("mm") != -1 && A.indexOf("ss") != -1;
    else
        return false
}
...
function isDateFormat(A) {
    if ( typeof (A) == "string")
        return A.indexOf("yyyy") != -1 && A.indexOf("MM") != -1 && A.indexOf("dd") != -1;
    else
        return false
}
...
function isFormatTime(E, D) {
    ...
    if (C == -1)
        return false;
    var B = E.substring(C, C + 2), A = D.indexOf("mm");
    if (A == -1)
        return false;
    var F = E.substring(A, A + 2), G = D.indexOf("ss");
    if (G == -1)
        return false;
    var H = E.substring(G, G + 2);
    if (!isNumber(B) || B < 0 || B > 23)
        return false;
    if (!isNumber(F) || F < 0 || F > 59)
        return false;
    if (!isNumber(H) || H < 0 || H > 59)
        return false;
    return true
}
...
function isFormatDate(H, E) {
    ...
    if (G == -1)
        return false;
    var D = H.substring(G, G + 4), B = E.indexOf("MM");
    if (B == -1)
        return false;
    var A = H.substring(B, B + 2), F = E.indexOf("dd");
    if (F == -1)
        return false;
    var C = H.substring(F, F + 2);
    if (!isNumber(D) || D > 10000 || D < 999)
        return false;
    if (!isNumber(A) || A > 12 || A < 1)
        return false;
    if (C > getMaxDay(D, A) || C < 1)
        return false;
    return true
}
...
function stringToDate(X, C) {
    ...
    if (!C || C == "")
        C = "yyyy-MM-dd";
    var E = C.replace(/[^y|Y]/g, ""), G = C.replace(/[^M]/g, ""), N = C.replace(/[^d]/g, ""), Q = C.indexOf(E), W = E.length, S = X.substring(Q, Q + W) * 1;
    if (W == 2)
        if (S < 50)
            S += 2000;
        else
            S += 1900;
    var R = C.indexOf(G), H = X.substring(R, R + G.length) * 1 - 1, K = C.indexOf(N), J = X.substring(K, K + N.length) * 1, T = C.replace(/[^H]/g, ""), L = C.replace(/[^h]/g, ""), U = C.replace(/[^m]/g, ""), F = C.replace(/[^S|s]/g, ""), P = 0;
    if (T && T != "") {
        var O = C.indexOf(T);
        if (O > -1)
            P = X.substring(O, O + T.length)
    }
    if (L && L != "") {
        var A = C.indexOf(L);
        if (A > -1)
            P = X.substring(A, A + L.length)
    }
    var V = 0;
    if (U && U != "") {
        var B = C.indexOf(U);
        if (B > -1)
            V = X.substring(B, B + U.length)
    }
    var M = 0;
    if (F && F != "") {
        var D = C.indexOf(F);
        if (D > -1)
            M = X.substring(D, D + F.length)
    }
...
function dateToString(C, E) {
    if (!E || E == "")
        E = "yyyy-MM-dd";
    C = C || new Date();
    var I = C.getFullYear().toString(), A = (C.getMonth() + 1).toString(), B = C.getDate().toString(), N = C.getMinutes().toString(), F = C.getSeconds().toString(), H = C.getHours().toString(), M = E.replace(/[^y|Y]/g, "");
    if (I.length < 4)
        I = "0" + I;
    if (M.length == 2)
        I = I.substring(2, 4);
...
function setDate(I, J, C) {
    var H = J.indexOf("yyyy"), E = I.substring(H, H + 4), B = J.indexOf("MM"), A = I.substring(B, B + 2), G = J.indexOf("dd"), D = I.substring(G, G + 2), F = C;
...
function setTime(H, I, F) {
    var C = "00", E = "00", B = "00", D = I.indexOf("HH"), A = I.indexOf("mm"), G = I.indexOf("ss");
    if (G != -1 && D != -1 && A != -1) {
        B = H.substring(G, G + 2);
        E = H.substring(A, A + 2);
        C = H.substring(D, D + 2);
...
function numberToChinese(J) {
    var O = 99999999999.99, I, K, N = "", B, M, D, C, E, F, L, H, A, Q, G, P = J;
    P = P.toString();
    if (P == "") {
...
function formatNumber(A, E, C) {
    var D = E.split(";"), B;
    if (D.length == 1)
        B = singleFormat(A, E, C);
    else if (D.length == 2) {
        if (A > 0)
            B = singleFormat(A, D[0], C);
        else
            B = singleFormat(A, D[1], C)
    } else if (D.length == 3) {
        if (A < 0)
            B = singleFormat(A, D[1], C);
        else if (A > 0)
...
function singleFormat(J, R, M) {
	//add by bruce hua J === null
    if (isNaN(J) || J === null)
        return J;
    J = Number(J);
    var K, F, L, B, H = "", I = "", D = "", E = "", G, O, N, P, A = 0, C = 0, Q = "";
    R = (R) ? R + "" : "";
    if (R.indexOf(",") >= 0)
        B = ",";
    if (R.indexOf("%") >= 0) {
...
function moveScript(F, B) {
    ...
    if (B.parentWindow && F.parentWindow)
        B.parentWindow.contextPath = F.parentWindow.contextPath;
    var D = "";
    for (var A = 0; A < E.length; A++)
        if (E[A].src != "")
            D += "<script src='" + E[A].src + "'></script>";
    var C = $create("div", B);
...
function moveCss(H, B) {
    ...
    for (var A = 0; A < G.length; A++) {
        var F = G[A].href;
        if (F.indexOf("style-component.css") == -1)
            continue;
        if (B.createStyleSheet) {
            ...
            var C = B.styleSheets.length, E = $create("style", B);
...
function getParentByTagName(A, B) {
    if (B && A) {
        ...
        if (A.tagName && A.tagName.toLowerCase() == B)
            return A;
        return null
    } else
        return A ? A : null
...
function getEventTargetByTagName(B, D, C) {
    ...
    var D = D.toLowerCase();
    do
        if (A.tagName && A.tagName.toLowerCase() == D)
            return A;
...
function getScreenPosition(B, E) {
    var F = window, D = getElementXY(B), C = D[0], A = D[1];
    while (F != E && F != top) {
        if (F.frameElement) {
...
function addButtonToText(K) {
    if (K) {
        ...
        function H() {
            if (!K.getAttribute("_isOver") && !K.getAttribute("_isFocus") && !F.getAttribute("_isOver"))
                K.className = "eos-drop-down-text"
        }
...
function hideButton(B, D) {
    var C = B.offsetHeight, A = B.offsetWidth;
    B.style.width = A + "px"
}

function setButtonPosition(C, G) {
    var D = C.offsetHeight, B = C.offsetWidth;
    C.style.width = B + "px";
...
function getRect(B) {
    var C = getPosition(B), A = C.left + B.offsetWidth, D = C.top + B.offsetHeight;
    return {
        left : C.left,
        top : C.top,
        right : A,
...
// modified by Bruce hua 2012.9.3 12:00
function runExpression(templateC,entity){
        ...
        var expression = rs.join('');
        var retV; 
        try{
            retV = eval(expression);
        }catch(e) { 
            return expression; 
        } 
        return retV;
...
function sortTableByCol(J, B, H) {
    ...
    var F = C.tBodies[0], K = G && G.rows.length > 0 ? 0 : (J.parentNode.rowIndex + 1), D = F.rows, N = [];
    for (var M = K; M < D.length; M++) {
        N[M - K] = D[M];
        if (!N[M - K].getAttribute("__eos_orgorder"))
            N[M - K].setAttribute("__eos_orgorder", M + 1 + "")
    }
    var P = I;
    function E(A) {
    ...
    for ( M = 0; M < N.length; M++)
        O.appendChild(N[M]);
    F.appendChild(O);
    if (C.sortFlag && C.sortCol != J)
        C.sortFlag.className = "eos-sorttable-default";
    C.sortCol = J;
...
function submitFormBy(D, B, A, C) {
    if (D) {
        D.action = A || D.action;
        D.target = C || D.target;
        if (D.elements["_eosFlowAction"])
            D.elements["_eosFlowAction"].value = B;
        D.submit()
    }
}
...
function prevPage(I, C, B, G, A) {
    ...
    var F = H && H.value ? H.value : 0, E = Number(J.value) - Number(H.value);
    J.value = E < 0 ? 0 : E;
...
function getMaxZindex(E) {
    if (__topWin.MAXZINDEX == -1) {
        E = E || document;
        var C = E.all || E.getElementsByTagName("*"), B = C.length, G = 0;
        if (isIE) {
            for (var D = 0; ; D++) {
                var F = C[D];
                if (F == null)
            ...
            for ( D = 0; D < B; D++) {
                F = C[D], A = parseInt(GetCurrentStyle(F, "zIndex"));
                if (!isNaN(A))
                    if (A > G)
                        G = A
            }
        __topWin.MAXZINDEX = G + 1
    } else
        __topWin.MAXZINDEX = __topWin.MAXZINDEX + 1;
...
function setOpacity(B, A) {
    ...
    if (!B.currentStyle || !B.currentStyle.hasLayout)
        B.style.zoom = 1;
    if (window.isIE)
        B.style.filter = (A == 1) ? "" : "alpha(opacity=" + A * 100 + ")";
    B.style.opacity = A;
    if (A === 0) {
...
function accordionIn(A) {
    ...
    if (A.height - A.progress < 0)
        A.div.style.display = "none";
    else
        A.div.style.height = A.height - A.progress;
...
function msgbox(A) {
    showModalCenter(contextPath + "/common/jsp/msgBox.jsp", [A], null, null, null, "message notes")
}

function checkOnsubmit(B) {
    var C = B.submit, A = B.onsubmit;
    B.oldsubmit = C;
    B.oldOnsubmit = A;
    B.submit = function() {
        if (!checkForm(this))
...
function initShadow(C, E) {
    if (isFF) {
        ...
        if (B == null || !B.isShadow) {
            var D = C.parentNode, F = C.offsetWidth, G = C.offsetHeight, A = $createElement("div", {
                doc : E
            });
            A.isShadow = true;
            D.style.width = F;
            ...
            D = C.parentNode, F = C.offsetWidth, G = C.offsetHeight, A = C.shadowContainter;
        ...
        B = C.nextSibling;
        if (B == null || !B.isShadow) {
            D = C.parentNode, F = C.offsetWidth, G = C.offsetHeight, A = $createElement("div", {
                doc : E
            });
            A.isShadow = true;
            D.style.width = F;
...
function setEosControlStyleforIE() {
    if (isIE) {
        var F = document["styleSheets"], D = F.length;
        for (var A = 0; A < D; A++) {
            var C = F[A].rules, B = C.length;
            for (var E = 0; E < B; E++)
                if (C[E].selectorText == ".eos-ic-button")
                    C[E].style.verticalAlign = "text-bottom"
        }
    }
}
...
function flowSubmit(A) {
    var C = A.parentNode;
    while (C != null)
    if (C.tagName && C.tagName.toLowerCase() == "form")
        break;
    else
        C = C.parentNode;
    if (C == null) {
...
function initEosControlReset(C, A) {
    if (C.eosResetContorlList == null) {
        C.eosResetContorlList = [];
        C.eosResetContorlList.push(A)
    } else {
        C.eosResetContorlList.push(A);
        return
    }
    var B = C.reset, D = C.onreset;
    C.oldreset = B;
    C.oldOnreset = D;
    C.reset = function() {
        this.oldreset()
...
var Drag = {
    ...
    init : function(A, H, J, B, D, I, G, F, E, C) {
        ...
        A.root = H && H != null ? H : A;
        if (A.hmode && isNaN(parseInt(A.root.style.left)))
            A.root.style.left = "0px";
        if (A.vmode && isNaN(parseInt(A.root.style.top)))
            A.root.style.top = "0px";
        if (!A.hmode && isNaN(parseInt(A.root.style.right)))
            A.root.style.right = "0px";
        if (!A.vmode && isNaN(parseInt(A.root.style.bottom)))
            A.root.style.bottom = "0px";
    ...
    start : function(D) {
        var A = Drag.obj = this;
        D = Drag.fixE(D);
        var B = parseInt(A.vmode ? A.root.style.top : A.root.style.bottom), C = parseInt(A.hmode ? A.root.style.left : A.root.style.right);
        A.root.onDragStart(C, B);
        A.lastMouseX = D.clientX;
        A.lastMouseY = D.clientY;
        if (A.hmode) {
            if (A.minX != null)
                A.minMouseX = D.clientX - C + A.minX;
            if (A.maxX != null)
                A.maxMouseX = A.minMouseX + A.maxX - A.minX
        } else {
            if (A.minX != null)
                A.maxMouseX = -A.minX + D.clientX + C;
            if (A.maxX != null)
                A.minMouseX = -A.maxX + D.clientX + C
        }
        if (A.vmode) {
            if (A.minY != null)
                A.minMouseY = D.clientY - B + A.minY;
            if (A.maxY != null)
                A.maxMouseY = A.minMouseY + A.maxY - A.minY
        } else {
            if (A.minY != null)
                A.maxMouseY = -A.minY + D.clientY + B;
            if (A.maxY != null)
                A.minMouseY = -A.maxY + D.clientY + B
        }
    ...
    drag : function(H) {
        H = Drag.fixE(H);
        var A = Drag.obj, B = H.clientY, F = H.clientX, D = parseInt(A.vmode ? A.root.style.top : A.root.style.bottom), G = parseInt(A.hmode ? A.root.style.left : A.root.style.right), E, C;
        if (A.minX != null)
            F = A.hmode ? Math.max(F, A.minMouseX) : Math.min(F, A.maxMouseX);
        if (A.maxX != null)
            F = A.hmode ? Math.min(F, A.maxMouseX) : Math.max(F, A.minMouseX);
        if (A.minY != null)
            B = A.vmode ? Math.max(B, A.minMouseY) : Math.min(B, A.maxMouseY);
        if (A.maxY != null)
            B = A.vmode ? Math.min(B, A.maxMouseY) : Math.max(B, A.minMouseY);
...
function $createElement(F, E) {
    ...
    if (F.indexOf("<") > -1) {
        ...
        var G = (E.name) ? " name=\"" + E.name + "\"" : "", D = (E.type) ? " type=\"" + E.type + "\"" : "";
        delete E.name;
        delete E.type;
        F = "<" + F + G + D + ">";
        F = B.createElement(F)
...
function getPosition(A) {
    var B = A.offsetTop, C = A.offsetLeft, D = A;
...
function getAbsPos(C) {
    var B = C.offsetTop, E = C.offsetLeft, F = C;
...
function getScreenPos(F, C) {
    ...
    if (F == C)
        ...
        var H = F;
        while (H && H != C) {
            var B = H.frameElement, G = H.parent;
            if (B) {
                var D = getPosition(B);
                E = E + D.left;
                A = A + D.top
...
function getScroll(C) {
    var A = C, B = document;
    if (A == B || A == B.body) {
        var E = window.pageXOffset || B.documentElement.scrollLeft || B.body.scrollLeft || 0, D = window.pageYOffset || B.documentElement.scrollTop || B.body.scrollTop || 0;
        return [E, D]
    } else
...
function translatePoints(B, G, D) {
    ...
    var E = B.style.position, A = getElementXY(B), F = parseInt(B.style.left, 10), C = parseInt(B.style.top, 10);
    if (isNaN(F))
        F = (E == "relative") ? 0 : B.offsetLeft;
    if (isNaN(C))
        C = (E == "relative") ? 0 : B.offsetTop;
    return {
        left : (G - A[0] + F),
        top : (D - A[1] + C)
    }
...
function getElementXY(H, N) {
    ...
    while (B) {
        L += B.offsetLeft;
        J += B.offsetTop;
        if (!G && B.style.position == "absolute")
            G = true;
        if (isGecko) {
            F = B;
            var I = parseInt(F.style.borderTopWidth, 10) || 0, C = parseInt(F.style.borderLeftWidth, 10) || 0;
            L += C;
            J += I;
...
var CSSUtil = function(D) {
    var C = null, B = D || document, E = /(-[a-z])/gi, A = function(A, B) {
    ...
    return {
        createStyleSheet : function(A, H, F) {
            ...
            var D = E[0], C = F.createElement("style");
...
EOSLog.refresh = function() {
    if (EOSLog.panel) {
        var A;
        for ( A = EOSLog.index; A < EOSLog.messages.length; A++)
            EOSLog.print(EOSLog.messages[A]);
...
EOSLog.handle = function(B) {
    ...
    if (B.number)
        A.append(EOSLOG_ERROR_CODE).append(B.number).append(".>");
    if (B.fileName)
        A.append(EOSLOG_ERROR_URL).append(B.fileName).append(".");
    if (B.lineNumber)
        A.append(EOSLOG_ERROR_LINE).append(B.lineNumber).append(".");
    if (B.stack)
        A.append(EOSLOG_ERROR_STACK).append(B.stack.replace(/ /g, "&nbsp;").replace(/\n/g, "<br>").replace(/\t/g, "&nbsp;&nbsp;&nbsp;&nbsp;")).append("<br>");
    if (B.description)
        A.append(EOSLOG_ERROR_DETAIL).append(B.description).append("<br>");
    if (EOSLog.handle.caller && EOSLog.handle.caller.caller)
        A.append(EOSLOG_ERROR_FUNCTION).append("<br>").append(EOSLog.handle.caller.caller.toString().replace(/ /g, "&nbsp;").replace(/\n/g, "<br>").replace(/\t/g, "&nbsp;&nbsp;&nbsp;&nbsp;")).append("<br>");
...
EOSLog.console = function() {
    ...
    if (isIE)
        C.append("<iframe src='" + blankURL + "' style=\"position:absolute;z-index:-1; frameborder=\"0\" ></iframe>");
    var H = "", D = navigator.userAgent.toLowerCase().indexOf("msie 6") != -1;
    if (D)
        H = "style=\"filter:alpha(opacity=50)\"";
    C.append("<div style=\"z-index:1000\">");
    C.append("<TABLE  cellspacing=\"0\" cellpadding=\"0\"  class=\"eos-dialog\"><TR><TD " + H + " class=\"left-top-conner\"><div class=\"ieBlank\"></div></TD><TD  " + H + " class=\"top\"></TD><TD  " + H + " class=\"right-top-conner\"><div class=\"ieBlank\"></div></TD></TR><TR><TD  " + H + " class=\"left\"><div class=\"ieBlank\"></div></TD><TD style=\"background-color: #eaf0f2;\">");
    ...
    C.append("&nbsp;<div  id=\"eos_log_button0\"  class=\"eos-button\" onclick=\"EOSLog.clear()\" onmouseover=\"buttonMouseOver(this)\" onmouseout=\"buttonMouseOut(this)\" onmousedown=\"buttonMouseDown(this)\" onmouseup=\"buttonMouseUp(this)\"><div align=\"center\" class=\"eos-button-inner\">Clear</div></div>&nbsp;");
    C.append("<div id=\"eos_log_button1\"  class=\"eos-button\" onclick=\"EOSLog.command()\" onmouseover=\"buttonMouseOver(this)\" onmouseout=\"buttonMouseOut(this)\" onmousedown=\"buttonMouseDown(this)\" onmouseup=\"buttonMouseUp(this)\"><div align=\"center\" class=\"eos-button-inner\">Cmd</div></div>&nbsp;");
    C.append("<div id=\"eos_log_button2\"  class=\"eos-button\" onclick=\"EOSLog.hide()\"onmouseover=\"buttonMouseOver(this)\" onmouseout=\"buttonMouseOut(this)\" onmousedown=\"buttonMouseDown(this)\" onmouseup=\"buttonMouseUp(this)\"><div align=\"center\" class=\"eos-button-inner\">Close</div></div>&nbsp;");
    ...
    C.append("<div  id=\"eos_log_button3\"  class=\"eos-button\" onclick=\"EOSLog.runEval()\"onmouseover=\"buttonMouseOver(this)\" onmouseout=\"buttonMouseOut(this)\" onmousedown=\"buttonMouseDown(this)\" onmouseup=\"buttonMouseUp(this)\"><div align=\"center\" class=\"eos-button-inner\">Run</div></div>&nbsp;");
    C.append("<div  id=\"eos_log_button4\"  class=\"eos-button\" onclick=\"EOSLog.clearEval()\"onmouseover=\"buttonMouseOver(this)\" onmouseout=\"buttonMouseOut(this)\" onmousedown=\"buttonMouseDown(this)\" onmouseup=\"buttonMouseUp(this)\"><div align=\"center\" class=\"eos-button-inner\">Clear</div></div>&nbsp;");
    C.append("</div>");
    C.append("</div>");
    ...
    C.append("</TD><TD  " + H + " class=\"right\" id=\"eos_log_right\"><div class=\"ieBlank\"></div></TD></TR><TR><TD  " + H + " class=\"left-bottom-conner\"></TD><TD  " + H + " class=\"bottom\" id=\"eos_log_bottom\"></TD><TD  " + H + " class=\"right-bottom-conner\" id=\"eos_log_rightBottom\"></TD></TR></TABLE>");
    C.append("</div>");
    ...
    I();
    var G = $id(EOSLog.BUTTONS_ID, EOSLog.doc);
    ...
    if (isFF)
        for (var A = 0; A < 5; A++)
            buttonForFF(EOSLog.doc.getElementById("eos_log_button" + A))
};
...
EOSLog.time = function(H, G, E) {
    ...
    if (!EOSLog.timeLog[H]) {
        ...
        var F = new Date(), C = EOSLog.timeLog[H], A = F - C.time, B = new StringBuffer();
        B.append("ID:").append(H).append("<br>");
        if (C.msg)
            B.append("start:").append(C.msg).append("<br>");
        B.append(" time:<b>").append(A).append("ms</b><br>");
        if (G)
            B.append("end:").append(G);
        EOSLog.log("time", B.toString());
        if (E)
            EOSLog.timeLog[H] = null
    }
};
function $info(B) {
...
EOSLog.evalKeyUp = function() {
    var C = $id(EOSLog.EVAL_INPUT_ID, EOSLog.doc), D = eventManager.getKeyCode(), B = $id(EOSLog.EVAL_SIGN_ID, EOSLog.doc);
    if (D == 190) {
...
EOSLog.selectDownOption = function(C) {
    var A = C.selectedIndex + 1, B = C.options[A];
    if (B)
        B.selected = true;
    else {
        B = C.options[0];
...
EOSLog.evalKeyDown = function() {
    var C = $id(EOSLog.EVAL_INPUT_ID, EOSLog.doc), D = eventManager.getKeyCode(), B = $id(EOSLog.EVAL_SIGN_SELECT_ID, EOSLog.doc), A = $id(EOSLog.EVAL_SIGN_ID, EOSLog.doc);
    if (D == 9) {
        eventManager.stopPropagation();
...
EOSLog.getkeyWord = function(C) {
    ...
    if (EOSLog.doc.selection) {
        var D = EOSLog.doc.selection.createRange(), E = EOSLog.doc.body.createTextRange();
        ...
        if ( typeof (L.selectionStart) == "number") {
            ...
            if (start == end) {
                var J = L.value.substring(0, end), K = EOSLog.getSingWordByLeft(L, end), A = J.split("\n"), B = A[A.length - 1], F = B.length * 6 - L.scrollLeft, I = A.length * 16 - L.scrollTop;
                if (F > L.offsetWidth)
...
EOSLog.getSingWordByLeft = function(D, C) {
    var A = D.value.substring(0, end), B = /([\w|\.]+)\.$/;
    B.test(A);
    return RegExp.$1
};
EOSLog.insertAtCaret = function(G, A) {
    if (isIE) {
        ...
        var F = G.selectionStart, C = G.selectionEnd, B = G.value.substring(0, F), E = G.value.substring(C);
        G.value = B + A + E;
        var D = F + A.length;
        G.focus();
...
EOSLog.getScriptStr = function(A) {
    if (!EOSLog.scriptObject[A])
        EOSLog.scriptObject[A] = getRemoteResource(A);
...
EOSLog.getDetailError = function(F, J, G, I) {
    ...
    if (F) {
        ...
        if (L.indexOf(F) > 0)
            A = getRemoteResource(window.location);
        else {
            K.sLine = I - 1;
        ...
        if (A) {
            ...
            if (K.errLine && K.errFunc)
                if (K.errFunc.indexOf(K.errLine) >= 0)
                    K.errFunc = K.errFunc.replace(K.errLine, "<b><span style='color:red;background-color:#eeeeee;font-size:larger'>" + K.errLine + "</span></b>")
        }
    }
    return K
};
...
function initDrag(C) {
    var B = EOSLog.topWin.fx_DD || fx_DD, D = $id("eos_log_right", EOSLog.doc), E = $id("eos_log_bottom", EOSLog.doc), A = $id("eos_log_rightBottom", EOSLog.doc);
    B(C, {
        handle : E,
        modifiers : true,
        onDrag : function() {
            var B = this.mouse.start["y"], A = this.mouse.now["y"];
            if (A - B + this.value.now["y"] >= 27)
                this.element.style.height = A - B + this.value.now["y"];
    ...
    B(C, {
        ...
        modifiers : true,
        onDrag : function() {
            var B = this.mouse.start["x"], A = this.mouse.now["x"];
            if (A - B + this.value.now["x"] >= 150)
                this.element.style.width = A - B + this.value.now["x"];
    ...
    B(C, {
        ...
        onDrag : function() {
            var E = this.mouse.start["y"], A = this.mouse.now["y"];
            if (A - E + this.value.now["y"] >= 27)
                C.style.height = A - E + this.value.now["y"];
            var D = this.mouse.start["x"], B = this.mouse.now["x"];
            if (B - D + this.value.now["x"] >= 150)
                C.style.width = B - D + this.value.now["x"];
...
function resizeLogPanel(B, A) {
    if (B != null && B != "")
        EOSLog.panel.style.height = B - 27;
    if (A != null && A != "")
        EOSLog.panel.style.width = A
}
...
Ajax.prototype.initParam = function() {
    var D = "", G = "<root>";
    for (var C = 0; C < this.param.length; C++) {
        var F = this.param[C];
        D += "<param><key>" + xmlConversion(F.key) + "</key><value>" + xmlConversion(F.value) + "</value></param>"
    }
    G += "<params>" + D + "</params>\n";
    var E = this.xmlParam;
    if (this.dataParam && this.dataParam.length > 0)
        for (var C = 0, B = this.dataParam.length; C < B; C++) {
            var A = this.dataParam[C];
            E += A.value
        }
    if (E && E.indexOf("<root><data>") == 0) {
        E = E.substr("<root><data>".length);
        E = E.substr(0, E.lastIndexOf("</data></root>"))
    }
    G += "<data>" + E + "</data></root>";
    this.submitParam = G
};
...
Ajax.prototype.wrapperUrl = function() {
    if (this.url.indexOf("?") > 0 ){
        return this.isCommonUrl ? this.url : this.url + "&time=" + (new Date())
    }else{
        return this.isCommonUrl ? this.url : this.url + "?time=" + (new Date())
   }
};
...
Ajax.prototype.callBack = function() {
    var B = false, D = null, C = this.getExceptionInvalid();
    if (C === true || C === "true") {
        D = this.getLoginPage();
        if (D)
            B = true;
        C = AJAX_EXCEPTION_INVALID + "\n\n" + this.url
    } else
        C = this.getExceptionMessage() || this.getException();
    if (B) {
        if (window.contextPath && D.indexOf(window.contextPath + "/") != 0)
            D = window.contextPath + (D.indexOf("/") == 0 ? "" : "/") + D;
        var A = _get_top_window();
        A.location = D;
        return
    }
    if (C && this.onFailure)
        return this.onFailure(C);
...
Ajax.prototype.getParam = function(A) {
...
Ajax.prototype.setOutParam = function(A) {
    this.addParam("__outParam", A)
};
Ajax.prototype.setParamsInfo = function(A) {
    this.addParam("__paramsInfo", A)
};
Ajax.prototype.setInoutParam = function(A) {
    this.addParam("__inoutParams", A)
};
...
Ajax.prototype.submit = function(I) {
    if (this.url && this.url.lastIndexOf(".beanx") == this.url.length - (".beanx").length)
        ...
        var P = this.url.lastIndexOf("/"), C = this.url.lastIndexOf(".kservice"), Q = this.url.substring(P + 1, C);
        P = Q.lastIndexOf(".");
        var L = Q.substring(0, P), R = Q.substring(P + 1);
        if (!this.xmlParam) {
            var O = this.getParam("__outParam"), M = this.getParam("__inoutParams"), D = this.getParam("__paramsInfo"), B = [];
            if (!D) {
                D = [];
                for (var H = 0; H < this.param.length; H++) {
                    var E = this.param[H];
                    D.push(E.key)
                }
                D = D.join(",")
            }
            if (D.indexOf(",__outParam") == -1)
                D += ",__outParam";
            if (D.indexOf(",__inoutParams") == -1)
                D += ",__inoutParams";
            var A = D.split(",");
            for (var N = 0; N < A.length; N++) {
                var G = A[N];
                B.push(this.getParam(G))
            }
            B.push(D);
            this.param = [];
            this.xmlParam = ServiceCaller.submitXMLTemplate(L, R, ServiceCaller.getSubmitXML(B))
        }
    }
    this.running = true;
    this.encoding = I || this.encoding;
    this.initParam();
    try {
        var J = this.wrapperUrl();
        this.xmlHttp.open(this.method, J, this.async);
        var K = this.wrapperData(), F = this;
        if (this.async)
            this.xmlHttp.onreadystatechange = function() {
                F.onStateChange.apply(F, arguments)
            };
        this.setRequestHeaders();
        $debug("submit url:" + this.url + "\n submit XML:\n" + EOSLog.formatXML(K));
        this.xmlHttp.send("__ajaxParam=" + encodeURIComponent(K));
        if (!this.async)
...
Ajax.prototype.setObjectValue = function(B, E, F) {
    ...
    if (C != null) {
        ...
        if (B.type == "select-one") {
            I = this.getProperty(C);
            for ( G = 0; G < B.options.length; G++)
                if (I == B.options[G].value)
...
Ajax.prototype.Updater = function(C) {
    ...
    this.submit("utf-8");
    var B = this.retDom.selectSingleNode("root/data"), A = getNodeValue(B);
    setInnerHTML(C, A)
};
...
Ajax.prototype.getReturnNodeValue = function(B) {
    var A = this.getResponseXMLDom(), C = A ? A.selectNodes(B) : null;
    if (C && C.length > 0)
        return getNodeValue(C[0]);
    return null
};
...
Ajax.prototype.getLoginPage = function() {
    var B = "/root/exceptions/loginPage", A = this.getReturnNodeValue(B);
    A = A ? trim(A + "") : null;
    return A
};
var HideSubmit = Ajax;
...
ServiceCaller.returnConversion = function(D) {
    var F = D.getAttribute("__category"), B = D.getAttribute("__type");
    if (F == "xml")
        return D.xml;
    else if (F == "entity")
        return Dataset.initEntity(D);
    else if (F == "dataset") {
        var E = new Dataset(D.tagName);
        E.init(D.childNodes);
        return E
    } else if (F == "collection") {
        var G = D.selectNodes("./entry"), C = [];
        for (var A = 0; A < G.length; A++) {
            var D = G[A];
            if (D.nodeType == 1)
                C.push(getNodeValue(D))
        }
        return C
    }
...
ServiceCaller.parameConversion = function(F) {
    var C = null, G = null, E = typeof (F), H = "primeval", D = {
        data : "",
        type : H
    };
    if (F === null || F === undefined)
    ...
    if (E == "string" || E == "number" || E == "boolean") {
        ...
        if (E == "object" && F.tagName && F.tagName.toUpperCase() == "FORM")
            F = getFormElementsValue(F);
        if ( F instanceof Dataset)
            F = F.getEntities();
        if ( F instanceof Array) {
            H = "collection";
            C = [];
            for (var B = 0; B < F.length; B++)
                C.push(ServiceCaller.parameConversion(F[B]).data);
            D.data = C.join("\n")
        } else {
            H = "form";
            C = [];
...
ServiceCaller.prototype.callAsyn = function(C, D, B) {
    var A = this;
    this._callCore(C, D, true, function() {
        A.callBack()
...
ServiceCaller.prototype.getLoginPage = function() {
    var B = "/root/data/ajaxCall/exceptions/loginPage", A = this.getReturnNodeValue(B);
    A = A ? trim(A + "") : null;
    return A
};
...
function getTopContainer(A) {
    ...
    while (true) {
        ...
        if (A == null) {
            if (B != null)
                if (B.getAttribute("height") != null || B.style.height != "")
                    return B;
            break
        }
...
function registerTopContainer(B) {
    ...
    if (C == null)
        return false;
    for (var A = 0; A < EOSResizeContainer.length; A++)
        if (EOSResizeContainer[A] == C)
            return true;
    ...
    if (isFF) {
        for (var A = 0; A < EOSResizeObjects.length; A++) {
            if (_eos_curr_open_panel != null)
                if (!EOSResizeObjects[A].isInCurrPanel()) {
                    EOSResizeObjects[A].doAutoResizeS2 = false;
                    continue
                }
            EOSResizeObjects[A].autoResizeS1()
        }
        for ( A = 0; A < EOSResizeContainer.length; A++)
            EOSResizeContainer[A].style.height = EOSResizeContainer[A].offsetHeight - 1;
...
function _layout_auto_resize() {
    for (var A = 0; A < EOSResizeContainer.length; A++)
        EOSResizeContainer[A].style.height = EOSResizeContainer[A].getAttribute("height");
...
function _layout_setResised() {
    for (var A = 0; A < _eos_colsed_panel.length; A++)
        _eos_colsed_panel[A].needResize = true
}
...
function isPort(A) {
    if ( typeof (A) == "object")
        A = A.value;
    if (/^\d+$/.test(A)) {
        if (parseInt(A, 10) > 65535 || parseInt(A, 10) <= 0)
            return false;
        return true
    } else
        return false
...
function isChinaMobileNo(C) {
    if ( typeof (C) == "object")
        C = C.value;
    var A = /(^[1][35][0-9]{9}$)|(^0[1][35][0-9]{9}$)/, B = new RegExp(A);
    if (B.test(C))
        return true;
    return false
}
...
function isPhoneNo(C) {
    if ( typeof (C) == "object")
        C = C.value;
    var A = /(^([0][1-9]{2,3}[-])?\d{3,8}(-\d{1,6})?$)|(^\([0][1-9]{2,3}\)\d{3,8}(\(\d{1,6}\))?$)|(^\d{3,8}$)/, B = new RegExp(A);
    if (B.test(C))
        return true;
    return false
}
...
function isChinaOrNumbOrLett(C) {
    if ( typeof (C) == "object")
        C = C.value;
    var A = "^[0-9a-zA-Z\u4e00-\u9fa5]+$", B = new RegExp(A);
    if (B.test(C))
        return true;
    return false
}
...
function isChinaIDNo(I) {
    if ( typeof (I) == "object")
        I = I.value;
    var H = EOS_CITY_LIST, F = 0, D = "", G = I, E = G.length;
...
function processExtAttr(C) {
    if (C.EOS_extendedAttribute != null)
        return false;
    var B = C.validateAttr || C.getAttribute("validateAttr");
    if (!B)
        return true;
    var D = new Object(), H = B.split(";");
...
//2014年6月6日 add by sqp 体检报销时用到
function f_check_float460(A) {
    if (/^(\+|-)?\d+($|\.\d+$)/.test(A.value))
        if(A.value<=460){
        return true;
        }else{
        	f_alert(A,"金额不能超过460元！")
        	return false;
        }
    else {
        f_alert(A, CHECK_INPUT_FLOAT);
...
function f_check_double(B) {
    var A = B.getAttr("totalDigit"), D = B.getAttr("fracDigit");
    if (isDecimal(B, A, D))
        return true;
    var C = CHECK_INPUT_NUMBER;
    if (A != null)
        C = C + CHECK_LEGNT_NOT_THAN + A + CHECK_WEI;
    if (D != null)
        C = C + CHECK_FRACTION_LENGTH_NOT_THAN + D + CHECK_WEI;
    f_alert(B, C);
    return false
}
...
function checkForm(E) {
    ...
    for ( A = E.elements.length - 1; A >= 0; A--) {
        if ((E.elements[A].validateAttr || E.elements[A].getAttribute("validateAttr")) + "" == "undefined")
            continue;
        if (E.elements[A].disabled == true)
            continue;
        if (E.elements[A].name == null || E.elements[A].name == "")
            if (!(E.elements[A].id != null && E.elements[A].id.indexOf("_input") != -1))
                continue;
...
function f_alert_getPosition(B) {
    ...
    if (G != null) {
        ...
        if (E != -1)
            B = $id(G.substr(0, E) + "_container") || B
    }
    var A = B.offsetTop, C = B.offsetLeft, D = B, F = true;
    while ( D = D.offsetParent) {
        if (D.tagName == "BODY")
            continue;
        A += D.offsetTop - D.scrollTop;
        C += D.offsetLeft - D.scrollLeft;
        if (D.style.position.toLowerCase() == "absolute")
            F = false
    }
...
function f_alert_show_message(A, B) {
    if (A.Eos_Message != null) {
        ...
        f_alert_create_Message_plane(A, B)
}

function f_alert_hidden_message(A) {
    if (A.Eos_Message != null)
        A.Eos_Message.hiddenPlane()
}
...
function f_alert_hidden_all_message() {
    f_alert_resetMessagePos();
    for (var A = 0; A < Eos_All_Message.length; A++)
        Eos_All_Message[A].Eos_Message.hiddenPlane()
}
...
function regCheckEvent(D, C) {
    ...
    for ( A = D.elements.length - 1; A >= 0; A--) {
        if ((D.elements[A].validateAttr || D.elements[A].getAttribute("validateAttr")) + "" == "undefined")
            continue;
        var B = D.elements[A];
        eventManager.add(B, C, function(B) {
...
function Mask(C, B) {
    ...
    this.autoResize = function() {
        if (this.isHidden())
            return;
        var E = parseInt(this.container.style.marginTop), G = parseInt(this.container.style.marginLeft);
        E = E || 0;
        G = G || 0;
        var C = 0, F = 0, A, B;
        ...
        if (this.isFrameSetPage) {
            for (var A = 0; A < this.frameList.length; A++) {
                try {
                    if (!this.frameList[A].contentWindow.eos_pageMask)
                        this.frameList[A].contentWindow.eos_pageMask = new Mask("eos_pageMask" + "_f_" + A, this.frameList[A].contentWindow);
    ...
    this.hide = function() {
        if (this.isFrameSetPage) {
            for (var A = 0; A < this.frameList.length; A++) {
                try {
                    if (!this.frameList[A].contentWindow.eos_pageMask)
                        this.frameList[A].contentWindow.eos_pageMask = new Mask("frame_" + this.frameList[A].contentWindow.location, this.frameList[A].contentWindow);
...
function maskElement(E, G) {
    ...
    var C = $id(E.maskId, G.document), B = E.offsetWidth, D = E.offsetHeight;
    if (!C) {
        C = $create("div", G.document);
        C.style.display = "none";
        C.id = E.maskId;
        if (document != G.document)
            moveCss(document, G.document);
    ...
    if (isFrameSet(F)) {
        ...
        if (!B) {
            ...
            if (document != F.document)
                moveCss(document, F.document);
            F.document.body.appendChild(B)
        }
        function C() {
            var G = parseInt(F.document.body.style.marginTop), J = parseInt(F.document.body.style.marginLeft);
            G = G || 0;
            J = J || 0;
            var E = 0, H = 0, A, D, C = (Math.max(F.document.documentElement.scrollLeft, F.document.body.scrollLeft) || 0), I = (Math.max(F.document.documentElement.scrollTop, F.document.body.scrollTop) || 0);
...
function hideSelect(C) {
    if (C) {
        if (!C["mask-select-style"])
            C["mask-select-style"] = createStyle("select", C.document, true);
        if (C["mask-select-style"])
            C["mask-select-style"].style.visibility = "hidden";
...
function showSelect(C) {
    if (C) {
        if (!C["mask-select-style"])
            C["mask-select-style"] = createStyle("select", C.document, true);
        if (C["mask-select-style"])
            C["mask-select-style"].style.visibility = "visible";
        var B = C.document.getElementsByTagName("frame");
        for (var A = 0; A < B.length; A++) {
            try {
...
var EOSProgressCache = {}, ProgressBar = function(D) {
...
function hideAllProgressBar() {
    for (var A in EOSProgressCache)
    EOSProgressCache[A].hide()
}
...
var getCurrentModalArguments = function() {
    ...
    if (A > 0)
        return EOStopWin["_eos_modal_dialog"][A - 1];
    else
        return null
    ...
    if (A) {
        A.hideMask();
        showSelect(A.frameWin)
    } else
        unMaskTop()
}, hideModelDialog = function(H, B) {
    try {
        var G = getModalDialog(H);
        EOStopWin["_eos_modal_dialog"].pop();
        if (!G)
            return;
        var F = G.frameWin || G.iframe.contentWindow;
        try {
            var A = _get_top_window();
            A.currStack = F.bfCurrStack || []
        } catch(C) {
        }
        try {
            var E = F["returnValue"]
    ...
    try {
        G.iframe.src = blankURL;
        G.ddDiv.style.display = "none";
        ReleaseElemEvents(G.container);
        ReleaseElemEvents(G.ddDiv);
        G.ddDiv.parentNode.removeChild(G.ddDiv);
...
function showModalCenter(H, B, A, F, G, C) {
    var E = null, D = null;
    return showModal(H, B, A, F, G, E, D, C)
}

function showModal(F, H, G, D, L, K, J, B) {
    D = D ? parseInt(D, 10) : 300;
    L = L ? parseInt(L, 10) : 200;
    D = D + 14;
    L = L + 14;
    var I = _get_top_window(), A = I.document.getElementsByTagName("BODY")[0];
    if (K == null || K == "") {
        K = ((I.document.body.clientWidth - D) / 2 + I.document.body.scrollLeft);
        if (K < 0)
            K = 0
    } else
        K = parseInt(K, 10);
    if (J == null || J == "") {
        ...
        J = parseInt(J, 10);
    var E = I.document.body.scrollTop, M = getModalDialogId(), C = new ModalArguments(M);
    C.parentWin = window;
    C.callBack = G;
    C.Argument = H;
    C.win = I;
    ...
    __showModal(M, F, C, D, L, K, J, B);
    I.document.body.scrollTop = E
}

function __showModal(a, G, C, P, Z, L, W, B) {
    var J = $create("div", C.win.document);
    J.id = a + "__model_dialog_container";
    ...
    if (isIE)
        ;
    var A = navigator.userAgent.toLowerCase().indexOf("msie 6") != -1, N = "";
    if (A)
        N = "style=\"filter:alpha(opacity=50)\"";
    var T = [F, "<div style=\"width:100%;height:100%\">", "<div id=\"" + a + "__model_dialog_mask\" onmousedown=\"eventManager.stopEvent();return false;\" class=\"eos-div-mask\" style=\"position:absolute;top:0px;left:0px;display:none;\"></div>", "<TABLE  cellspacing=\"0\" cellpadding=\"0\" width=\"100%\" height=\"100%\" class=\"eos-dialog\"><TR><TD ", N, " class=\"left-top-conner\"><div class=\"ieBlank\"></div></TD><TD  ", N, " class=\"top\"></TD><TD  ", N, " class=\"right-top-conner\"><div class=\"ieBlank\"></div></TD></TR><TR><TD  ", N, " class=\"left\"><div class=\"ieBlank\"></div></TD><TD style=\"background-color: #eaf0f2;\">", "<div style=\"width:100%;cursor:move\" id=\"" + a + "__model_dialog_head\" class=\"eos-popwin-head\" ><a id=\"" + a + "__model_dialog_focus\" href=\"#f\"></a>", "<div class=\"eos-popwin-head-icon\">&#160;</div>", "<div class=\"eos-popwin-head-title\">" + (B || " Dialog Window") + "</div>", "<div id=\"popupControls\" class=\"eos-popwin-head-button\"  >", "<a href=\"javascript:void(0);\" id=\"" + a + "__model_dialog_closebutton\" onmousedown=\"window.returnValue=undefined;hideModelDialog('" + a + "',true)\">&#160;</a>", "</div>", "</div>", "<div id=\"" + a + "__model_dialog_body\" class=\"eos-popwin-body\">", "</div>", "</TD><TD  ", N, " class=\"right\" id=\"", a, "_right\"><div class=\"ieBlank\"></div></TD></TR><TR><TD  ", N, " class=\"left-bottom-conner\"></TD><TD  ", N, " class=\"bottom\" id=\"", a, "_bottom\"></TD><TD  ", N, " class=\"right-bottom-conner\" id=\"", a, "_rightBottom\"></TD></TR></TABLE>", "</div>"].join(""), Q = ["<iframe src=\"", blankURL, "\" type=\"_eos_modal_dialog\" eosid=\"" + a + "\" name=\"" + a + "__model_dialog_frame\" id=\"" + a + "__model_dialog_frame\" scrolling=\"auto\" ", "  HSPACE =\"0\" VSPACE=\"0\" MARGINHEIGHT=\"0\" MARGINWIDTH=\"0\" ", " class=\"eos-popwin-body-iframe\"  border =\"0\"   ", "frameborder=\"0\" allowtransparency=\"true\" ></iframe>"].join("\n"), D = getMaxZindex(C.win.document);
    J.style.width = P + "px";
    J.style.height = Z + "px";
    J.style.left = L + "px";
    J.style.top = W + "px";
    ...
    var b = $id(a + "__model_dialog_mask", C.win.document);
    b.style.width = P + "px";
    b.style.height = Z + "px";
    C.maskDiv = b;
    b.style.zIndex = D + 2;
    J.style.zIndex = D + 1;
    var O = $id(a + "__model_dialog_body", C.win.document), S = O.offsetWidth;
    O.style.height = (Z - 27) > 0 ? (Z - 27 - 7) : 0 + "px";
    O.innerHTML = Q;
    var U = $id(a + "__model_dialog_frame", C.win.document);
    if (U) {
        U.src = G;
        U.style.height = (Z - 28) > 0 ? (Z - 28 - 7) : 0 + "px";
        U.style.width = S;
        U.style.zIndex = D + 1
    }
    var Y = $id(a + "__model_dialog_head", C.win.document);
    ...
    var R = $create("div", C.win.document);
    ...
    R.style.zIndex = D;
    C.win.document.body.appendChild(R);
    var V = C.win.fx_DD || fx_DD;
    if (V)
        V(R, {
            handle : Y,
            limit : {
            ...
            onComplete : function() {
                ...
                R.style.height = "24px";
                J.style.left = R.style.left;
                J.style.top = R.style.top;
    ...
    C.ddDiv = R;
    J.style.display = "none";
    var X = $id(a + "__model_dialog_focus", C.win.document), K = $id(a + "_right", C.win.document), E = $id(a + "_bottom", C.win.document), I = $id(a + "rightBottom", C.win.document);
    V(J, {
        handle : E,
        modifiers : true,
        onDrag : function() {
            ...
            if (A - B + this.value.now["y"] >= 27)
                this.element.style.height = A - B + this.value.now["y"];
            resizeModelDialog(a, parseInt(J.style.width), parseInt(J.style.height));
    ...
    V(J, {
        ...
        onDrag : function() {
            var E = this.mouse.start["y"], A = this.mouse.now["y"];
            if (A - E + this.value.now["y"] >= 27)
                J.style.height = A - E + this.value.now["y"];
            var D = this.mouse.start["x"], B = this.mouse.now["x"];
            if (B - D + this.value.now["x"] >= 150)
                J.style.width = B - D + this.value.now["x"];
            resizeModelDialog(a, parseInt(J.style.width), parseInt(J.style.height));
            C.iframe.style.display = "none"
        },
        onComplete : function() {
...
function loadParent(A) {
    if (window.parentWindow)
        if (A)
            window.parentWindow.location = A;
        else
...
function showAllSelect(B) {
    if (isIE && B) {
        var D = B.getElementsByTagName("select");
        for (var A = 0; A < D.length; A++)
            D[A].style.display = D[A].back_display;
        var C = getFrameDocuments(B);
        for ( A = 0; A < C.length; A++)
            showAllSelect(C[A])
    }
...
RichTextEditor.prototype.getValue = function() {
    var B = this.getFCKEditor(), A = B.GetHTML();
    if (A && this.value)
        if (A == "<p>" + this.value + "</p>")
            return this.value;
    if (A == "" || A == "<p></p>")
...
RichTextEditor.prototype.setPosition = function(D, A, B, C) {
    ...
    if (this.container.style.zIndex != E)
...
CheckGroup.prototype.getSelectRows = function() {
    ...
    for (var A = 0; A < this.subCheck.length; A++)
        if (this.subCheck[A].getStatus())
            B.push(this.subCheck[A]);
    return B
};
CheckGroup.prototype.getSelectParams = function(D) {
    var E = this.getSelectRows(), C = [];
    for (var A = 0; A < E.length; A++) {
...
rowCheckBox.prototype.getParam = function(C) {
    for (var A = 0; A < this.subboxes.length; A++) {
        var B = this.subboxes[A];
        if (B.name.replace(/\[\d*\]/, "[*]") == C || B.name.replace(/\[\d*\]/, "") == C)
            return B.value
    }
    return null
};
...
RadioGroup.prototype.init = function() {
    for (var A = 0; A < this.subRadio.length; A++) {
        if (this.subRadio[A].isChecked)
            this.subRadio[A].reverseSelect();
        else
            this.subRadio[A].disSelected();
...
Entity.prototype.getPropertyByXpath = function(C) {
    var D = (C + "").split("/"), B = this;
    for (var A = 0; A < D.length; A++) {
        var E = D[A];
        if ( B instanceof Entity)
            B = B.getPropertyByFieldName(E);
        else
            B = B[E];
        if (B === null || B === undefined)
            break
...
Entity.prototype.toXMLString = function(C) {
    C = C !== false;
    var J = new StringBuffer(), F = "", I = this.name;
    if (C) {
    ...
    var A = this.getKeys(), D = {};
    for (var H = 0; H < A.length; H++) {
        var K = this.getProperty(A[H]), L = K instanceof Dataset, B = K instanceof Entity;
        if (K === null || K === undefined || K === "") {
            if (A[H].indexOf("/") > 0) {
                ...
                J.append("<").append(A[H]).append(" __isNullOrEmpty=\"null\"/>\n")
        } else if (!L && !B) {
            if (A[H].indexOf("/") > 0) {
                E = A[H].split("/");
                if (!D[E[0]])
...
Dataset.prototype.getRemovedEntities = function() {
    ...
    for (var A = 0; A < this.values.length; A++) {
        var B = this.values[A];
        if (B.status == Entity.STATUS_DELT)
            C.push(B)
    }
    return C
};
...
Dataset.initEntity = function(E) {
    ...
    for (var F = 0; F < B.length; F++) {
        var A = B[F];
        if (A.nodeType == 1)
            if (!A.getAttribute(Entity.LINK_SIGN))
                Dataset.initField(G, A);
            else if (D["type"] == A.tagName)
                D.push(A);
            else {
                if (D["type"] != null) {
...
Dataset.initField = function(D, C) {
    if (C.childNodes.length > 1) {
        ...
        D.setProperty(C.tagName, getNodeValue(C))
};
...
Dataset.prototype.getInsertDataset = function() {
    var B = new Dataset(this.entityType), C = this.getInsertEntities();
    for (var A = 0; A < C.length; A++)
        B.addEntity(C[A].clone(true));
    return B
};
Dataset.prototype.getRemovedDataset = function() {
    var B = new Dataset(this.entityType), C = this.getRemovedEntities();
    for (var A = 0; A < C.length; A++)
        B.addEntity(C[A].clone(true));
    return B
};
Dataset.prototype.getModifiedDataset = function() {
    var B = new Dataset(this.entityType), C = this.getModifiedEntities();
    for (var A = 0; A < C.length; A++)
        B.addEntity(C[A].clone(true));
    return B
};
Dataset.prototype.getSubmitXML = function() {
    var C = this.getInsertEntities(), B = new StringBuffer();
    if (C.length > 0) {
        for (var A = 0; A < C.length; A++) {
        ...
        B.append("<insertEntities __isNullOrEmpty=\"null\"/>");
    ...
    if (C.length > 0) {
        ...
        B.append("<updateEntities __isNullOrEmpty=\"null\"/>");
    ...
    if (C.length > 0) {
        ...
        B.append("<deleteEntities __isNullOrEmpty=\"null\"/>");
    return B.toString()
};
...
function Dataset2Array(I, H, G) {
    ...
    for (var E = 0; E < C; E++) {
        var K = I.get(E);
        if (G)
            if (K.status == Entity.STATUS_DELT || K.status == Entity.STATUS_HIDDEN)
                continue;
        var D = Entity2Array(K), B = E + 1;
...
function Entity2Array(G) {
    ...
    var A = G.keys.length;
    for (var C = 0; C < A; C++) {
        var H = G.keys[C], E = G.getProperty(H);
...
function Dataset2Json(C, F, B) {
    F = F || [];
    for (var A = 0, E = C.values.length; A < E; A++) {
        var D = C.values[A];
        if (B !== true && (D.status == Entity.STATUS_DELT || D.status == Entity.STATUS_HIDDEN))
            continue;
...
function SimpleEditor(A) {
    var B = null;
    if (A)
        if (!A.getAttribute("__eos__editor__inited")) {
            if (A.tagName && A.tagName.toLowerCase() == "form")
                B = initFormEditor(A);
            else
                B = initSimpleEditor(A);
...
function initSimpleEditor(A) {
    ...
    A.getValue = function() {
        if (this.type == "select-multiple") {
            var B = [];
            for (var A = 0; A < this.options.length; A++)
                if (this.options[A].selected == true)
                    B.push(this.options[A].value);
            return B
        }
        return this.value
    };
    ...
    for (var C = 0; C < E.elements.length; C++) {
        ...
        if (B.name) {
            ...
            if (A && A.length > 0) {
                ...
                E.editors[B.name] = (SimpleEditor(B))
        }
    }
    return E
}
...
TimeSelect.prototype.init = function() {
    var B = this.toHtml(), A = $create("div");
    A.innerHTML = B;
    this.container = $id(this.id + "_container");
...
TimeSelect.prototype.setValue = function(A) {
    if (A === null || A === undefined || A == "")
        ...
        this.dateTime = timeFormat(A, this.format, this.__privateFormat);
    this.refresh()
};
...
TimeSelect.prototype.refreshValue = function() {
    var A = dateToStringValue(this.hourInput.value), B = dateToStringValue(this.minuteInput.value), C = dateToStringValue(this.secondInput.value);
    if (A && B && C) {
        this.dateTime = A + ":" + B + ":" + C;
...
TimeSelect.prototype.toHtml = function() {
    var B = "tabIndex=\"" + this.tabIndex + "\" accesskey=\"" + this.accesskey + "\" ", A = B + "type=\"text\" autocomplete=\"off\" onpropertychange=\"$o('" + this.id + "').setForObjValue(this)\"  maxlength=\"2\" onclick=\"this.focus();this.select()\" onfocus=\"$o('";
    ...
    var C = "";
    C += "<div id=\"" + this.id + "_time_container\" class=\"eos-time-container\" style=\"background-color:#ffffff;width: 91px;height:20px;float:left;overflow:hidden\">";
    C += "<div style=\"font-size:11pt;float:left;width: 72px;height:20px;overflow:hidden;float:left;padding-top:1px;padding-buttom:3px;padding-left:3px;\">";
    ...
    C += "</div>";
    C += "<div style=\"width: 16px;height:20px;overflow:hidden;float:right;padding-top:0px;\">";
    C += "<div style=\"width: 16px;height:9px;overflow:hidden;\">";
    ...
    C += "</div>";
    C += "</div>";
    C += "<div style=\"width: 16px;height:9px;overflow:hidden;margin-top:1px;\">";
...
Calendar.prototype.init = function() {
    ...
    var A = getFormByObj(this.inputObject);
    if (A != null)
        initEosControlReset(A, this);
    this.inited = true
};
...
Calendar.prototype.setDisplayValue = function(B) {
    var A = this.inputObject.value;
    this.inputObject.value = B;
    if (A !== B && this.inited)
...
Calendar.prototype.initEvent = function() {
    ...
    this.eventObject.onmouseover = function() {
        if (A.getDisabled() || A.getReadOnly())
            return;
...
Calendar.prototype.show = function() {
    ...
    var C = getPosition(this.container), G = C.left, D = C.top;
    ...
    var F = this.inputObject.value, E = false;
    if (F)
        if (isDate(F, this.format))
            if (this.showTime) {
    ...
    if (!E)
        F = dateToString(new Date(), this.__privateFormat);
...
Calendar.prototype.validate = function() {
    ...
    if (B || B === 0) {
        var F = isDate(this.inputObject.value, this.format), A = true;
        if (this.showTime)
            A = isFormatTime(this.inputObject.value, this.format);
        if (F && A) {
            var D = stringToDate(this.maxValue, this.__privateFormat), C = stringToDate(this.minValue, this.__privateFormat), E = stringToDate(B, this.format);
            if (E - D > 0) {
    ...
    return this.onValidate(dateFormat(B, this.format, this.__privateFormat), this.text.value)
};
...
Calendar.prototype.getDisplayValue = function(A) {
    if (A === null || A === undefined || A == "")
        return "";
    if (isDate(A, this.__privateFormat))
        return dateFormat(A, this.__privateFormat, this.format);
    else {
        if (isDate(A, "yyyy-MM-dd"))
            return dateFormat(A, "yyyy-MM-dd", this.format);
        if (isDate(A, "yyyy-MM-dd HH:mm:ss"))
            return dateFormat(A, "yyyy-MM-dd HH:mm:ss", this.format);
        if (isDate(A, "yyyy-MM-dd HH:mm:ss.S"))
            return dateFormat(A, "yyyy-MM-dd HH:mm:ss", this.format);
        return A
    }
};
...
CalendarEditor.init = function() {
    ...
    if (B)
        ...
        var E = "";
        if (isIE)
            E += "<iframe id=\"calendarIframe\" src=\"" + blankURL + "\" style=\"position:absolute;z-index:-1;\" frameborder=\"0\" ></iframe>";
        ...
        E += "<div  id=\"_eos_calendar_ok\" class=\"eos-button\" onmouseover=\"buttonMouseOver(this)\" onmouseout=\"buttonMouseOut(this)\" onmousedown=\"buttonMouseDown(this)\" onmouseup=\"buttonMouseUp(this)\"><div align=\"center\" class=\"eos-button-inner\">" + CALENDAR_BUTTON_OK + "</div></div>&nbsp;";
        E += "<div  id=\"_eos_calendar_today\" class=\"eos-button\" onmouseover=\"buttonMouseOver(this)\" onmouseout=\"buttonMouseOut(this)\" onmousedown=\"buttonMouseDown(this)\" onmouseup=\"buttonMouseUp(this)\"><div align=\"center\" class=\"eos-button-inner\">" + CALENDAR_TODAY + "</div></div>&nbsp;";
        E += "<div  id=\"_eos_calendar_clear\" class=\"eos-button\" onmouseover=\"buttonMouseOver(this)\" onmouseout=\"buttonMouseOut(this)\" onmousedown=\"buttonMouseDown(this)\" onmouseup=\"buttonMouseUp(this)\"><div align=\"center\" class=\"eos-button-inner\">" + CLEAR + "</div></div>";
        E += "            </td>";
        E += "        </tr>        ";
        E += "    </table>";
        E += "    </div>";
...
CalendarEditor.showEditor = function() {
    ...
    if (isIE) {
        var A = $id("calendarIframe", CalendarEditor.doc);
        A.style.width = A.nextSibling.offsetWidth;
        A.style.height = A.nextSibling.offsetHeight;
        A.style.top = A.nextSibling.offsetTop;
        A.style.left = A.nextSibling.offsetLeft
    }
    var B = $id("_eos_calendar_editor_container", CalendarEditor.doc);
    initShadow(B, CalendarEditor.doc)
};
...
CalendarEditor.setPosition = function(H, B, D, G) {
    CalendarEditor.container.style.position = "absolute";
    var J = getMaxZindex(CalendarEditor.doc);
    if (CalendarEditor.container.style.zIndex != J)
        CalendarEditor.container.style.zIndex = J;
    var F = CalendarEditor.container.offsetHeight, I = CalendarEditor.container.offsetWidth, K = CalendarEditor.doc.body.clientHeight, C = CalendarEditor.doc.body.clientWidth, A = K + CalendarEditor.doc.body.scrollTop, E = C + CalendarEditor.doc.body.scrollLeft;
    if (F >= K)
        B = CalendarEditor.doc.body.scrollTop;
    else if ((B + F) > A)
        B = A - F;
    if (I >= C)
        H = CalendarEditor.doc.body.scrollLeft;
    else if ((H + I) > E)
        H = E - I;
    if (window.isGecko)
        B = B - 4;
    CalendarEditor.container.style.left = H;
...
CalendarEditor.freshTimeComp = function() {
    if (!CalendarEditor.showTime)
        return;
    var A = CalendarEditor.value.getHours(), B = CalendarEditor.value.getMinutes(), C = CalendarEditor.value.getSeconds();
...
CalendarEditor.changeMonth = function(A) {
    ...
    if (A.length != 2)
        return;
    A = A - 1;
    var I = CalendarEditor.getMaxMonth(), D = CalendarEditor.getMinMonth(), E = CalendarEditor.value.getFullYear(), H = CalendarEditor.getMaxYear(), C = CalendarEditor.getMinYear();
    if (A > I && E == H)
        A = I;
    if (A < D && E == C)
        A = D;
    var G = CalendarEditor.value.getDate(), B = new Date(E, A + 1, 0).getDate();
    G = G > B ? B : G;
    var F = new Date(E, A, G);
    F.setFullYear(E);
    F = CalendarEditor.refreshTime(F);
    CalendarEditor.freshDate(dateToString(F, CalendarEditor.format))
};
CalendarEditor.changeYear = function(A) {
    ...
    if (A.length <= 3)
        return;
    var F = CalendarEditor.getMaxYear(), B = CalendarEditor.getMinYear();
    if (A > F)
        A = F;
    if (A < B)
        A = B;
    var C = CalendarEditor.value.getMonth(), E = CalendarEditor.value.getDate();
    if (C == 1 && E == 29)
        if (A % 4 != 0)
            E = 28;
...
CalendarEditor.freshDate = function(L) {
    var V = new Array(31, 30, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31), T = new Date();
    if (isDate(L, CalendarEditor.format))
        T = stringToDate(L, CalendarEditor.format);
    var O = null, N = null;
    if (isDate(CalendarEditor.maxValue, CalendarEditor.format)) {
    ...
    var Q = T.getFullYear(), G = T.getMonth(), B = 1, M = new Date(Q, G, 1).getDay(), D = G == 0 ? Q - 1 : Q, S = G == 0 ? 11 : G - 1, U = V[S];
    if (S == 1)
        U = ((D % 4 == 0) && (D % 100 != 0) || (D % 400 == 0)) ? 29 : 28;
    U -= M - 1;
    var K = 1;
    ...
    for ( i = 0; i < 42; i++) {
        ...
        if (i < M) {
            ...
            P.value = R;
            P.innerHTML = B;
            var C = new Date(), F = P.value;
            if (CalendarEditor.isHoliday(Q, G, B))
                P.className = "eos-calendar-editor-holiday";
            B++
        } else {
            P.innerHTML = "";
            P.title = ""
        ...
        if (P.value) {
            ...
            if (P.value.getYear() == E.getYear() && P.value.getMonth() == E.getMonth() && P.value.getDate() == E.getDate()) {
                ...
                removeClass(P, "eos-calendar-editor-currday");
            var A = false;
            if (O != null)
                if (P.value - O > 0)
                    A = true;
...
LookUp.prototype.init = function() {
    ...
    this.lookupBtn.onmouseover = function() {
        if (B.getDisabled() || B.getReadOnly())
            return;
    ...
    this.setReadOnly(this.readOnly);
    this.setDisabled(this.disabled);
    eventManager.add(this.lookupText, "change", A)
};
...
MultiBox.prototype.initEvent = function() {
    ...
    function E() {
        var F = eventManager.getEvent(), B = F.keyCode;
        if (!(B == 37 || B == 39))
            return;
        var A = C.checkboxes;
...
MultiBox.prototype.refreshInput = function() {
    ...
    for (var A = 0; A < this.checkboxes.length; A++) {
        var B = this.checkboxes[A], C = this.splitChar + B.value + this.splitChar;
        if (str.indexOf(C) > -1)
            B.checked = true;
        else
            B.checked = false
...
MultiBox.prototype.getDisplayValue = function(D) {
    ...
    if (EOS_DICT_DISPLAY_SEPERATOR)
        B = EOS_DICT_DISPLAY_SEPERATOR;
    else
        B = this.splitChar;
    var E = D.split(B), F = "", A = [];
    for (var C = 0; C < E.length; C++)
        A.push(this.jsonObj[E[C]] || E[C]);
    F = A.join(B);
    return F
};
...
MultiSelect.prototype.refreshInput = function() {
    ...
    for (var A = 0; A < this.select.options.length; A++) {
        var B = this.select.options[A], C = B.value + this.splitChar;
        if (str.indexOf(C) > -1)
            B.selected = true;
        else
            B.selected = false
...
PopMenu.prototype.init = function() {
    ...
    var D = $create("ul", this.doc), A = $create("div", this.doc);
    this.shadowdiv = A;
...
PopMenuItem.prototype.initSubMenu = function() {
    if (this.hasChild()) {
        ...
        if (isIE)
            C.innerHTML = "<iframe src=\"" + blankURL + "\" style=\"position:absolute;z-index:" + this.zIndex + ";width:expression(this.nextSibling.childNodes[0].offsetWidth);height:expression(this.nextSibling.childNodes[0].offsetHeight);top:expression(this.nextSibling.offsetTop);left:expression(this.nextSibling.offsetLeft);\" frameborder=\"0\" ></iframe>";
...
PopMenuItem.prototype.showSubMenu = function() {
    if (this.hasChild()) {
        ...
        var E = getPosition(this.subMenuContainer), C = this.doc || document, B = E.left + this.subMenuContainer.offsetWidth, F = C.body.clientWidth;
        if (B > F) {
...
function showPopMenu(B, D) {
    ...
    if (F) {
        F.args = D;
        var C = eventManager.getEvent(), G = C.x || C.clientX, E = C.y || C.clientY;
        if (isIE) {
...
function RTree(G, A, C, E) {
    ...
    if (isFF) {
        ...
        if (registerTopContainer(B))
            EOSResizeObjects.push(F)
    }
    return F
}

function _rtreemodel_loadNodeChild(E, M) {
    var C;
    if (E.isloadData !== true) {
        var A = this.getNodeExpandAction(E), Q = this.getExpandParam(E), J = this.getExpandRetXpaths(E);
        if (!(A && J)) {
            E.icon.src = E.openIconSrc;
            E.loadFinished = true;
            return
        }
        var N = new HideSubmit(A), F = E.entity.name, O = this.getEntityInfo(F);
        if (O.queryReturnValue)
            N.setOutParam(O.queryReturnValue);
        if (O.queryParamsInfo)
            N.setParamsInfo(O.queryParamsInfo);
        if (O.queryInoutParams)
            N.setInoutParam(O.queryInoutParams);
        N.submitXML(Q);
        var B = N.getXMLDom();
        C = Dataset.create(B, "root/data/" + J[0]);
        for (var H = 1; H < J.length; H++)
            C.appendDataset(Dataset.create(B, "root/data/" + J[H]))
    } else
        C = E.dataset;
    var S = C.values;
    if (!E.isroot) {
        if (S.length == 0) {
            ...
            E.refreshExpendIcon();
            E.icon.src = E.openIconSrc;
            E.childrenContainer.style.display = "none";
            E.loadFinished = true;
            if (M)
                M(E);
            return
    ...
    for ( H = 0; H < S.length; H++) {
        ...
        if (H == S.length - 1)
            ...
            I = false;
        D = S[H], T = this.getTreeNodeName(D), L = this.getTreeNodeIcon(D.name), R = this.getEntityInfo(D.name).iconType;
        if (!(this.getEntityInfo(D.name)).expandAction)
            P = false;
        else
            P = true;
        G = new RTreeNode(E, T, L, D, K, I, P, this.hasRoot, R);
        E.childrenContainer.appendChild(G);
        if (P && this.getEntityInfo(D.name).preload == true)
            if (this.preLoad(G) == false)
                G.setLeaf();
        G.refresh()
    }
    E.childrenContainer.style.display = "";
    E.icon.src = E.openIconSrc;
    if (M)
        M(E);
    E.loadFinished = true
...
function _rtreemodel_preload(I) {
    var C = this.getNodeExpandAction(I), H = this.getExpandParam(I), B = this.getExpandRetXpaths(I);
    I.isloadData = true;
    if (!(C && B)) {
        I.icon.src = I.openIconSrc;
        I.loadFinished = true;
        return false
    }
    var G = new HideSubmit(C), K = I.entity.name, F = this.getEntityInfo(K);
    if (F.queryReturnValue)
        G.setOutParam(F.queryReturnValue);
    if (F.queryParamsInfo)
        G.setParamsInfo(F.queryParamsInfo);
    if (F.queryInoutParams)
        G.setInoutParam(F.queryInoutParams);
    G.submitXML(H);
    var D = G.getXMLDom(), E = Dataset.create(D, "root/data/" + B[0]);
    for (var A = 1; A < B.length; A++)
        E.appendDataset(Dataset.create(D, "root/data/" + B[A]));
    I.dataset = E;
    var J = E.values;
    if (J.length == 0)
        return false;
...
function _rtreemodel_getNodeExpandAction(B) {
    var C = B.entity.name, A = this.getEntityInfo(C);
...
function _rtreemodel_getExpandParam(F) {
    var H = F.entity.name, D = this.getEntityInfo(H), B = D.InitExpandParamFunc, C = D.submitXpath, G = "", E;
    G = fireUserEvent(B, [F]);
    if (C == "" || C == null)
        E = F.entity.toString();
    else {
    ...
    return "<root><data>" + E + G + "</data></root>"
}

function _rtreemodel_initAttachParam(A) {
    if (A && isUserEventDefined(A))
        return fireUserEvent(A, []);
    else
        return ""
}

function _rtreemodel_getTreeNodeName(D) {
    var E = D.name, C = this.getEntityInfo(E), B = C.expression, A = D.getProperty(B);
    if (A == null)
        return B;
    else
        return A
...
function _rtreemodel_setEntityInfo(G, H, J, F, D, K, O, N, B, I, M, C, A, E, P) {
    G = G.toLowerCase();
    var L = this.getEntityInfo(G);
    L.expression = H;
    L.iconSrc = J;
    L.onrefresh = D;
    L.onclick = F;
    L.ondblclick = K;
    L.InitExpandParamFunc = N;
    L.childEntityXpaths = B;
    L.expandAction = I;
    L.iconType = O;
    L.submitXpath = M;
    L.preload = C;
    L.queryReturnValue = A;
    L.queryParamsInfo = E;
    L.queryInoutParams = P
}
...
function _rtreemodel_isNodeMovable(C) {
    var D = C.entity.name, A = this.getEntityInfo(D), B = A.moveActions;
    if (B.length > 0)
        return true;
    else
        return false
...
function _rtreemodel_moveNode() {
    if (isCanMoveTo(document.moveModel, document.movetoModel) == false)
        return;
    var D = this.move_node, E = document.movetoModel.moveto_node;
    this.move_node = null;
    this.moveto_node = null;
    this.move_menu.hide();
    var H = D.getParent(), M = this.getMoveAction(D, E);
    if (M) {
        var K = D.getTree().beforeMove;
        if (K)
            if (K(D, E) == false)
                return;
        var A = this.getMoveParam(D, E), I = new HideSubmit(M), F = D.entity.name, J = this.getEntityInfo(F);
        if (J.queryReturnValue)
            I.setOutParam(J.queryReturnValue);
        if (J.queryParamsInfo)
            I.setParamsInfo(J.queryParamsInfo);
        if (J.queryInoutParams)
            I.setInoutParam(J.queryInoutParams);
        I.submitXML(A);
        var B = D.getTree(), C = B.afterMove;
        if (C)
            if (B.afterMove(I) == false)
                return;
        var G, L;
        G = !D.getParent().isChildOf(E);
        L = !E.isChildOf(D.getParent());
        if (G)
            D.getParent().reloadChild();
        if (L)
            E.reloadChild()
    }
}
...
function _rtreemodel_showNodeMenu(F) {
    ...
    if (D.beforeShowMenu && D.beforeShowMenu(F) === false)
        return;
    var H = F.entity.name, E = this.getEntityInfo(H), G = E.menuItems;
    if (G.length > 0) {
        for (var A = 0; A < G.length; A++) {
            if (A != 0) {
...
function _rtreemodel_getMovemenu(B) {
    if (isIE) {
        if (B.icon)
            this.move_menu.nodeView.innerHTML = B.icon.outerHTML + B.cell.outerHTML;
        else
            this.move_menu.nodeView.innerHTML = B.cell.outerHTML;
        var A = this.move_menu.nodeView.getElementsByTagName("span");
        addClass(A[0], "RC_TREE_CELL");
        removeClass(A[0], "RC_TREE_ACTIVENODE")
    } else {
        if (B.icon)
            this.move_menu.nodeView.innerHTML = "<img style='vertical-align:bottom'src='" + B.icon.src + "'>" + B.cell.innerHTML;
        else
            this.move_menu.nodeView.innerHTML = B.cell.innerHTML;
...
function RTreeView(G, F, C, E) {
    var A = G, D = $id(G.getAttribute("id") + "_container");
    if (isFF && E == "100%") {
        var B = D;
        while (true) {
            if (B == null)
...
function _rtree_isInCurrPanel() {
    if (_eos_curr_open_panel != null) {
        ...
        while (true) {
            if (A == null)
                return false;
            if (A == _eos_curr_open_panel.table)
                return true;
            A = A.offsetParent
        }
    } else
...
function _rtree_move(G, I) {
    var B = G.getTree(), H = B.model.getMoveAction(G, I);
    if (H) {
        var A = B.model.getMoveParam(G, I), E = new HideSubmit(H), J = G.entity.name, D = B.model.getEntityInfo(J);
        if (D.queryReturnValue)
            E.setOutParam(D.queryReturnValue);
        if (D.queryParamsInfo)
            E.setParamsInfo(D.queryParamsInfo);
        if (this.queryInoutParams)
            E.setInoutParam(D.queryInoutParams);
        E.submitXML(A);
        var C = B.afterMove;
        if (C)
            if (B.afterMove(E) == false)
                return;
        var K, F;
        K = !G.getParent().isChildOf(I);
        F = !I.isChildOf(G.getParent());
        if (K)
            G.getParent().reloadChild();
        if (F)
            I.reloadChild()
    }
}
...
function _rtreeview_onkeydown() {
    ...
    function B(C) {
        if (!C)
            return null;
        var D = C.getChildren(), A = D.length;
        if (C.isExpanded() && (A > 0))
            return B(D[A - 1]);
        else
            return C
    ...
    eventManager.stopPropagation();
    var F = this, D = F.cur_node;
    if (!D)
        return;
    switch(E.keyCode) {
        case 40:
            if (D.getChildren()[0] && D.isExpanded())
                D.getChildren()[0].selected();
            else {
                var C = A(D);
                if (C)
                    C.selected()
...
function _rtreeNode_setIcon(A, B) {
    ...
    if (B)
        this.closeIconSrc = _rtree_addContextPath(B);
    else
...
function RTreeRootNode(F, I) {
    var B = I.hasRoot, K = I.getTreeNodeName(F), J = I.getTreeNodeIcon(F.name), E = document.createElement("DIV");
    E.noWrap = true;
    ...
    if (J == "")
        J = STREE_DEFAULTOPEN_ICON;
    else
        J = _rtree_addContextPath(J);
    E.openIconSrc = J;
    D.src = J;
...
function _rtreeNode_processNodeStatus(F, H, J, G, D, L, P, Q) {
    var A = F.imgLevel, N = L, M = P;
    if (N != "" && N != "null") {
        var C = N.split(",");
        if (M == "xpath") {
            if (C.length != 2) {
                if (H.getProperty(C[0]) != null)
                    F.openIconSrc = _rtree_addContextPath(H.getProperty(C[0]));
                F.closeIconSrc = F.openIconSrc
            } else {
                if (H.getProperty(C[0]) != null)
                    F.openIconSrc = _rtree_addContextPath(H.getProperty(C[0]));
                if (H.getProperty(C[1]) != null)
                    F.closeIconSrc = _rtree_addContextPath(H.getProperty(C[1]))
            }
        } else if (C.length != 2) {
...
function _rtreeNoderefreshExpendIcon() {
    var A = this.imgLevel, B = this.hasRoot, D = this.isFirstNode, C = this.isLastNode;
...
function _rtreeNode_refresh(C) {
    ...
    var E = D.getEntityInfo(this.entity.name), B = E.onrefresh;
    if (!B)
        return;
...
function _rtreenode_ondblclick() {
    eventManager.stopPropagation();
    var A = findRTree(this), D = A.model, B = this, C = eventManager.getElement();
    if (C == this.cell) {
        var E = D.getEntityInfo(this.entity.name).ondblclick;
        if (E)
            fireUserEvent(E, [this])
    }
}

function _rtreenode_oncontextmenu() {
    eventManager.stopPropagation();
    var A = findRTree(this), C = A.model, D = C.menu, B = this;
    B.selected();
...
function _rtreenode_onmousedown() {
    ...
    if (B != this.cell)
        return;
    var E = findRTree(this), D = E.model;
    if (D.isNodeMovable(this)) {
        ...
        document.onmouseup = function() {
            ...
            if (isCanMoveTo(document.moveModel, document.movetoModel) == false) {
                var B;
                if (C.move_node.icon.style.display == "none")
                    B = getPosition(C.move_node.cell);
                else
                    B = getPosition(C.move_node.icon);
                if (A.style.display != "none") {
...
function _rtreenode_onmouseover() {
    if (eventManager.getElement() != this.cell)
        return;
    var B = findRTree(this), A = B.model;
    if (__isStartDrag()) {
        eventManager.stopPropagation();
        A.moveto_node = this;
...
function EOSTreeMenu(B) {
    var A = _get_top_window(), C = A.document.createElement("div"), D = A.document.createElement("div");
    C.appendChild(D);
    C.container = D;
    ...
    C.show = function() {
        ...
        if (isIE) {
            ...
            var D = E.x || E.clientX, C = E.y || E.clientY, B = getScreenPos(window, A);
...
function EOSTreeMenuItem(D, B) {
    var A = _get_top_window(), C = A.document.createElement("div");
...
function _treemenu_clearItems() {
    ...
    if (A)
        while (A.length > 0)
        this.container.removeChild(A[0])
}
...
function _rtreemovemenu_reset(B, C) {
    ...
    if (this.style.posTop > B)
        if (this.style.posTop - B <= 3)
            this.style.posTop = this.style.posTop - 1;
        else
            this.style.posTop = this.style.posTop - (this.style.posTop - B) * 0.3;
    if (this.style.posTop < B)
        if (B - this.style.posTop <= 3)
            this.style.posTop = this.style.posTop + 1;
        else
            this.style.posTop = this.style.posTop + (B - this.style.posTop) * 0.3;
    if (this.style.posLeft > C)
        if (this.style.posLeft - C <= 3)
            this.style.posLeft = this.style.posLeft - 1;
        else
            this.style.posLeft = this.style.posLeft - (this.style.posLeft - C) * 0.3;
    if (this.style.posLeft < C)
        if (C - this.style.posLeft <= 3)
            this.style.posLeft = this.style.posLeft + 1;
        else
            this.style.posLeft = this.style.posLeft + (C - this.style.posLeft) * 0.3;
    setTimeout(function() {
...
function isCanMoveTo(E, C) {
    ...
    if (!C)
        return false;
    var B = E.move_node, D = C.moveto_node;
    if (D == null)
        return false;
    if (B == null)
        return false;
    ...
    if (D.isChildOf(B))
        return false;
    var F = E.getMoveAction(B, D), G = B.getTree().isCanMove;
    if (!F)
        return false;
    if (G)
        if (G(B, D) == false)
...
function _rree_auto_resize_step2() {
    var B = $id(this.getAttribute("id") + "_container"), A = B.parentNode;
    while (A != null)
    if (A.getAttribute && A.getAttribute("layout") != null) {
...
STree.prototype.processRelation = function() {
    ...
    for (var A = 0; A < B.length; A++) {
        var C = B[A];
        if (C.parentEntity == "root")
            _stree_linkToRoot(C, this.getDataset(C.entityName), this.getDataset(C.parentEntity));
        else
            _stree_linkToParnet(C, this.getDataset(C.entityName), this.getDataset(C.parentEntity))
    }
};
STree.prototype.processCheckBox = function() {
    ...
    for (var G = 0; G < D.length; G++) {
        ...
        if (F.entityName == F.checkedEntity) {
            var C = this.getDataset(F.entityName), K = C.values, J = F.checkedField, I = F.entityField, M = C.getLength();
            for (var L = 0; L < M; L++) {
            ...
            var C = this.getDataset(F.entityName), K = C.values, M = C.getLength(), B = this.getDataset(F.checkedEntity), N = B.values, E = B.getLength(), I = F.entityField, O = F.checkedField;
            for ( L = 0; L < M; L++) {
                H = K[L].getProperty(I);
                for (var A = 0; A < E; A++) {
                    J = N[A].getProperty(O);
...
function _stree_linkToRoot(I, F, E) {
    ...
    var D = E.childList, K = F.getLength(), H = F.values, J = I.entityName, C = I.parnetFeild, B = I.entityField;
    if (C === "ADD_ALL") {
        for (var A = 0; A < K; A++) {
            var G = H[A];
            G.type = J;
...
function _stree_linkToParnet(O, D, B) {
    var P = D.getLength(), N = D.values, H = B.getLength(), C = B.values, E = O.entityName, J = O.parnetFeild, K = O.entityField, G = J.split(","), A = K.split(",");
    if (G.length == 1) {
        for (var I = 0; I < P; I++) {
...
function STreeRootNode(F) {
    ...
    E.isRoot = true;
    var B = document.createElement("div"), G = E.entityInfo.icon;
    if (G != "")
        E.openIconSrc = _stree_addContextPath(G);
    var D;
    D = document.createElement("IMG");
    D.style.verticalAlign = "middle";
    if (E.openIconSrc == "null")
        D.style.display = "none";
    D.src = E.openIconSrc;
    B.appendChild(D);
    var A = null;
    if (E.hasCheckbox) {
        ...
        if (E.entity.checked == null || E.entity.checked == "0")
            A.src = STREE_CHECKBOX_FALSE_ICON;
        else if (E.entity.checked == "1")
            A.src = STREE_CHECKBOX_TRUE_ICON;
        else
            A.src = STREE_CHECKBOX_TRUE_ICON1;
        B.appendChild(A)
    }
    var C;
    if (E.entityInfo.url != "") {
        ...
        if (E.entityInfo.target != "")
            C.target = E.entityInfo.target
    } else {
...
function STreeNode(A, E, F, D) {
    var C = A.tree, B = document.createElement("DIV");
    B.noWrap = true;
    B.level = A.level;
    B.tree = C;
...
function _streeNode_processNodeStatus(D, F, H, E) {
    var A = D.level, N = D.entityInfo.icon, M = D.entityInfo.iconType;
    if (N != "") {
        var B = N.split(",");
        if (M == "xpath") {
            if (B.length != 2) {
    ...
    if (D.hasCheckbox) {
        ...
        if (D.entity.checked == null || D.entity.checked == "0")
            J.src = STREE_CHECKBOX_FALSE_ICON;
        else if (D.entity.checked == "1")
            J.src = STREE_CHECKBOX_TRUE_ICON;
        else
            J.src = STREE_CHECKBOX_TRUE_ICON1;
        K.appendChild(J)
    }
    var C;
    if (D.entityInfo.url != "") {
        ...
        if (D.entityInfo.target != "")
            C.target = D.entityInfo.target
    } else
        C = document.createElement("span");
    ...
    if (C.hasChild)
        if (C.isChildLoaded) {
            ...
            for (var B = 0; B < A.length; B++)
                _stree_checkAllChild(A[B])
        } else {
            A = C.entity.childList;
            for ( B = 0; B < A.length; B++)
                _stree_unload_checkAllChild(A[B])
        }
    return
}
...
function _stree_uncheckAllChild(C) {
    ...
    if (C.hasChild)
        if (C.isChildLoaded) {
            var A = C.getChildren();
            for (var B = 0; B < A.length; B++)
                _stree_uncheckAllChild(A[B])
        } else {
            A = C.entity.childList;
            for ( B = 0; B < A.length; B++)
                _stree_unload_uncheckAllChild(A[B])
        }
    return
}
...
STree.prototype.getCheckedList = function(H) {
    if (H != null) {
        ...
        var F = new CheckList(H), E = C.values, G = C.getLength();
        for (var B = 0; B < G; B++) {
            var D = E[B].checked;
            if (D == "1" || D == "2")
                F.push(E[B])
        ...
        for (H in this.datasetList) {
            ...
            F = new CheckList(H), E = C.values, G = C.getLength();
            for ( B = 0; B < G; B++) {
                D = E[B].checked;
                if (D == "1" || D == "2")
                    F.push(E[B])
...
function _streeNode_setIcon(A, B) {
    ...
    if (B)
        this.closeIconSrc = _stree_addContextPath(B);
    else
        this.closeIconSrc = this.openIconSrc;
    if (this.closeIconSrc == "null")
        this.icon.style.display = "none";
...
STree.prototype.createHiddenData = function(submitAll) {
	submitAll = submitAll==null? true:submitAll;
    var C = new StringBuffer();
    for (entityType in this.datasetList) {
        if (entityType == "root")
            continue;
        var A = 1, F = this.getDataset(entityType);
        if (F == null) {
            alert(entityType + STREE_ENTITY_NOT_EXIST);
            return
        }
        var H = F.values, I = F.getLength();
        for (var B = 0; B < I; B++) {
            var G = H[B].checked;
            var ifEnter = false;
            if (submitAll){
            ...
            if (ifEnter){
                var E = H[B].type, D = this.getEntityInfo(E).submitXpath;
                if (D != "")
                    E = D;
...
function _stree_create_hiddenStr(B, G, F) {
    var E = new StringBuffer(), A = G.getKeys();
...
STree.prototype.autoResizeS2 = function() {
    var B = this.treeContainer.parentNode, A = B.parentNode;
    while (A != null)
    if (A.getAttribute && A.getAttribute("layout") != null) {
        B.style.height = A.offsetHeight;
        A = null
...
TabPane.prototype.remove = function(C) {
    for (var B = 0; B < this.subTabs.length; B++)
        if (this.subTabs[B] == C) {
...
TabPane.prototype.getWidth = function() {
    ...
    if (String(this.width).indexOf("%") > 0)
        A = parseInt(this.container.offsetWidth);
    else
...
TabPane.prototype.refreshTitles = function() {
    this.isTitleOut = false;
    var C = this.getWidth(), D = 0, A = this.getShowTabIndex();
    for (var B = 0; B < this.subTabs.length; B++)
        if (B < A)
...
TabPane.showPrivousTab = function(C) {
    var B = $id(C), A = B.getShowTabIndex() - 1;
    B.showPilot(A)
};
...
TabPane.prototype.appendTitle = function(B) {
    var A = $create("td"), C = "<table class=\"" + TAB_STYLE_TABLE + "\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\"><tr>";
...
TabPane.prototype.getTab = function(B) {
    if ( typeof (B) == "object") {
        if ( B instanceof TabPage)
            return B
    } else {
        for (var A = 0; A < this.subTabs.length; A++)
            if (this.subTabs[A].id == B)
                return this.subTabs[A];
        if (!isNaN(B))
            if (this.subTabs[B])
                return this.subTabs[B]
    }
    return null
};
...
TabPage.prototype.setTitleTD = function() {
    var B = this.group, A = this.__titleTD, C = this;
    A.onclick = function() {
        var D = this.childNodes[0].rows[0], E = D.cells[0].getElementsByTagName("div")[0];
        E.style.backgroundImage = "";
        E = D.cells[D.cells.length-1].getElementsByTagName("div")[0];
        for (var A = 1; A < D.cells.length - 1; A++)
            D.cells[A].style.backgroundImage = "";
        E.style.backgroundImage = "";
        B.selectTab(C)
    };
    A.onmouseover = function() {
        if (C.status == TABPANE_STATUS_NOMAL) {
            var B = this.childNodes[0].rows[0], D = B.cells[0].getElementsByTagName("div")[0], E = getCurrentStyle(D, "backgroundImage");
            D.style.backgroundImage = E.replace("tabs_nomal_left", "tabs_hover_left");
            D = B.cells[B.cells.length-1].getElementsByTagName("div")[0];
            for (var A = 1; A < B.cells.length - 1; A++)
                B.cells[A].style.backgroundImage = E.replace("tabs_nomal_left", "tabs_hover_center");
    ...
    A.onmouseout = function() {
        if (C.status == TABPANE_STATUS_NOMAL) {
            var B = this.childNodes[0].rows[0], D = B.cells[0].getElementsByTagName("div")[0];
            D.style.backgroundImage = "";
            D = B.cells[B.cells.length-1].getElementsByTagName("div")[0];
            for (var A = 1; A < B.cells.length - 1; A++)
                B.cells[A].style.backgroundImage = "";
            D.style.backgroundImage = ""
        }
        eventManager.stopPropagation()
    }
...
TabPage.prototype.getUrl = function() {
    if (this.src) {
        ...
        for (var A = 0; A < this.params.length; A++) {
            var B = this.params[A];
            D += "&" + B.key + "=" + encodeURI(B.value)
        }
        var C = this.src;
        if (C.indexOf("/") == 0)
            if (C.indexOf(contextPath) != 0)
                C = contextPath + C;
        if (C.indexOf("?") < 0) {
            if (D.length > 0)
                D = "?" + D;
...
TabPage.prototype.getWindow = function() {
    ...
    if (this.type == TABPANE_TYPE_IFRAME)
        if (this.cache)
            ...
            A = this.group.noCacheIframe.contentWindow;
    return A
};
TabPage.prototype.getDocument = function() {
    ...
    if (this.type == TABPANE_TYPE_IFRAME)
        if (this.cache)
            A = this.iframe.document || this.iframe.contentDocument;
        else
            A = this.group.noCacheIframe.document || this.group.noCacheIframe.contentDocument;
    return A
};
...
ComboSelect.prototype.setValue = function(A, D) {
    var C = this.getEntityByValue(A);
    if (!C)
        return false;
    var B = C.__index;
    return this.selectOptionByIndex(B, D)
};
ComboSelect.prototype.selectOptionByIndex = function(B, C) {
    var A = this.optionsTable.tBodies[0].rows;
    if (!isNaN(B) && A && A.length > 0) {
        this.selectOption(A[B], C);
        return true
    }
    return false
};
...
ComboSelect.prototype.getEntityByValue = function(F, C) {
    var E = this.dataset ? this.dataset.getLength() : 0;
    F = "" + (F || this.hiddenObject.value || "");
    (C === true) && ( F = F.toLowerCase());
    for (var A = 0; A < E; A++) {
        var D = this.dataset.get(A);
        D.__index = A;
        var B = D ? D.getProperty(this.valueField) : null;
        if (B !== undefined && B !== null) {
            (C === true) && ( B = ("" + B).toLowerCase());
            if (F === B)
                return D
        }
    }
...
ComboSelect.prototype.getEntityByText = function(F, C) {
    var E = this.dataset ? this.dataset.getLength() : 0;
    F = "" + (F || this.inputObject.value || "");
    (C === true) && ( F = F.toLowerCase());
    for (var A = 0; A < E; A++) {
        var D = this.dataset.get(A), B = D ? D.getProperty(this.textField) : null;
        if (B !== undefined && B !== null) {
            (C === true) && ( B = ("" + B).toLowerCase());
            if (F === B) {
                D.__index = A;
...
ComboSelect.prototype.init = function() {
    ...
    if (this.isIeMode == true) {
        ...
        this.select.onchange = function() {
            var A = this.options[this.selectedIndex].entity;
            this.selectEntity = A;
            combo._onChange();
            combo.setSubComponent(A)
        };
        return
    }
...
ComboSelect.prototype.getFilterNum = function() {
    var C = 0, B = this.optionsTable.tBodies[0].rows;
    for (var A = 0; A < B.length; A++)
...
ComboSelect.prototype.doFilter = function(J, E) {
    ...
    J = J.toLowerCase();
    var F = 0, H = null, B = false;
    for (var A = 0; A < C.length; A++) {
        var I = this.dataset.get(A), D = I ? I.getProperty(this.filterField) : null;
...
ComboSelect.prototype.onInputBlur = function() {
    ...
    if (C)
        if (!F.showStatus) {
            var B = F.getFilterNum(), G = F.hiddenObject.value, D = F.getEntityByRow(F.activeOptionRow), E, A;
            if (D) {
...
ComboSelect.prototype.show = function(F) {
    PageControl.addtoStack(this);
    if (this.optionsDiv && this.optionsDiv.style && !this.disabled) {
        ...
        if (isIE) {
            var C = this.optionsDiv.firstChild;
            C.style.width = C.nextSibling.offsetWidth;
            C.style.height = C.nextSibling.offsetHeight;
            C.style.top = C.nextSibling.offsetTop;
            C.style.left = C.nextSibling.offsetLeft
        }

        var D = this.optionsDiv.firstChild.nextSibling;
        initShadow(D);
        if (isFF)
            D.nextSibling.style.width = this.optionsDiv.offsetWidth + 7
    }
};
...
ComboSelect.prototype.loadData = function() {
    ...
    if (this.onLoadData() !== false) {
        ...
        ajax.loadData(param);
        xmlDom = ajax.retDom;
        this.dataset.appendDataset(Dataset.create(xmlDom, this.xpath, null))
    }
...
ComboSelect.prototype.getEntityByRow = function(A) {
    if (!A)
        return null;
    var C = A.getAttribute("__entity_rowno") / 1, B = this.dataset.get(C);
    return B
};
...
ComboSelect.prototype.freshFromEntity = function(A) {
    if (this.queryAction) {
        if (A)
            this.queryParam = A.toString();
        else
            this.queryParam = null;
        this.loadData()
    } else if (this.linkField)
        if (A) {
            this.dataset = A.getProperty(this.linkField);
            if (!this.dataset) {
                this.dataset = new Dataset(this.linkField);
                A.setProperty(this.linkField, this.dataset)
            }
        } else
            this.dataset = new Dataset(this.linkField);
    this.refresh()
};
ComboSelect.prototype.refresh = function(E) {
    this.init();
    if (this.isIeMode == true) {
        var F = this.dataset ? this.dataset.getLength() : 0;
        while (this.select.options.length != 0)
        this.select.options.remove(this.select.options[0]);
        if (this.nullText !== null) {
            var A = new Option();
            A.text = this.nullText;
            A.value = "";
            this.select.options.add(A)
        }
        for (var Q = 0; Q < F; Q++) {
            var G = this.dataset.get(Q), C = G.getProperty(this.valueField), H = G.getProperty(this.textField), A = new Option();
            A.text = H;
            A.value = C;
            if (C == this.value)
                A.selected = true;
        ...
        A = this.select.options[this.selectedIndex || 0];
        if (A != null)
            G = A.entity;
        if (G != null)
            ;
        this.setSubComponent(G);
        return
    }
    if (this.beforeRefresh() === false)
        return;
    var K = this;
    this.dataset = E || this.dataset;
    F = this.dataset ? this.dataset.getLength() : 0;
    if (this.noOrgOptionsHeight == true)
        this.optionsHeight = ((F > 10 ? 10 : F) * 21 + 2);
    if (this.optionsDiv)
    ...
    function M() {
        var A = eventManager.getElement();
        if (A != K.eventObject && !K.inputObject.getAttribute("_isFocus") && !K.eventObject.getAttribute("_isFocus")) {
            if (!K.allowInput) {
                K.inputObject.value = K.selectEntity.getProperty(K.textField);
                K.hiddenObject.value = K.selectEntity.getProperty(K.valueField)
            }
            K.hide()
        }
    }
    ...
    function P(C) {
        ...
        if (A.tagName.toLowerCase() != "tr")
            while (( A = A.parentNode))
            if (A.tagName && A.tagName.toLowerCase() == "tr" && A.getAttribute("__entity_rowno"))
                break;
    ...
    function J(A) {
        if (K.selectOptionRow)
            removeClass(K.selectOptionRow, "eos-selectoption");
        K.optionsDiv.setAttribute("_isOver", true);
        var B = P(A);
        addClass(B, "eos-selectoption");
        if (K.activeOptionRow == B)
            return;
        if (K.activeOptionRow)
            removeClass(K.activeOptionRow, "eos-selectoption");
        K.activeOptionRow = B
    }

    function O(A) {
        K.optionsDiv.setAttribute("_isOver", false)
    }
    ...
    eventManager.add(this.optionsDiv, "mouseover", J);
    eventManager.add(this.optionsDiv, "mouseout", function() {
        setTimeout(O, 300)
    });
    eventManager.add(this.optionsDiv, "click", I);
    function D() {
        eventManager.stopPropagation()
    }
    eventManager.add(this.optionsDiv, "mousedown", D);
    var L = ["<div class=\"eos-combo-optiondiv-core\"><table  class=\"eos-combo-optiontable\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\" width=\"100%\" ><tbody>"];
    for ( Q = 0; Q < F; Q++) {
        G = this.dataset.get(Q), C = G.getProperty(this.valueField), H = G.getProperty(this.textField);
        L.push("<tr __entity_rowno=\"" + Q + "\" >");
        L.push("<td><div><nobr>");
        L.push(H == "" ? " " : H);
        L.push("</nobr></div></td>");
        if (this.showValue) {
    ...
    var N = window.isIE ? "<iframe src='" + blankURL + "' SCROLLING=\"no\" style=\"overflow:hidden;position:absolute;z-index:-1;\" frameborder=\"0\" ></iframe>" : "<span style=\"display:none;\"></span>";
    this.optionsDiv.innerHTML = N + L.join("\n");
...
ComboSelect.prototype.autoScroll = function(A) {
    var E = A.offsetTop, D = E + A.offsetHeight, C = this.optionsDivCore.scrollTop, B = C + this.optionsDivCore.clientHeight;
    if (E < C)
        this.optionsDivCore.scrollTop = E;
    else if (D > B)
        this.optionsDivCore.scrollTop = C + D - B
};
...
Datacell.prototype.refreshCell = function(E, F) {
    var B = this, A = E.getAttribute("fieldId");
    F = F || B.keys[A];
    if (A == Datacell.__INDEX_NAME || B.beforeRefreshCell(E, F) === false)
        return;
    var C = E.firstChild, D = F.viewType;
...
Datacell.prototype.getFieldsByName = function(C) {
    var B = [];
    for (var A = 0; A < this.fields.length; A++)
        if (this.fields[A].fieldName == C)
            B.push(this.fields[A]);
    return B
};
...
Datacell.prototype.setTitle = function(C, A) {
    ...
    if (C) {
        ...
        if (B && B.firstChild && B.firstChild.firstChild)
...
Datacell.prototype.getCellValue = function(B) {
    ...
    if (C) {
        var D = this.getField(B), E = D.fieldName;
...
Datacell.prototype.setCellValue = function(C, A) {
    if (!C)
        return false;
    var B = C.parentNode, D = B ? this.getEntity(B) : null;
    if (D) {
        var E = this.getField(C), F = E.fieldName;
        D.setProperty(F, A);
        this.refreshCell(C);
        this.refreshRow(B);
...
Datacell.prototype.getCellByRow = function(B, C) {
    if (isNaN(C)) {
        ...
        for (var A = 0; A < B.cells.length; A++)
            if (B.cells[A].getAttribute("fieldId") == D)
                return B.cells[A]
    } else
        return B.cells[C]
};
...
Datacell.prototype.getCellByCurrRow = function(A) {
    if (this.activeRow)
        return this.getCellByRow(this.activeRow, A);
    else
        return null
};
Datacell.prototype.getRow = function(A) {
    if ( typeof A == "number")
        return this.getRowAt(A);
    if (A.tagName && A.tagName.toLowerCase() == "td")
        return A.parentNode;
    return A
};
...
Datacell.prototype.getRowByCell = function(A) {
    if (A)
        if (A.parentNode && A.parentNode.tagName && A.parentNode.tagName.toLowerCase() == "tr")
            return A.parentNode;
    return null
};
...
Datacell.prototype.selectEventRow = function() {
    var B = eventManager.getElement(), A = getParentByTagName(B, "tr");
    this.selectRow(A);
    return A
};
Datacell.prototype.showCellEditor = function(B, A) {
    if (this.activeEditor && this.activeCell) {
        ...
        var E = this.getEntity(this.activeCell), B = this.getValueForEdit(E, this.activeField.fieldName);
        if (this.dict)
        ...
        try {
            ...
            if (this.activeEditor.setPosition) {
                ...
                var H = this.activeEditor.tagName;
                if (isIE && H && H.toLowerCase() == "select")
            ...
            if (this.activeEditor.xtype == "combo") {
                this.activeEditor.setValue(B === undefined || B === null ? "" : B, true);
                if (this.activeEditor.setDisplayValue) {
                    var F = trim(this.activeCell.innerText);
                    this.activeEditor.setDisplayValue(F)
                }
            } else {
                this.activeEditor.setValue(B === undefined || B === null ? "" : B);
                if (this.activeEditor.setDisplayValue) {
                    F = trim(this.activeCell.innerText);
                    this.activeEditor.setDisplayValue(F)
                }
            }
        } catch(G) {
...
Datacell.prototype.hideCellEditor = function() {
    if (this.activeCell && this.activeEditor) {
        if (this.editing) {
            var D = this.activeCell.parentNode, C = this.activeEditor.getValue(), F = this.activeValue, B = C;
            if (F != B) {
                ...
                if (this.onUpdateRow(this.activeCell.parentNode)) {
                    var G = this.onUpdateCell(this.activeCell, this.activeEntity, this.activeField.fieldName, C);
                    (G !== false) && this.updateEntity(this.activeEntity, this.activeField.fieldName, C);
                    ...
                    if (this.linkId && this.linkField) {
                        ...
                        if (E) {
                            if (E.activeRow)
                                E.getEntity(E.activeRow).status = Entity.STATUS_MODIFIED;
                            E.refreshAfterEdit();
                            E.isModefied = true
                        }
                    }
...
Datacell.prototype.startEdit = function() {
    ...
    if (this.beforeEdit(A, A.cellIndex, A.parentNode.rowIndex) == false)
        return;
    this.editing = true;
...
Datacell.prototype.autoScroll = function() {
    var B = this.activeCell.offsetLeft + this.tableMarginLeft, I = B + this.activeCell.offsetWidth, H = this.activeCell.offsetTop, E = H + this.activeCell.offsetHeight, G = this.bodyDiv.scrollLeft, D = this.bodyDiv.scrollTop, F = G + this.bodyDiv.clientWidth, C = D + this.bodyDiv.clientHeight, A = this.freezeHeadDiv.offsetWidth;
    if (B < G)
        this.bodyDiv.scrollLeft = B - this.tableMarginLeft;
    else if (I > F)
        this.bodyDiv.scrollLeft = G + I - F;
    if (H < D)
        this.bodyDiv.scrollTop = H;
    else if (E > C)
        this.bodyDiv.scrollTop = D + E - C
};
Datacell.prototype.setActiveCell = function(D, A) {
    if (arguments.length == 1)
        if (D != null && D.tagName.toLowerCase() == "td") {
            A = D;
            D = null
        }
    this.activeMe();
...
Datacell.prototype.initEvent = function() {
    ...
    function dblclickTable(D) {
        ...
        if (C && C !== A.containerTD) {
            ...
            A.onClickCell(C, E, F, A.activeEntity, A.fields[E], A);
            //A.onClickRow(B, F, A.activeEntity, A);
            A.onDblClickRow(B, F, A.activeEntity, A);
            A.beforeShowEditor = true;
            A.startEdit()
        }
    }
    eventManager.add(this.bodyDiv, "dblclick", dblclickTable);
    ...
    function H(D) {
        ...
        // add by bruce hua
        if ("INPUT" != C.tagName.toUpperCase()){
            eventManager.stopPropagation();
        }
        // end of add
...
Datacell.prototype.processCheckBox = function(D, B) {
    var A = this, F = D.srcElement || D.target;
    if ("INPUT" == F.tagName.toUpperCase())
        if (F.getAttribute("swidthCheckbox") == "true" && F.getAttribute("type") == "checkbox") {
            var C = A.getCellValue(B), E = A.fields[B.cellIndex];
            C = C == E.checkedValue ? E.unCheckedValue : E.checkedValue;
            A.setCellValue(B, C)
        }
};
Datacell.prototype.startColumnResize = function(B, C) {
    if (C.allowResize === false)
        return;
    B = B || window.event;
...
Datacell.prototype.setFieldEditor = function(C, A) {
    ...
    A = A || C.editId;
    if (A)
        E = Datacell.$editor($id(A));
    if (!E) {
        var B = $id(this.id + "_" + C.fieldId + "_field_editor_container") || $id(C.fieldId + "_field_editor_container"), D = firstElement(B);
        if (B && D) {
            A = D.id;
...
Datacell.prototype.initCSS = function() {
    ...
    for (var B = 0; B < this.fields.length; B++) {
        ...
        if (D.align)
            D.rules.style.textAlign = D.align;
...
Datacell.prototype.initAllRows = function(C) {
    if (!C && this.allTR && this.allTR.length > 0)
        return this.allTR;
    this.allTR = [];
...
Datacell.prototype.refreshRowStyle = function() {
    ...
    for (var A = 0; A < this.allTR.length; A++) {
        ...
        if (A % 2 == 0)
            D.className = D.className + " " + this.rowEvenStyleClass;
        else
            D.className = D.className.replace(this.rowEvenStyleClass, "");
        var B = C[A] || {};
...
Datacell.prototype.getPageRowCount = function() {
    ...
    if (this.dataset) {
        ...
        A = A - (this.dataset.getInsertEntities() ? this.dataset.getInsertEntities().length : 0);
        A = A + (this.dataset.getRemovedEntities() ? this.dataset.getRemovedEntities().length : 0)
    }
    return A
};
...
Datacell.prototype.freezeColumn = function(C) {
    ...
    for (var B = 0; B < C; B++) {
        ...
        if (H.length > 0)
            H[0].appendChild(E.frozeHeadCell)
    }
    this.initAllRows();
    ...
    for (var F = 0; F < D; F++)
        for ( B = 0; B < C; B++) {
            ...
            // add by bruce hua
            var ocolL = this.allTR[F].cells[B];

            if(ocolL.firstChild && B == 0){
                    ocolL.firstChild.innerHTML = "";
            }            
            //end of add
            H[F].appendChild(G)
        }
...
Datacell.prototype.quickSort = function(D, F, E, H, M) {
    var A = this, B = D, L = F + 1, J = L, G = A.allTR, K = G[D], I = E(this, K, H), C;
    if (M) {
        while (true) {
    ...
    L -= 1;
    if (D < L)
        A.quickSort(D, L, E, H, M);
    L += 2;
    if (L < F)
        A.quickSort(L, F, E, H, M);
    return G
};
Datacell.prototype.doSort = function(I) {
    ...
    if (I.sortAt) {
        ...
        if (I.sortAt == "server" || I.sortAt.indexOf("s") == 0) {
            var A = ["<_orderby>", "<_property>", I.fieldName, "</_property>", I.sorted ? "<_sort>" : "", I.sorted ? I.sorted : "", I.sorted ? "</_sort>" : "", "</_orderby>"];
            if (I.sorted)
    ...
    if (I.sorted)
        this.quickSort(0, this.allTR.length - 1, Datacell.getSortValue, I.fieldName, I.sorted == "desc");
    else
        this.quickSort(0, this.allTR.length - 1, Datacell.getSortDefaultValue, I.fieldName, I.sorted == "desc");
    var K = this.freezeBodyTable.tBodies[0].rows, G = this.showIndex ? 1 : 0, E = document.createDocumentFragment();
    for (var J = 0; J < this.allTR.length; J++) {
        var C = this.allTR[J];
        C.className = C.className.replace(this.rowEvenStyleClass, "");
        if (J % 2 == 0)
            C.className = C.className + " " + this.rowEvenStyleClass;
        else
            C.className = C.className.replace(this.rowEvenStyleClass, "");
        for (var D = 0; D < this.freezeNumber; D++) {
            var B = K[J].cells[D + G], F = C.cells[D].cloneNode(true);
...
Datacell.prototype.initHeadEvent = function() {
    if (this.hasInitHeadEvent)
        return;
    var A = this, C = A.fields.length;
    for (var B = 0; B < C; B++) {
        var D = A.fields[B];
        (function(C) {
            eventManager.add(C.splitter, "mousedown", function(B) {
                A.startColumnResize(B, C)
            });
            eventManager.add(C.frozeSplitter, "mousedown", function(B) {
                A.startColumnResize(B, C)
            });
            eventManager.add(C.headCell, "mouseover", function(A) {
                addClass(C.headCell, "eos-hd-row-over")
            });
            eventManager.add(C.frozeHeadCell, "mouseover", function(A) {
                addClass(C.frozeHeadCell, "eos-hd-row-over")
            });
            eventManager.add(C.headCell, "mouseout", function(A) {
                removeClass(C.headCell, "eos-hd-row-over")
            });
            eventManager.add(C.frozeHeadCell, "mouseout", function(A) {
                removeClass(C.frozeHeadCell, "eos-hd-row-over")
            });
            //alert(A.isHeadEvent);
            if (A.isHeadEvent){
                eventManager.add(C.headCell, "click", function(D) {
                    A.onClickHead(C.headCell, B, C, A);
                    A.doSort(C)
                });
                eventManager.add(C.frozeHeadCell, "click", function(B) {
                    A.doSort(C)
                })
...
Datacell.prototype.initHead = function() {
    ...
    var L = this.getSubmitXpath(), B = false;
    if (!this.styleEntity) {
    ...
    for (var I = 0; I < J; I++) {
        var E = $createElement("td"), K = this.fields[I];
        if (B)
            this.initStyleField(K);
        //add by bruce hua
        if (this.headHeight != 23){
            //alert(this.headHeight);
            E.style.height = this.headHeight;
        }
        //end of add            
        ...
        E.innerHTML = ["<div class=\"eos-hd-inner ", (window.isGecko ? "eos-hd-inner-ff " : ""), K.styleClass, "\" unselectable=\"on\" style=\"text-align:" + this.headTextAlign + ";\" >", "<span>", K.label, "</span>", "<div class=\"eos-hd-tool\" ><span class=\"eos-hd-icon\"></span><span class=\"eos-hd-split\">&#160;</span></div>", "</div>"].join("");
        A.appendChild(E);
        K.headCell = E;
...
Datacell.prototype.initStyleField = function(B) {
    if (B.editType == "entity") {
        ...
        this.styleEntity.setProperty(B.fieldName, A)
    } else if (B.defaultValue !== null && B.defaultValue !== undefined)
        this.styleEntity.setProperty(B.fieldName, B.defaultValue);
    else
        this.styleEntity.setProperty(B.fieldName, "")
};
...
Datacell.prototype.initBody = function() {
    ...
    if (this.showIndex) {
        ...
        var colN = $createElement("td", {
            innerHTML : "<div class=\"eos-hd-inner" + (window.isGecko ? " eos-hd-inner-ff " : "") + "\" style=\"width:" + (indexColumnDivWidth - 1) + "px\" >&#160;</div>",
            className : "eos-index-col"
        });
        
        //add by bruce hua
        if (this.headHeight != 23 ){
            colN.style.height = this.headHeight;
        }
    ...
    var datasetLength = !this.dataset ? 0 : this.dataset.getLength(), rowNum = this.pageSize || datasetLength;
    ...
    for ( rn = 0; rn < rowNum; ) {
        ...
        for (var cn = 0; cn < colNum; cn++) {
            var field = this.fields[cn], cellText = entity.getProperty(field.fieldName);
            if (field.onRefreshFunc) {
                if ( typeof (field.onRefreshFunc) == "string")
                    field.onRefreshFunc = eval(field.onRefreshFunc);
...
Datacell.prototype.locked = function() {
    this.endEdit();
    var A = $id(Datacell.BLOLK_ID), C = $id(Datacell.BLOLK_ID + "_IMG");
    if (!A) {
        A = $createElement("div");
...
Datacell.prototype.insertRow = function(E, D) {
    ...
    if (D && E) {
        ...
        this.dataset.addEntity(E);
        this.tbody.insertBefore(A, D);
        A.setAttribute("__entity_rowno", this.dataset.getLength() - 1);
        var B = this.freezeBodyTable.tBodies[0].rows[C], G = this.styleFreezeRow.cloneNode(true);
...
Datacell.prototype.addRow = function(F) {
    ...
    E.setAttribute("__entity_rowno", this.dataset.getLength());
    if (this.beforeAdd(E) === false) {
        E = null;
        delete E;
        return null
    }
    this.dataset.addEntity(H);
    this.tbody.appendChild(E);
    for (var B = 0; B < E.cells.length; B++)
        E.cells[B].setAttribute("fieldId", this.fields[B].fieldId);
...
Datacell.prototype.deleteRow = function(A) {
    ...
    if (this.beforeDel(A) !== false && A) {
        ...
        if (this.activeCell)
            this.endEdit();
        if (this.getEntity(A).status == Entity.STATUS_NEW)
            this.getEntity(A).status = Entity.STATUS_HIDDEN;
        else {
            var C = false;
            if (this.linkId && this.linkField) {
            ...
            this.dataset.removeEntity(this.getEntity(A), C)
        }
...
Datacell.prototype.getEntity = function(B) {
    var A = B;
    if ( typeof (A) == "number")
        A = this.table.tBodies[0].rows[A];
    if (!A)
        return null;
    if (A.tagName.toLowerCase() == "td")
        A = A.parentNode;
    var D = A.getAttribute("__entity_rowno") / 1, C = this.dataset.get(D);
    return C
};
...
Datacell.prototype.setSubComponent = function(C) {
    if (this.submitXpath && C)
        C.name = this.getSubmitXpath();
    var A = this;
    setTimeout(B, 100);
    function B() {
...
Datacell.prototype.initCell = function(B, C) {
    ...
    if (A == Datacell.__INDEX_NAME)
        return;
    var D = this.keys[A], E = D.fieldName;
...
Datacell.prototype.refreshRow = function(D, F) {
    ...
    for (var E = 0; E < I; E++) {
        ...
        if (A == Datacell.__INDEX_NAME)
            continue;
        var G = this.keys[A], H = G.fieldName, B = F.getProperty(H) ? F.getProperty(H) : "";
        if (F == this.activeEntity && B != this.activeEntityBackup.getProperty(H)) {
        	//B = this.activeEntityBackup.getProperty(H) && B == "null" 
        	if (B == '' && this.activeEntityBackup.getProperty(H) == null) {
        		//alert(this.activeEntityBackup.getProperty(H) + ' B:' + B);
        	} else {
        		try {
                    var C = cell.parentNode.rowIndex;
                    if (this.showIndex)
                        addClass(this.freezeBodyTable.tBodies[0].rows[C].cells[0], "eos-row-modefied");
                    else
...
Datacell.prototype.refreshSortTh = function(A) {
    ...
    else if (A.sortType == "_asc")
        this.sortArrow.src = PICTURE_SORT_UP
};
Datacell.prototype.setPageParam = function(D, C, B, A) {
    this.pageParam = "<page __type=\"sdo:com.eos.foundation.PageCond\"><begin>" + D + "</begin><length>" + C + "</length><count>" + B + "</count><isCount>" + (!!A) + "</isCount></page>";
...
Datacell.prototype.getQueryForm = function() {
    if (!this.entityType && !this.entityName && !this.sortParam)
        return "";
    var A = this.entityType || this.entityName || "", B = A.indexOf(":");
    if (B != -1)
        A = A.substring(B + 1);
    var C = "<_entity>" + A + "</_entity>";
    if (this.sortParam)
        C += this.sortParam;
    return this.setCriteriaParam(C)
};
Datacell.prototype.setCriteriaParam = function(C) {
    var A = "<criteria __type=\"sdo:com.primeton.das.criteria.criteriaType\">", B = "</criteria>";
    this.criteriaParam = A + C + B;
    return this.criteriaParam
};
Datacell.prototype.setQueryParam = function(A) {
    this.queryParam = A;
...
Datacell.prototype.freshPagePilot = function(G) {
    ...
    if (!G) {
        var F = 1, B = this.dataset ? this.dataset.getLength() : 0;
        if (B < F) {
            F = 0;
            B = F
        }
        for (var A = 0; A < this.__pagePilot.length; A++) {
            ...
            if (E.gotoPageText)
                E.gotoPageText.value = 1;
            if (E.pageInfoDiv) {
                E.pageInfoDiv.innerHTML = "<nobr>";
                E.pageInfoDiv.innerHTML += "<nobr>" + this.PAGE_RECORDINFO_NOCOUNT.replace("{0}", F).replace("{1}", this.dataset.getLength()) + "</nobr>";
                E.pageInfoDiv.innerHTML += "</nobr>"
            }
    ...
    var C = Number(D.getProperty("totalPage")), H = Number(D.getProperty("begin"));
    if (("" + C) != "-1")
...
Datacell.prototype.selectNextRow = function() {
    if (this.activeRow) {
        var B = nextElement(this.activeRow), A = prevElement(this.activeRow);
        if (B)
            this.selectRow(B);
        else if (A)
            this.selectRow(A)
...
Datacell.prototype.validateAll = function() {
    var E = this.activeEntity, J = this.getActiveCell(), I = this.activeRow;
    for (var A = 0; A < this.table.tBodies[0].rows.length; A++) {
        var D = this.table.tBodies[0].rows[A], G = this.getEntity(D);
        if (G.status == Entity.STATUS_NEW) {
            this.activeEntity = G;
            for (var C = 0; C < this.fields.length; C++) {
                var H = this.fields[C];
                if (H.editor) {
                    this.activeCell = D.cells[C];
                    this.activeRow = D;
            ...
            if (this.paramFormId) {
                ...
                if (E)
                    for (var B = 0; B < E.elements.length; B++) {
                        var C = E.elements[B];
                        if (C.name)
                            G.addParam(C.name, xmlConversion(getElementValue(C)))
                    }
            }
            if (this.paramList)
                for ( B = 0; B < this.paramList.length; B++) {
                    C = this.paramList[B];
                    if (C)
                        G.addParam(C.key, C.value)
                }
            if (this.submitReturnValue)
                G.setOutParam(this.submitReturnValue);
            if (this.submitParamsInfo)
                G.setParamsInfo(this.submitParamsInfo);
            if (this.submitInoutParams)
                G.setInoutParam(this.submitInoutParams);
            G.loadData(this.dataset.getSubmitXML());
            if (G.retDom) {
...
Datacell.prototype.setReadonly = function(B) {
    if (B == null)
        B = true;
    this.readonly = B;
    for (var A = 0; A < this.fields.length; A++)
        this.fields[A].allowModify = !this.readonly
};
...
Datacell.prototype.submitAllByXML = function() {
    ...
    var C = this.getSubmitXpath(), A = $createElement("input", {
        type : "hidden",
        name : "insertEntities",
        value : this.getEntitiesXML(C, this.dataset.getAlltEntities())
    });
    B.appendChild(A);
...
Datacell.prototype.submitModifyByXML = function(F, A, G, C) {
    ...
    var D = [];
    D[0] = this.getEntitiesXML(F || "insertEntities", this.dataset.getInsertEntities());
    D[1] = this.getEntitiesXML(A || "updateEntities", this.dataset.getModifiedEntities());
    D[2] = this.getEntitiesXML(G || "deleteEntities", this.dataset.getRemovedEntities());
...
Datacell.prototype.submitModifyBy3XML = function(D, A, F) {
    ...
    var E = $createElement("input", {
        ...
        value : this.getEntitiesXML(D || "insertEntities", this.dataset.getInsertEntities())
    }), G = $createElement("input", {
        type : "text",
        name : A || "updateEntities",
        value : this.getEntitiesXML(A || "updateEntities", this.dataset.getModifiedEntities())
    }), C = $createElement("input", {
        type : "hidden",
        name : F || "deleteEntities",
        value : this.getEntitiesXML(F || "deleteEntities", this.dataset.getRemovedEntities())
    });
    B.appendChild(E);
    B.appendChild(G);
    B.appendChild(C);
...
Datacell.prototype.submitAllByHidden = function() {
    ...
    var B = this.dataset.name, C = this.getSubmitXpath(), D = Dataset2Array(this.dataset, C, true);
...
Datacell.prototype.submitModifyByHidden = function(C, A, D) {
    ...
    var E = Dataset2Array(this.dataset.getInsertDataset(), C || "insertEntities");
    E = E.concat(Dataset2Array(this.dataset.getModifiedDataset(), A || "updateEntities"));
    E = E.concat(Dataset2Array(this.dataset.getRemovedDataset(), D || "deleteEntities"));
...
Pilot.prototype.build = function() {
    ...
    if (B && H) {
        var N = {}, O = this.datacell.pageSizeList.split(",");
    ...
    function T() {
        C.endEdit();
        if (M.firstPageButton._disabled)
            return;
        M.gotoFirstPage()
    }
    ...
    function G() {
        eventManager.stopEvent();
        C.endEdit();
        if (M.saveButton._disabled)
            return;
        var B = false;
        for (var A = 0; A < M.datacell.fields.length; A++)
            if (M.datacell.fields[A].allowModify == true) {
                B = true;
                break
            }
        if (!B)
    ...
    function A() {
        C.endEdit();
        if (M.reloadButton._disabled)
            return;
        var A = true;
        if (M.datacell.isModefied && confirm(DATACELL_MODIFY_CONFIRM))
            M.datacell.submit();
        else
            M.datacell.reload()
    }
...
Pilot.prototype.freshPilot = function(M, E, K, I) {
    var A = true, N = true, B = true, F = true, C = false;
    if (M) {
        C = true;
        ...
        var H = isNaN(this.currentPage / 1) || this.currentPage < 1 ? 1 : this.currentPage, J = isNaN(this.totalPage / 1) || this.totalPage < 1 ? 1 : this.totalPage, G = isNaN(this.count / 1) || this.count < 0 ? 0 : this.count, D = Number(isNaN(this.begin / 1) || this.begin < 0 ? 0 : this.begin) + 1, L = D + this.datacell.pageSize - 1;
        L = L > G ? G : L;
        L = L < D ? D : L;
        if (this.gotoPageText)
            this.gotoPageText.value = H;
        if (this.pageInfoDiv) {
            this.pageInfoDiv.innerHTML = "<nobr>";
            if (!this.datacell.isCount || G < 0)
                this.pageInfoDiv.innerHTML += "<nobr>" + this.datacell.PAGE_RECORDINFO_NOCOUNT.replace("{0}", D).replace("{1}", L) + "</nobr>";
            else {
    ...
    if (this.firstPageButton)
        this.firstPageButton._disabled = this.isFirst || !C;
    if (this.prePageButton)
        this.prePageButton._disabled = this.isFirst || !C;
    if (this.nextPageButton)
        this.nextPageButton._disabled = this.isLast || !C;
    if (this.lastPageButton)
        this.lastPageButton._disabled = this.isLast || this.count < 0 || !C;
    if (this.isFirst || !C) {
...
Datacell.prototype.rowMoveUp = function(B) {
    ...
    if (!B)
        return;
    var F = B.parentNode, C = B.rowIndex, D = B.getAttribute("__entity_rowno");
    if (C < 1)
        return;
    var E = F.rows[C - 1], A = E.getAttribute("__entity_rowno");
    F.insertBefore(B, E)
};
...
Datacell.prototype.setPosition = function(C, A, B, D) {
    ...
    var E = getMaxZindex(), F = getMaxZindex(document);
    if (this.datacellContainer.style.zIndex != F)
        this.datacellContainer.style.zIndex = F;
...
Datacell.prototype.onDblClickRow = function(D, C, B, A) {
};
Datacell.activeGrid = null;
...
Datacell.outClick = function(A, K, G) {
    var B = A.datacell, F = B.editorContainer;
    if (F && K) {
        var L = G.x, J = G.y, D = getPosition(F), E = D.top, H = D.left, C = H + F.offsetWidth, I = E + F.offsetHeight;
        if (!F.contains(K))
            if (E > J || H > L || C < L || I < J)
                B.endEdit()
    } else
...
Datacell.isCell = function(A) {
    if (A && A.tagName) {
        var B = A.tagName.toLowerCase() == "td", C = A.getAttribute("fieldId");
        if (C && B)
            return true;
        return false
    }
...
Datacell.isHead = function(A) {
    if (A.tagName) {
        var B = A.tagName.toLowerCase() == "th", C = A.getAttribute("__type") == "head";
        if (C && B)
            return true;
        return false
    }
...
Datacell.getDatacellByCell = function(C) {
    try {
        var A = getParentByTagName(C, "table"), B = A.datacell;
        return B
    } catch(D) {
        $handle(D);
        return null
...
Datacell.getFormatValue = function(A, B) {
    if (isDateFormat(B)) {
        if (isTimeFormat(B))
            return dateFormat(A, DATETIME_FORMAT, B);
        else
            return dateFormat(A, DATE_FORMAT, B)
    } else if (isTimeFormat(B))
        return timeFormat(A, TIME_FORMAT, B);
    else
...
Datacell.getSortValue = function(C, B, E) {
    var D = C.dataset.get(B.getAttribute("__entity_rowno") / 1).getProperty(E), A = D ? Number(D) : D;
    D = isNaN(A) ? D : A;
    return D
};
...
function getDatacellRoot(B) {
    var A = getParentByTagName(B, "table");
    if (A) {
        if (A.className && A.className.indexOf("eos-grid-container") != -1)
            return A;
        else if (A)
            return getDatacellRoot(A)
    } else
        return null
}
Datacell.initGlobalEvent = function() {
    eventManager.add(document, "keydown", function(A) {
        Datacell.activeGrid && Datacell.activeGrid._onKeyDown(A)
    });
    eventManager.add(document.body, "click", function(B) {
        var A = B.srcElement || B.target;
        if (!getDatacellRoot(A))
            Datacell.activeGrid = null
    });
    eventManager.add(document.body, "mousemove", function(A) {
        Datacell.activeGrid && Datacell.doDocGridHandler(A, Datacell.activeGrid)
    });
    eventManager.add(document.body, "mouseup", function(A) {
...
Datacell.endDocGridHandler = function(E, A) {
    ...
    if (A.splitLine.style.display == "block") {
        ...
        var G = F.newRightX - F.oldRightX, B = parseInt(CSSUtil.getRule("." + F.styleClass, true).style.width), D = G + B;
...
Datacell.prototype.autoResizeS1 = function() {
    ...
    A.bodyDivWidth = A.containerTD.offsetWidth;
    var B = false;
    ...
    if (A.height.indexOf("%") != -1) {
        ...
        if (A.bodyDivHeight > 0)
            A.bodyDiv.style.height = A.bodyDivHeight + "px";
        if (A.bodyDiv.clientHeight > 0)
            A.freezeBodyDiv.style.height = A.bodyDiv.clientHeight + "px"
    }
    if (B)
        setTimeout(function() {
...
Datacell.prototype.autoResizeS2 = function() {
    ...
    if (A.width.indexOf("%") != -1)
        if (A.bodyDivWidth > 0)
            A.bodyDiv.style.width = A.bodyDivWidth + "px"
};
...
Datacell.createSwidthCheckBox = function(C, H, G, F, A) {
    var I = A.getField(F), D = I.checkedValue, E = I.unCheckedValue, B = "";
    if (D == C)
        B = "checked";
