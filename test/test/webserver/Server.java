...
//			Server s = new Server(System.out, 2);
//
...
//	}
	...
	public static class HTTPMirror implements Service {
		public void serve(InputStream i, OutputStream o) throws IOException {
			BufferedReader in = new BufferedReader(new InputStreamReader(i));
			PrintWriter out = new PrintWriter(o);
			out.print("Welcome to the line mirror server.\n");
			for (;;) {
				out.print("> ");
				out.flush();
				String line = in.readLine();
	...
	public static class Control implements Service {
		...
		private String getClassNameFromServiceName(String serviceName)
		{
			if(serviceName.equals("time"))
				return "net.Server$Time";
			else if(serviceName.equals("reverse"))
				return "net.Server$Reverse";
			else if(serviceName.equals("mirror"))
				return "net.Server$HTTPMirror";
			else if(serviceName.equals("uniqueid"))
				return "net.Server$UniqueID";
			return "";
		}
				...
				try {
					...
					if (command.equals("password")) { // Password command
						...
						if (!authorized)
							...
							Class serviceClass = Class.forName(getClassNameFromServiceName(serviceName));
